...
--TEST--
Deprecated VM coercions (float/null offsets, false->array, dynamic prop, string ++/--) are rejected
--SKIPIF--
<?php if (function_exists('zend_version')) echo 'skip PHL removes what php only deprecates'; ?>
--FILE--
<?php
// php only DEPRECATES these; PHL targets php's non-deprecated surface and rejects them.
...
tc(function () { $a = [1.5 => "v"]; });
// null array offset (read / write / literal)
tc(function () { $a = ["x" => 1]; return $a[null]; });
tc(function () { $a = []; $a[null] = 1; });
tc(function () { $a = [null => "v"]; });
...
TypeError: Cannot access offset of type float on array
TypeError: Cannot access offset of type float on array
TypeError: Cannot access offset of type null on array
TypeError: Cannot access offset of type null on array
TypeError: Cannot access offset of type null on array
