...
--TEST--
implode()/join() give every call form php's DIRECT-call contract, and name the invoked alias (PHL half)
--DESCRIPTION--
...
which is the form real code writes and the one the corpus already pins. This half asserts that;
the `_zend` twin records php's generic-path answers for the same three shapes.
--SKIPIF--
<?php
if (function_exists('zend_version')) {
    echo "skip PHL half of the twin pair; php's call-form split is in the _zend twin";
}
?>
--FILE--
<?php
...
join(obj, arr): TypeError: join(): Argument #1 ($separator) must be of type string, ImBare given
join(arr, obj): TypeError: join(): Argument #1 ($separator) must be of type string, array given
join(arr, int): TypeError: join(): Argument #1 ($separator) must be of type string, array given
join(arr, null): TypeError: join(): Argument #1 ($separator) must be of type string, array given
