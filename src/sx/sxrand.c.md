...
|        - |  129 | ` */` |
| 28957340 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28957345 |  135 | `  pCtx->i++;` |
| 28957345 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28957345 |  137 | `  pCtx->j += t;` |
| 28957345 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28957345 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28957345 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28957345 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2898780 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2898785 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2898785 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2898785 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4345669 |  155 | `	for(;;){` |
|  8691343 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8686105 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8686105 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5792587 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2898785 |  161 | `	return SXRET_OK;` |
|  1449395 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      387 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      389 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
