...
|        - |  129 | ` */` |
| 29002148 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29002153 |  135 | `  pCtx->i++;` |
| 29002153 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29002153 |  137 | `  pCtx->j += t;` |
| 29002153 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29002153 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29002153 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29002153 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2903256 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2903261 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2903261 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2903261 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4352389 |  155 | `	for(;;){` |
|  8704779 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8699549 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8699549 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5801547 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2903261 |  161 | `	return SXRET_OK;` |
|  1451634 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
