...
|        - |  129 | ` */` |
| 23826484 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23826489 |  135 | `  pCtx->i++;` |
| 23826489 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23826489 |  137 | `  pCtx->j += t;` |
| 23826489 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23826489 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23826489 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23826489 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2385665 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2385670 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2385670 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2385670 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3576026 |  155 | `	for(;;){` |
|  7152047 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146858 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146858 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4766406 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2385670 |  161 | `	return SXRET_OK;` |
|  1192840 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      191 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      191 |  204 | `	return SXRET_OK;` |
