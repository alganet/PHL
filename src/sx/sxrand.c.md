...
|        - |  129 | ` */` |
| 24017968 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24017973 |  135 | `  pCtx->i++;` |
| 24017973 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24017973 |  137 | `  pCtx->j += t;` |
| 24017973 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24017973 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24017973 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24017973 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2404818 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2404823 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2404823 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2404823 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3604748 |  155 | `	for(;;){` |
|  7209501 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7204299 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7204299 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4804707 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2404823 |  161 | `	return SXRET_OK;` |
|  1202414 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  206 | `	{` |
|      176 |  207 | `		sxu32 nDone = 0;` |
|      352 |  208 | `		while( nDone < nLen ){` |
|      176 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      176 |  210 | `			if( n > 0 ){` |
|      176 |  211 | `				nDone += (sxu32)n;` |
|      176 |  212 | `				continue;` |
|        - |  213 | `			}` |
