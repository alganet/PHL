...
|        - |  129 | ` */` |
| 26492608 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26492613 |  135 | `  pCtx->i++;` |
| 26492613 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26492613 |  137 | `  pCtx->j += t;` |
| 26492613 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26492613 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26492613 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26492613 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2652318 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2652323 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2652323 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2652323 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3975968 |  155 | `	for(;;){` |
|  7951945 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946675 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946675 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5299651 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2652323 |  161 | `	return SXRET_OK;` |
|  1326163 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
