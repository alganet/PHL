...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3784 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 | `{` |
|    3789 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3784 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3784 |   70 | `	if (fd >= 0 ){` |
|    3784 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3784 |   72 | `			close(fd);` |
|    3784 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3784 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 | `{` |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3789 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3789 |  106 | `	if( xSeed == 0 ){` |
|    3789 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1892 |  108 | `	}` |
|    3789 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3789 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3789 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  972493 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  968709 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  484357 |  116 | `    }` |
|  972493 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  968709 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  968709 |  119 | `      t = pCtx->s[pCtx->j];` |
|  968709 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  968709 |  121 | `      pCtx->s[i] = t;` |
|  484357 |  122 | `    }` |
|    3789 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3789 |  125 | `	return SXRET_OK;` |
|    1897 |  126 | `}` |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2865216 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2865221 |  135 | `  pCtx->i++;` |
| 2865221 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2865221 |  137 | `  pCtx->j += t;` |
| 2865221 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2865221 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2865221 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2865221 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  288596 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  288601 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  288601 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  288601 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  431200 |  155 | `	for(;;){` |
|  862405 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  858797 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  858797 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  573833 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  288601 |  161 | `	return SXRET_OK;` |
|  144303 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
