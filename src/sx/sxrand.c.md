...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3296 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3301 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3296 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3296 |   70 | `	if (fd >= 0 ){` |
|    3296 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3296 |   72 | `			close(fd);` |
|    3296 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3296 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3301 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3301 |  106 | `	if( xSeed == 0 ){` |
|    3301 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1648 |  108 | `	}` |
|    3301 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3301 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3301 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  847077 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  843781 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  421893 |  116 | `    }` |
|  847077 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  843781 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  843781 |  119 | `      t = pCtx->s[pCtx->j];` |
|  843781 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  843781 |  121 | `      pCtx->s[i] = t;` |
|  421893 |  122 | `    }` |
|    3301 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3301 |  125 | `	return SXRET_OK;` |
|    1653 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2494856 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2494861 |  135 | `  pCtx->i++;` |
| 2494861 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2494861 |  137 | `  pCtx->j += t;` |
| 2494861 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2494861 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2494861 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2494861 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  251288 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  251293 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  251293 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  251293 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  375464 |  155 | `	for(;;){` |
|  750933 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  747781 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  747781 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  499669 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  251293 |  161 | `	return SXRET_OK;` |
|  125649 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
