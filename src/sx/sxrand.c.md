...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3406 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3411 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3406 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3406 |   70 | `	if (fd >= 0 ){` |
|    3406 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3406 |   72 | `			close(fd);` |
|    3406 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3406 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3411 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3411 |  106 | `	if( xSeed == 0 ){` |
|    3411 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1703 |  108 | `	}` |
|    3411 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3411 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3411 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  875347 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  871941 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  435973 |  116 | `    }` |
|  875347 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  871941 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  871941 |  119 | `      t = pCtx->s[pCtx->j];` |
|  871941 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  871941 |  121 | `      pCtx->s[i] = t;` |
|  435973 |  122 | `    }` |
|    3411 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3411 |  125 | `	return SXRET_OK;` |
|    1708 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2578692 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2578697 |  135 | `  pCtx->i++;` |
| 2578697 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2578697 |  137 | `  pCtx->j += t;` |
| 2578697 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2578697 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2578697 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2578697 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  259736 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  259741 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  259741 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  259741 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  388083 |  155 | `	for(;;){` |
|  776171 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  772909 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  772909 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  516459 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  259741 |  161 | `	return SXRET_OK;` |
|  129873 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
