...
|       - |  129 | ` */` |
| 2917404 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2917409 |  135 | `  pCtx->i++;` |
| 2917409 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2917409 |  137 | `  pCtx->j += t;` |
| 2917409 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2917409 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2917409 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2917409 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  294824 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  294829 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  294829 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  294829 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  439701 |  155 | `	for(;;){` |
|  879407 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  874117 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  874117 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  584607 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  294829 |  161 | `	return SXRET_OK;` |
|  147417 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
