...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3948 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3953 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3948 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3948 |   70 | `	if (fd >= 0 ){` |
|     3948 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3948 |   72 | `			close(fd);` |
|     3948 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3948 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3953 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3953 |  106 | `	if( xSeed == 0 ){` |
|     3953 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1974 |  108 | `	}` |
|     3953 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3953 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3953 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1014641 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  1010693 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   505349 |  116 | `    }` |
|  1014641 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  1010693 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  1010693 |  119 | `      t = pCtx->s[pCtx->j];` |
|  1010693 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  1010693 |  121 | `      pCtx->s[i] = t;` |
|   505349 |  122 | `    }` |
|     3953 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3953 |  125 | `	return SXRET_OK;` |
|     1979 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 24675744 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24675749 |  135 | `  pCtx->i++;` |
| 24675749 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24675749 |  137 | `  pCtx->j += t;` |
| 24675749 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24675749 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24675749 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24675749 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2470714 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2470719 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2470719 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2470719 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3703492 |  155 | `	for(;;){` |
|  7406985 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7401599 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7401599 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4936295 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2470719 |  161 | `	return SXRET_OK;` |
|  1235363 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
