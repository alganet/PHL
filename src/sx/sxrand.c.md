...
|      - |   32 | `#endif` |
|   2460 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   2462 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   2460 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   2460 |   55 | `	if (fd >= 0 ){` |
|   2460 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   2460 |   57 | `			close(fd);` |
|   2460 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|   1232 |   73 |  |
|   2460 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   2462 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   2462 |   88 | `	if( xSeed == 0 ){` |
|   2462 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|   1230 |   90 | `	}` |
|   2462 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   2462 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   2462 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 632222 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 629762 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 314882 |   98 | `    }` |
| 632222 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 629762 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 629762 |  101 | `      t = pCtx->s[pCtx->j];` |
| 629762 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 629762 |  103 | `      pCtx->s[i] = t;` |
| 314882 |  104 | `    }` |
|   2462 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   2462 |  107 | `	return SXRET_OK;` |
|   1232 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 727808 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 727810 |  117 | `  pCtx->i++;` |
| 727810 |  118 | `  t = pCtx->s[pCtx->i];` |
| 727810 |  119 | `  pCtx->j += t;` |
| 727810 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 727810 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 727810 |  122 | `  t += pCtx->s[pCtx->i];` |
| 727810 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  74141 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  74143 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  74143 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  74143 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
| 110113 |  137 | `	for(;;){` |
| 220226 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 217811 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 217811 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 146109 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  74143 |  143 | `	return SXRET_OK;` |
|  37073 |  144 |  |
|      - |  145 |  |
