...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3808 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3813 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3808 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3808 |   70 | `	if (fd >= 0 ){` |
|     3808 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3808 |   72 | `			close(fd);` |
|     3808 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3808 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3813 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3813 |  106 | `	if( xSeed == 0 ){` |
|     3813 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1904 |  108 | `	}` |
|     3813 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3813 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3813 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   978661 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   974853 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   487429 |  116 | `    }` |
|   978661 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   974853 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   974853 |  119 | `      t = pCtx->s[pCtx->j];` |
|   974853 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   974853 |  121 | `      pCtx->s[i] = t;` |
|   487429 |  122 | `    }` |
|     3813 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3813 |  125 | `	return SXRET_OK;` |
|     1909 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23801496 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23801501 |  135 | `  pCtx->i++;` |
| 23801501 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23801501 |  137 | `  pCtx->j += t;` |
| 23801501 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23801501 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23801501 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23801501 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2383162 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2383167 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2383167 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2383167 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3572270 |  155 | `	for(;;){` |
|  7144545 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7139363 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7139363 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4761407 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2383167 |  161 | `	return SXRET_OK;` |
|  1191586 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      393 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      395 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
