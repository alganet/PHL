...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3954 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3959 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3954 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3954 |   70 | `	if (fd >= 0 ){` |
|     3954 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3954 |   72 | `			close(fd);` |
|     3954 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3954 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3959 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3959 |  106 | `	if( xSeed == 0 ){` |
|     3959 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1977 |  108 | `	}` |
|     3959 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3959 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3959 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1016183 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  1012229 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   506117 |  116 | `    }` |
|  1016183 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  1012229 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  1012229 |  119 | `      t = pCtx->s[pCtx->j];` |
|  1012229 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  1012229 |  121 | `      pCtx->s[i] = t;` |
|   506117 |  122 | `    }` |
|     3959 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3959 |  125 | `	return SXRET_OK;` |
|     1982 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 24713208 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24713213 |  135 | `  pCtx->i++;` |
| 24713213 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24713213 |  137 | `  pCtx->j += t;` |
| 24713213 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24713213 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24713213 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24713213 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2474464 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2474469 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2474469 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2474469 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3709114 |  155 | `	for(;;){` |
|  7418229 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7412837 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7412837 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4943789 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2474469 |  161 | `	return SXRET_OK;` |
|  1237238 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      374 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      376 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      195 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      195 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      179 |  207 | `		sxu32 nDone = 0;` |
|      358 |  208 | `		while( nDone < nLen ){` |
|      179 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      179 |  210 | `			if( n > 0 ){` |
|      179 |  211 | `				nDone += (sxu32)n;` |
|      179 |  212 | `				continue;` |
|        - |  213 | `			}` |
