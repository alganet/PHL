...
|      - |   32 | `#endif` |
|   1894 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1896 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1894 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1894 |   55 | `	if (fd >= 0 ){` |
|   1894 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1894 |   57 | `			close(fd);` |
|   1894 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    949 |   73 |  |
|   1894 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1896 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1896 |   88 | `	if( xSeed == 0 ){` |
|   1896 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    947 |   90 | `	}` |
|   1896 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1896 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1896 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 486760 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 484866 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 242434 |   98 | `    }` |
| 486760 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 484866 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 484866 |  101 | `      t = pCtx->s[pCtx->j];` |
| 484866 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 484866 |  103 | `      pCtx->s[i] = t;` |
| 242434 |  104 | `    }` |
|   1896 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1896 |  107 | `	return SXRET_OK;` |
|    949 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 559880 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 559882 |  117 | `  pCtx->i++;` |
| 559882 |  118 | `  t = pCtx->s[pCtx->i];` |
| 559882 |  119 | `  pCtx->j += t;` |
| 559882 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 559882 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 559882 |  122 | `  t += pCtx->s[pCtx->i];` |
| 559882 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  57008 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  57010 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  57010 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  57010 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  84696 |  137 | `	for(;;){` |
| 169394 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 167546 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 167546 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 112410 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  57010 |  143 | `	return SXRET_OK;` |
|  28506 |  144 |  |
|      - |  145 |  |
