...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3540 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3545 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3540 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3540 |   70 | `	if (fd >= 0 ){` |
|    3540 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3540 |   72 | `			close(fd);` |
|    3540 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3540 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3545 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3545 |  106 | `	if( xSeed == 0 ){` |
|    3545 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1770 |  108 | `	}` |
|    3545 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3545 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3545 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  909785 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  906245 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  453125 |  116 | `    }` |
|  909785 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  906245 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  906245 |  119 | `      t = pCtx->s[pCtx->j];` |
|  906245 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  906245 |  121 | `      pCtx->s[i] = t;` |
|  453125 |  122 | `    }` |
|    3545 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3545 |  125 | `	return SXRET_OK;` |
|    1775 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2716116 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2716121 |  135 | `  pCtx->i++;` |
| 2716121 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2716121 |  137 | `  pCtx->j += t;` |
| 2716121 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2716121 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2716121 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2716121 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  273557 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  273562 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  273562 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  273562 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  408750 |  155 | `	for(;;){` |
|  817503 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  814110 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  814110 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  543970 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  273562 |  161 | `	return SXRET_OK;` |
|  136784 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
