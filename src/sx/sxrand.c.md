...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3544 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3549 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3544 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3544 |   70 | `	if (fd >= 0 ){` |
|    3544 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3544 |   72 | `			close(fd);` |
|    3544 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3544 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3549 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3549 |  106 | `	if( xSeed == 0 ){` |
|    3549 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1772 |  108 | `	}` |
|    3549 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3549 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3549 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  910813 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  907269 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  453637 |  116 | `    }` |
|  910813 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  907269 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  907269 |  119 | `      t = pCtx->s[pCtx->j];` |
|  907269 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  907269 |  121 | `      pCtx->s[i] = t;` |
|  453637 |  122 | `    }` |
|    3549 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3549 |  125 | `	return SXRET_OK;` |
|    1777 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2684068 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2684073 |  135 | `  pCtx->i++;` |
| 2684073 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2684073 |  137 | `  pCtx->j += t;` |
| 2684073 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2684073 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2684073 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2684073 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  270354 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  270359 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  270359 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  270359 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  403943 |  155 | `	for(;;){` |
|  807891 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  804495 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  804495 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  537561 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  270359 |  161 | `	return SXRET_OK;` |
|  135182 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
