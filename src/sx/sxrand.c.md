...
|       - |   32 | `#endif` |
|    2576 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2578 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2576 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2576 |   55 | `	if (fd >= 0 ){` |
|    2576 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2576 |   57 | `			close(fd);` |
|    2576 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1290 |   73 |  |
|    2576 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2578 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2578 |   88 | `	if( xSeed == 0 ){` |
|    2578 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1288 |   90 | `	}` |
|    2578 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2578 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2578 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  662034 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  659458 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  329730 |   98 | `    }` |
|  662034 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  659458 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  659458 |  101 | `      t = pCtx->s[pCtx->j];` |
|  659458 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  659458 |  103 | `      pCtx->s[i] = t;` |
|  329730 |  104 | `    }` |
|    2578 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2578 |  107 | `	return SXRET_OK;` |
|    1290 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1252184 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1252186 |  117 | `  pCtx->i++;` |
| 1252186 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1252186 |  119 | `  pCtx->j += t;` |
| 1252186 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1252186 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1252186 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1252186 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  126635 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  126637 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  126637 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  126637 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  188805 |  137 | `	for(;;){` |
|  377614 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  375105 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  375105 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  251003 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  126637 |  143 | `	return SXRET_OK;` |
|   63319 |  144 |  |
|       - |  145 |  |
