...
|      - |   32 | `#endif` |
|   1594 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1596 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1594 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1594 |   55 | `	if (fd >= 0 ){` |
|   1594 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1594 |   57 | `			close(fd);` |
|   1594 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    799 |   73 |  |
|   1594 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1596 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1596 |   88 | `	if( xSeed == 0 ){` |
|   1596 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    797 |   90 | `	}` |
|   1596 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1596 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1596 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 409660 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 408066 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 204034 |   98 | `    }` |
| 409660 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 408066 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 408066 |  101 | `      t = pCtx->s[pCtx->j];` |
| 408066 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 408066 |  103 | `      pCtx->s[i] = t;` |
| 204034 |  104 | `    }` |
|   1596 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1596 |  107 | `	return SXRET_OK;` |
|    799 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 471564 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 471566 |  117 | `  pCtx->i++;` |
| 471566 |  118 | `  t = pCtx->s[pCtx->i];` |
| 471566 |  119 | `  pCtx->j += t;` |
| 471566 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 471566 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 471566 |  122 | `  t += pCtx->s[pCtx->i];` |
| 471566 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  47985 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  47987 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  47987 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  47987 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  71320 |  137 | `	for(;;){` |
| 142644 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 141115 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 141115 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  94683 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  47987 |  143 | `	return SXRET_OK;` |
|  23994 |  144 |  |
|      - |  145 |  |
