...
|        - |  129 | ` */` |
| 29031944 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29031949 |  135 | `  pCtx->i++;` |
| 29031949 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29031949 |  137 | `  pCtx->j += t;` |
| 29031949 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29031949 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29031949 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29031949 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2906241 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2906246 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2906246 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2906246 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4356859 |  155 | `	for(;;){` |
|  8713725 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708486 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708486 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5807508 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2906246 |  161 | `	return SXRET_OK;` |
|  1453125 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      356 |  208 | `		while( nDone < nLen ){` |
|      178 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      178 |  210 | `			if( n > 0 ){` |
|      178 |  211 | `				nDone += (sxu32)n;` |
|      178 |  212 | `				continue;` |
|        - |  213 | `			}` |
