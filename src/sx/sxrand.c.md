...
|        - |  129 | ` */` |
| 23826476 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23826481 |  135 | `  pCtx->i++;` |
| 23826481 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23826481 |  137 | `  pCtx->j += t;` |
| 23826481 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23826481 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23826481 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23826481 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2385663 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2385668 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2385668 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2385668 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3576020 |  155 | `	for(;;){` |
|  7152043 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146856 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146856 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4766404 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2385668 |  161 | `	return SXRET_OK;` |
|  1192837 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
