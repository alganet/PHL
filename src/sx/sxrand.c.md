...
|        - |  129 | ` */` |
| 29017248 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29017253 |  135 | `  pCtx->i++;` |
| 29017253 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29017253 |  137 | `  pCtx->j += t;` |
| 29017253 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29017253 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29017253 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29017253 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2904782 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2904787 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2904787 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2904787 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4354666 |  155 | `	for(;;){` |
|  8709337 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8704067 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8704067 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5804579 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2904787 |  161 | `	return SXRET_OK;` |
|  1452396 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
