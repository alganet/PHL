...
|    3134 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3136 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3136 |  106 | `	if( xSeed == 0 ){` |
|    3136 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1567 |  108 | `	}` |
|    3136 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3136 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3136 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  805440 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  802306 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  401154 |  116 | `    }` |
|  805440 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  802306 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  802306 |  119 | `      t = pCtx->s[pCtx->j];` |
|  802306 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  802306 |  121 | `      pCtx->s[i] = t;` |
|  401154 |  122 | `    }` |
|    3136 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3136 |  125 | `	return SXRET_OK;` |
|    1569 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2372072 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2372074 |  135 | `  pCtx->i++;` |
| 2372074 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2372074 |  137 | `  pCtx->j += t;` |
| 2372074 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2372074 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2372074 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2372074 |  141 | `  return pCtx->s[t];` |
|       2 |  142 |  |
|  238925 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  144 |  |
|  238927 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  238927 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  238927 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  356989 |  155 | `	for(;;){` |
|  713982 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  710971 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  710971 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  475081 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  160 | `	}` |
|  238927 |  161 | `	return SXRET_OK;` |
|  119464 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
