...
|      - |   32 | `#endif` |
|   2570 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   2572 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   2570 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   2570 |   55 | `	if (fd >= 0 ){` |
|   2570 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   2570 |   57 | `			close(fd);` |
|   2570 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|   1287 |   73 |  |
|   2570 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   2572 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   2572 |   88 | `	if( xSeed == 0 ){` |
|   2572 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|   1285 |   90 | `	}` |
|   2572 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   2572 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   2572 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 660492 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 657922 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 328962 |   98 | `    }` |
| 660492 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 657922 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 657922 |  101 | `      t = pCtx->s[pCtx->j];` |
| 657922 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 657922 |  103 | `      pCtx->s[i] = t;` |
| 328962 |  104 | `    }` |
|   2572 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   2572 |  107 | `	return SXRET_OK;` |
|   1287 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 760712 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 760714 |  117 | `  pCtx->i++;` |
| 760714 |  118 | `  t = pCtx->s[pCtx->i];` |
| 760714 |  119 | `  pCtx->j += t;` |
| 760714 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 760714 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 760714 |  122 | `  t += pCtx->s[pCtx->i];` |
| 760714 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  77483 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  77485 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  77485 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  77485 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
| 115079 |  137 | `	for(;;){` |
| 230166 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 227665 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 227665 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 152707 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  77485 |  143 | `	return SXRET_OK;` |
|  38742 |  144 |  |
|      - |  145 |  |
