...
|       - |   32 | `#endif` |
|    2602 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2604 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2602 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2602 |   55 | `	if (fd >= 0 ){` |
|    2602 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2602 |   57 | `			close(fd);` |
|    2602 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1303 |   73 |  |
|    2602 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2604 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2604 |   88 | `	if( xSeed == 0 ){` |
|    2604 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1301 |   90 | `	}` |
|    2604 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2604 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2604 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  668716 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  666114 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  333058 |   98 | `    }` |
|  668716 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  666114 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  666114 |  101 | `      t = pCtx->s[pCtx->j];` |
|  666114 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  666114 |  103 | `      pCtx->s[i] = t;` |
|  333058 |  104 | `    }` |
|    2604 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2604 |  107 | `	return SXRET_OK;` |
|    1303 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1264752 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1264754 |  117 | `  pCtx->i++;` |
| 1264754 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1264754 |  119 | `  pCtx->j += t;` |
| 1264754 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1264754 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1264754 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1264754 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  127905 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  127907 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  127907 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  127907 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  190701 |  137 | `	for(;;){` |
|  381402 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  378871 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  378871 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  253521 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  127907 |  143 | `	return SXRET_OK;` |
|   63955 |  144 |  |
|       - |  145 |  |
