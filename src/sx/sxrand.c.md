...
|        - |  129 | ` */` |
| 29031984 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29031989 |  135 | `  pCtx->i++;` |
| 29031989 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29031989 |  137 | `  pCtx->j += t;` |
| 29031989 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29031989 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29031989 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29031989 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2906251 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2906256 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2906256 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2906256 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4356867 |  155 | `	for(;;){` |
|  8713745 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708496 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708496 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5807518 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2906256 |  161 | `	return SXRET_OK;` |
|  1453129 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
