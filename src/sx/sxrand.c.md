...
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3150 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3150 |  106 | `	if( xSeed == 0 ){` |
|    3150 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1574 |  108 | `	}` |
|    3150 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3150 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3150 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  809038 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  805890 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  402946 |  116 | `    }` |
|  809038 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  805890 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  805890 |  119 | `      t = pCtx->s[pCtx->j];` |
|  805890 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  805890 |  121 | `      pCtx->s[i] = t;` |
|  402946 |  122 | `    }` |
|    3150 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3150 |  125 | `	return SXRET_OK;` |
|    1576 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2382892 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2382894 |  135 | `  pCtx->i++;` |
| 2382894 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2382894 |  137 | `  pCtx->j += t;` |
| 2382894 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2382894 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2382894 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2382894 |  141 | `  return pCtx->s[t];` |
|       2 |  142 |  |
|  240022 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  144 |  |
|  240024 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  240024 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  240024 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  358625 |  155 | `	for(;;){` |
|  717248 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  714212 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  714212 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  477250 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  160 | `	}` |
|  240024 |  161 | `	return SXRET_OK;` |
|  120014 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
