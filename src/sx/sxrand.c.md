...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3886 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3891 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3886 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3886 |   70 | `	if (fd >= 0 ){` |
|     3886 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3886 |   72 | `			close(fd);` |
|     3886 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3886 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3891 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3891 |  106 | `	if( xSeed == 0 ){` |
|     3891 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1943 |  108 | `	}` |
|     3891 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3891 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3891 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   998707 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   994821 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   497413 |  116 | `    }` |
|   998707 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   994821 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   994821 |  119 | `      t = pCtx->s[pCtx->j];` |
|   994821 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   994821 |  121 | `      pCtx->s[i] = t;` |
|   497413 |  122 | `    }` |
|     3891 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3891 |  125 | `	return SXRET_OK;` |
|     1948 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 29031948 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29031953 |  135 | `  pCtx->i++;` |
| 29031953 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29031953 |  137 | `  pCtx->j += t;` |
| 29031953 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29031953 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29031953 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29031953 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2906242 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2906247 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2906247 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2906247 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4356861 |  155 | `	for(;;){` |
|  8713727 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708487 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708487 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5807509 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2906247 |  161 | `	return SXRET_OK;` |
|  1453126 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      211 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      211 |  204 | `	return SXRET_OK;` |
