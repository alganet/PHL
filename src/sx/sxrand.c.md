...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3834 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3839 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3834 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3834 |   70 | `	if (fd >= 0 ){` |
|     3834 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3834 |   72 | `			close(fd);` |
|     3834 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3834 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3839 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3839 |  106 | `	if( xSeed == 0 ){` |
|     3839 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1917 |  108 | `	}` |
|     3839 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3839 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3839 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   985343 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   981509 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   490757 |  116 | `    }` |
|   985343 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   981509 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   981509 |  119 | `      t = pCtx->s[pCtx->j];` |
|   981509 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   981509 |  121 | `      pCtx->s[i] = t;` |
|   490757 |  122 | `    }` |
|     3839 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3839 |  125 | `	return SXRET_OK;` |
|     1922 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 25921116 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 25921121 |  135 | `  pCtx->i++;` |
| 25921121 |  136 | `  t = pCtx->s[pCtx->i];` |
| 25921121 |  137 | `  pCtx->j += t;` |
| 25921121 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 25921121 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 25921121 |  140 | `  t += pCtx->s[pCtx->i];` |
| 25921121 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2595139 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2595144 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2595144 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2595144 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3890222 |  155 | `	for(;;){` |
|  7780455 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7775240 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7775240 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5185340 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2595144 |  161 | `	return SXRET_OK;` |
|  1297573 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
