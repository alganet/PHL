...
|      - |   32 | `#endif` |
|   1674 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1676 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1674 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1674 |   55 | `	if (fd >= 0 ){` |
|   1674 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1674 |   57 | `			close(fd);` |
|   1674 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    839 |   73 |  |
|   1674 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1676 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1676 |   88 | `	if( xSeed == 0 ){` |
|   1676 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    837 |   90 | `	}` |
|   1676 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1676 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1676 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 430220 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 428546 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 214274 |   98 | `    }` |
| 430220 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 428546 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 428546 |  101 | `      t = pCtx->s[pCtx->j];` |
| 428546 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 428546 |  103 | `      pCtx->s[i] = t;` |
| 214274 |  104 | `    }` |
|   1676 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1676 |  107 | `	return SXRET_OK;` |
|    839 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 495080 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 495082 |  117 | `  pCtx->i++;` |
| 495082 |  118 | `  t = pCtx->s[pCtx->i];` |
| 495082 |  119 | `  pCtx->j += t;` |
| 495082 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 495082 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 495082 |  122 | `  t += pCtx->s[pCtx->i];` |
| 495082 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  50384 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  50386 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  50386 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  50386 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  74880 |  137 | `	for(;;){` |
| 149762 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 148154 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 148154 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  99402 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  50386 |  143 | `	return SXRET_OK;` |
|  25194 |  144 |  |
|      - |  145 |  |
