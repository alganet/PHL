...
|      - |   32 | `#endif` |
|   2072 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   2074 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   2072 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   2072 |   55 | `	if (fd >= 0 ){` |
|   2072 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   2072 |   57 | `			close(fd);` |
|   2072 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|   1038 |   73 |  |
|   2072 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   2074 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   2074 |   88 | `	if( xSeed == 0 ){` |
|   2074 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|   1036 |   90 | `	}` |
|   2074 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   2074 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   2074 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 532506 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 530434 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 265218 |   98 | `    }` |
| 532506 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 530434 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 530434 |  101 | `      t = pCtx->s[pCtx->j];` |
| 530434 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 530434 |  103 | `      pCtx->s[i] = t;` |
| 265218 |  104 | `    }` |
|   2074 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   2074 |  107 | `	return SXRET_OK;` |
|   1038 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 612224 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 612226 |  117 | `  pCtx->i++;` |
| 612226 |  118 | `  t = pCtx->s[pCtx->i];` |
| 612226 |  119 | `  pCtx->j += t;` |
| 612226 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 612226 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 612226 |  122 | `  t += pCtx->s[pCtx->i];` |
| 612226 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  62351 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  62353 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  62353 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  62353 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  92617 |  137 | `	for(;;){` |
| 185242 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 183213 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 183213 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 122915 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  62353 |  143 | `	return SXRET_OK;` |
|  31176 |  144 |  |
|      - |  145 |  |
