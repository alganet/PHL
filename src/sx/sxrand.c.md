...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3538 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3543 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3538 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3538 |   70 | `	if (fd >= 0 ){` |
|    3538 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3538 |   72 | `			close(fd);` |
|    3538 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3538 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3543 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3543 |  106 | `	if( xSeed == 0 ){` |
|    3543 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1769 |  108 | `	}` |
|    3543 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3543 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3543 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  909271 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  905733 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  452869 |  116 | `    }` |
|  909271 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  905733 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  905733 |  119 | `      t = pCtx->s[pCtx->j];` |
|  905733 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  905733 |  121 | `      pCtx->s[i] = t;` |
|  452869 |  122 | `    }` |
|    3543 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3543 |  125 | `	return SXRET_OK;` |
|    1774 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2537828 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2537833 |  135 | `  pCtx->i++;` |
| 2537833 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2537833 |  137 | `  pCtx->j += t;` |
| 2537833 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2537833 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2537833 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2537833 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  255727 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  255732 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  255732 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  255732 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  382002 |  155 | `	for(;;){` |
|  764015 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  760624 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  760624 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  508312 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  255732 |  161 | `	return SXRET_OK;` |
|  127867 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
