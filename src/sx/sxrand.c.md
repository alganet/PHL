...
|      - |   32 | `#endif` |
|   1858 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1860 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1858 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1858 |   55 | `	if (fd >= 0 ){` |
|   1858 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1858 |   57 | `			close(fd);` |
|   1858 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    931 |   73 |  |
|   1858 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1860 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1860 |   88 | `	if( xSeed == 0 ){` |
|   1860 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    929 |   90 | `	}` |
|   1860 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1860 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1860 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 477508 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 475650 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 237826 |   98 | `    }` |
| 477508 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 475650 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 475650 |  101 | `      t = pCtx->s[pCtx->j];` |
| 475650 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 475650 |  103 | `      pCtx->s[i] = t;` |
| 237826 |  104 | `    }` |
|   1860 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1860 |  107 | `	return SXRET_OK;` |
|    931 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 549300 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 549302 |  117 | `  pCtx->i++;` |
| 549302 |  118 | `  t = pCtx->s[pCtx->i];` |
| 549302 |  119 | `  pCtx->j += t;` |
| 549302 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 549302 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 549302 |  122 | `  t += pCtx->s[pCtx->i];` |
| 549302 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  55929 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  55931 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  55931 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  55931 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  83094 |  137 | `	for(;;){` |
| 166192 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 164379 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 164379 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 110287 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  55931 |  143 | `	return SXRET_OK;` |
|  27966 |  144 |  |
|      - |  145 |  |
