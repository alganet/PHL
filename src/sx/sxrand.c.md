...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3916 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3921 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3916 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3916 |   70 | `	if (fd >= 0 ){` |
|     3916 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3916 |   72 | `			close(fd);` |
|     3916 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3916 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3921 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3921 |  106 | `	if( xSeed == 0 ){` |
|     3921 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1958 |  108 | `	}` |
|     3921 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3921 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3921 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1006417 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  1002501 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   501253 |  116 | `    }` |
|  1006417 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  1002501 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  1002501 |  119 | `      t = pCtx->s[pCtx->j];` |
|  1002501 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  1002501 |  121 | `      pCtx->s[i] = t;` |
|   501253 |  122 | `    }` |
|     3921 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3921 |  125 | `	return SXRET_OK;` |
|     1963 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 24475768 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24475773 |  135 | `  pCtx->i++;` |
| 24475773 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24475773 |  137 | `  pCtx->j += t;` |
| 24475773 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24475773 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24475773 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24475773 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2450696 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2450701 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2450701 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2450701 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3673482 |  155 | `	for(;;){` |
|  7346965 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7341613 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7341613 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4896293 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2450701 |  161 | `	return SXRET_OK;` |
|  1225354 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      379 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      381 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      192 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      192 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      187 |  207 | `		sxu32 nDone = 0;` |
|      374 |  208 | `		while( nDone < nLen ){` |
|      187 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      187 |  210 | `			if( n > 0 ){` |
|      187 |  211 | `				nDone += (sxu32)n;` |
|      187 |  212 | `				continue;` |
|        - |  213 | `			}` |
