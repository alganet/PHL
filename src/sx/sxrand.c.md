...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3796 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3801 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3796 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3796 |   70 | `	if (fd >= 0 ){` |
|     3796 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3796 |   72 | `			close(fd);` |
|     3796 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3796 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3801 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3801 |  106 | `	if( xSeed == 0 ){` |
|     3801 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1898 |  108 | `	}` |
|     3801 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3801 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3801 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   975577 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   971781 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   485893 |  116 | `    }` |
|   975577 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   971781 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   971781 |  119 | `      t = pCtx->s[pCtx->j];` |
|   971781 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   971781 |  121 | `      pCtx->s[i] = t;` |
|   485893 |  122 | `    }` |
|     3801 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3801 |  125 | `	return SXRET_OK;` |
|     1903 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23726400 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23726405 |  135 | `  pCtx->i++;` |
| 23726405 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23726405 |  137 | `  pCtx->j += t;` |
| 23726405 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23726405 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23726405 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23726405 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2375650 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2375655 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2375655 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2375655 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3561002 |  155 | `	for(;;){` |
|  7122009 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7116839 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7116839 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4746383 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2375655 |  161 | `	return SXRET_OK;` |
|  1187830 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
