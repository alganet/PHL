...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3892 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3897 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3892 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3892 |   70 | `	if (fd >= 0 ){` |
|     3892 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3892 |   72 | `			close(fd);` |
|     3892 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3892 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3897 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3897 |  106 | `	if( xSeed == 0 ){` |
|     3897 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1946 |  108 | `	}` |
|     3897 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3897 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3897 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1000249 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   996357 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   498181 |  116 | `    }` |
|  1000249 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   996357 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   996357 |  119 | `      t = pCtx->s[pCtx->j];` |
|   996357 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   996357 |  121 | `      pCtx->s[i] = t;` |
|   498181 |  122 | `    }` |
|     3897 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3897 |  125 | `	return SXRET_OK;` |
|     1951 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 29076764 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29076769 |  135 | `  pCtx->i++;` |
| 29076769 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29076769 |  137 | `  pCtx->j += t;` |
| 29076769 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29076769 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29076769 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29076769 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2910732 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2910737 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2910737 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2910737 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4363589 |  155 | `	for(;;){` |
|  8727183 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8721929 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8721929 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5816475 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2910737 |  161 | `	return SXRET_OK;` |
|  1455371 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      365 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      367 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      184 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      184 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      181 |  207 | `		sxu32 nDone = 0;` |
|      362 |  208 | `		while( nDone < nLen ){` |
|      181 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      181 |  210 | `			if( n > 0 ){` |
|      181 |  211 | `				nDone += (sxu32)n;` |
|      181 |  212 | `				continue;` |
|        - |  213 | `			}` |
