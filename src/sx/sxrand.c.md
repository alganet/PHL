...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3576 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3581 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3576 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3576 |   70 | `	if (fd >= 0 ){` |
|    3576 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3576 |   72 | `			close(fd);` |
|    3576 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3576 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3581 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3581 |  106 | `	if( xSeed == 0 ){` |
|    3581 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1788 |  108 | `	}` |
|    3581 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3581 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3581 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  919037 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  915461 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  457733 |  116 | `    }` |
|  919037 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  915461 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  915461 |  119 | `      t = pCtx->s[pCtx->j];` |
|  915461 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  915461 |  121 | `      pCtx->s[i] = t;` |
|  457733 |  122 | `    }` |
|    3581 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3581 |  125 | `	return SXRET_OK;` |
|    1793 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2708236 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2708241 |  135 | `  pCtx->i++;` |
| 2708241 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2708241 |  137 | `  pCtx->j += t;` |
| 2708241 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2708241 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2708241 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2708241 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  272787 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  272792 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  272792 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  272792 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  407578 |  155 | `	for(;;){` |
|  815163 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  811740 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  811740 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  542400 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  272792 |  161 | `	return SXRET_OK;` |
|  136398 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
