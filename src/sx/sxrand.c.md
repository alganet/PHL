...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3568 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3573 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3568 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3568 |   70 | `	if (fd >= 0 ){` |
|    3568 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3568 |   72 | `			close(fd);` |
|    3568 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3568 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3573 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3573 |  106 | `	if( xSeed == 0 ){` |
|    3573 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1784 |  108 | `	}` |
|    3573 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3573 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3573 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  916981 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  913413 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  456709 |  116 | `    }` |
|  916981 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  913413 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  913413 |  119 | `      t = pCtx->s[pCtx->j];` |
|  913413 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  913413 |  121 | `      pCtx->s[i] = t;` |
|  456709 |  122 | `    }` |
|    3573 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3573 |  125 | `	return SXRET_OK;` |
|    1789 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2702204 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2702209 |  135 | `  pCtx->i++;` |
| 2702209 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2702209 |  137 | `  pCtx->j += t;` |
| 2702209 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2702209 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2702209 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2702209 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  272179 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  272184 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  272184 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  272184 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  406670 |  155 | `	for(;;){` |
|  813347 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  809932 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  809932 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  541192 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  272184 |  161 | `	return SXRET_OK;` |
|  136094 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
