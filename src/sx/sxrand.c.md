...
|        - |  129 | ` */` |
| 13816452 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13816457 |  135 | `  pCtx->i++;` |
| 13816457 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13816457 |  137 | `  pCtx->j += t;` |
| 13816457 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13816457 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13816457 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13816457 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1384742 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1384747 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1384747 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1384747 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2074569 |  155 | `	for(;;){` |
|  4149139 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4143827 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4143827 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2764421 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1384747 |  161 | `	return SXRET_OK;` |
|   692377 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  206 | `	{` |
|      210 |  207 | `		sxu32 nDone = 0;` |
|      420 |  208 | `		while( nDone < nLen ){` |
|      210 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      210 |  210 | `			if( n > 0 ){` |
|      210 |  211 | `				nDone += (sxu32)n;` |
|      210 |  212 | `				continue;` |
|        - |  213 | `			}` |
