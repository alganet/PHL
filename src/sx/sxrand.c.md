...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3826 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3831 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3826 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3826 |   70 | `	if (fd >= 0 ){` |
|     3826 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3826 |   72 | `			close(fd);` |
|     3826 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3826 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3831 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3831 |  106 | `	if( xSeed == 0 ){` |
|     3831 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1913 |  108 | `	}` |
|     3831 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3831 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3831 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   983287 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   979461 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   489733 |  116 | `    }` |
|   983287 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   979461 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   979461 |  119 | `      t = pCtx->s[pCtx->j];` |
|   979461 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   979461 |  121 | `      pCtx->s[i] = t;` |
|   489733 |  122 | `    }` |
|     3831 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3831 |  125 | `	return SXRET_OK;` |
|     1918 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 24030560 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24030565 |  135 | `  pCtx->i++;` |
| 24030565 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24030565 |  137 | `  pCtx->j += t;` |
| 24030565 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24030565 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24030565 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24030565 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2406079 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2406084 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2406084 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2406084 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3606639 |  155 | `	for(;;){` |
|  7213281 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7208076 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7208076 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4807226 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2406084 |  161 | `	return SXRET_OK;` |
|  1203045 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  206 | `	{` |
|      182 |  207 | `		sxu32 nDone = 0;` |
|      364 |  208 | `		while( nDone < nLen ){` |
|      182 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      182 |  210 | `			if( n > 0 ){` |
|      182 |  211 | `				nDone += (sxu32)n;` |
|      182 |  212 | `				continue;` |
|        - |  213 | `			}` |
