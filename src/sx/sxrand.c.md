...
|       - |  129 | ` */` |
| 2900172 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2900177 |  135 | `  pCtx->i++;` |
| 2900177 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2900177 |  137 | `  pCtx->j += t;` |
| 2900177 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2900177 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2900177 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2900177 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  292121 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  292126 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  292126 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  292126 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  436462 |  155 | `	for(;;){` |
|  872931 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  869274 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  869274 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  580834 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  292126 |  161 | `	return SXRET_OK;` |
|  146065 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
