...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3814 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3819 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3814 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3814 |   70 | `	if (fd >= 0 ){` |
|     3814 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3814 |   72 | `			close(fd);` |
|     3814 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3814 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3819 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3819 |  106 | `	if( xSeed == 0 ){` |
|     3819 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1907 |  108 | `	}` |
|     3819 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3819 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3819 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   980203 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   976389 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   488197 |  116 | `    }` |
|   980203 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   976389 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   976389 |  119 | `      t = pCtx->s[pCtx->j];` |
|   976389 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   976389 |  121 | `      pCtx->s[i] = t;` |
|   488197 |  122 | `    }` |
|     3819 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3819 |  125 | `	return SXRET_OK;` |
|     1912 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23953948 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23953953 |  135 | `  pCtx->i++;` |
| 23953953 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23953953 |  137 | `  pCtx->j += t;` |
| 23953953 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23953953 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23953953 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23953953 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2398413 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2398418 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2398418 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2398418 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3595146 |  155 | `	for(;;){` |
|  7190291 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7185094 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7185094 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4791902 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2398418 |  161 | `	return SXRET_OK;` |
|  1199213 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  206 | `	{` |
|      172 |  207 | `		sxu32 nDone = 0;` |
|      344 |  208 | `		while( nDone < nLen ){` |
|      172 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      172 |  210 | `			if( n > 0 ){` |
|      172 |  211 | `				nDone += (sxu32)n;` |
|      172 |  212 | `				continue;` |
|        - |  213 | `			}` |
