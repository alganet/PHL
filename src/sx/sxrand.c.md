...
|       - |   32 | `#endif` |
|    2592 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2594 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2592 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2592 |   55 | `	if (fd >= 0 ){` |
|    2592 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2592 |   57 | `			close(fd);` |
|    2592 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1298 |   73 |  |
|    2592 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2594 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2594 |   88 | `	if( xSeed == 0 ){` |
|    2594 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1296 |   90 | `	}` |
|    2594 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2594 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2594 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  666146 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  663554 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  331778 |   98 | `    }` |
|  666146 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  663554 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  663554 |  101 | `      t = pCtx->s[pCtx->j];` |
|  663554 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  663554 |  103 | `      pCtx->s[i] = t;` |
|  331778 |  104 | `    }` |
|    2594 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2594 |  107 | `	return SXRET_OK;` |
|    1298 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1259928 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1259930 |  117 | `  pCtx->i++;` |
| 1259930 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1259930 |  119 | `  pCtx->j += t;` |
| 1259930 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1259930 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1259930 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1259930 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  127419 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  127421 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  127421 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  127421 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  189975 |  137 | `	for(;;){` |
|  379950 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  377425 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  377425 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  252555 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  127421 |  143 | `	return SXRET_OK;` |
|   63712 |  144 |  |
|       - |  145 |  |
