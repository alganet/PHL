...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3802 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 | `{` |
|    3807 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3802 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3802 |   70 | `	if (fd >= 0 ){` |
|    3802 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3802 |   72 | `			close(fd);` |
|    3802 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3802 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 | `{` |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3807 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3807 |  106 | `	if( xSeed == 0 ){` |
|    3807 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1901 |  108 | `	}` |
|    3807 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3807 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3807 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  977119 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  973317 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  486661 |  116 | `    }` |
|  977119 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  973317 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  973317 |  119 | `      t = pCtx->s[pCtx->j];` |
|  973317 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  973317 |  121 | `      pCtx->s[i] = t;` |
|  486661 |  122 | `    }` |
|    3807 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3807 |  125 | `	return SXRET_OK;` |
|    1906 |  126 | `}` |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2878840 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2878845 |  135 | `  pCtx->i++;` |
| 2878845 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2878845 |  137 | `  pCtx->j += t;` |
| 2878845 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2878845 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2878845 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2878845 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  289971 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  289976 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  289976 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  289976 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  433249 |  155 | `	for(;;){` |
|  866509 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  862880 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  862880 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  576562 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  289976 |  161 | `	return SXRET_OK;` |
|  144989 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
