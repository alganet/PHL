...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3434 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3439 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3434 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3434 |   70 | `	if (fd >= 0 ){` |
|    3434 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3434 |   72 | `			close(fd);` |
|    3434 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3434 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3439 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3439 |  106 | `	if( xSeed == 0 ){` |
|    3439 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1717 |  108 | `	}` |
|    3439 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3439 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3439 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  882543 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  879109 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  439557 |  116 | `    }` |
|  882543 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  879109 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  879109 |  119 | `      t = pCtx->s[pCtx->j];` |
|  879109 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  879109 |  121 | `      pCtx->s[i] = t;` |
|  439557 |  122 | `    }` |
|    3439 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3439 |  125 | `	return SXRET_OK;` |
|    1722 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2599824 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2599829 |  135 | `  pCtx->i++;` |
| 2599829 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2599829 |  137 | `  pCtx->j += t;` |
| 2599829 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2599829 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2599829 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2599829 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  261866 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  261871 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  261871 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  261871 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  391264 |  155 | `	for(;;){` |
|  782533 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  779243 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  779243 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  520691 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  261871 |  161 | `	return SXRET_OK;` |
|  130938 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
