...
|      - |   32 | `#endif` |
|   1498 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1500 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1498 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1498 |   55 | `	if (fd >= 0 ){` |
|   1498 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1498 |   57 | `			close(fd);` |
|   1498 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    751 |   73 |  |
|   1498 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1500 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1500 |   88 | `	if( xSeed == 0 ){` |
|   1500 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    749 |   90 | `	}` |
|   1500 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1500 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1500 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 384988 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 383490 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 191746 |   98 | `    }` |
| 384988 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 383490 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 383490 |  101 | `      t = pCtx->s[pCtx->j];` |
| 383490 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 383490 |  103 | `      pCtx->s[i] = t;` |
| 191746 |  104 | `    }` |
|   1500 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1500 |  107 | `	return SXRET_OK;` |
|    751 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 443336 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 443338 |  117 | `  pCtx->i++;` |
| 443338 |  118 | `  t = pCtx->s[pCtx->i];` |
| 443338 |  119 | `  pCtx->j += t;` |
| 443338 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 443338 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 443338 |  122 | `  t += pCtx->s[pCtx->i];` |
| 443338 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  45104 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  45106 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  45106 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  45106 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  67048 |  137 | `	for(;;){` |
| 134098 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 132666 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 132666 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  89018 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  45106 |  143 | `	return SXRET_OK;` |
|  22554 |  144 |  |
|      - |  145 |  |
