...
|      - |   32 | `#endif` |
|   1204 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1206 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1204 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1204 |   55 | `	if (fd >= 0 ){` |
|   1204 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1204 |   57 | `			close(fd);` |
|   1204 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    604 |   73 |  |
|   1204 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1206 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1206 |   88 | `	if( xSeed == 0 ){` |
|   1206 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    602 |   90 | `	}` |
|   1206 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1206 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1206 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 309430 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 308226 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 154114 |   98 | `    }` |
| 309430 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 308226 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 308226 |  101 | `      t = pCtx->s[pCtx->j];` |
| 308226 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 308226 |  103 | `      pCtx->s[i] = t;` |
| 154114 |  104 | `    }` |
|   1206 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1206 |  107 | `	return SXRET_OK;` |
|    604 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 356908 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 356910 |  117 | `  pCtx->i++;` |
| 356910 |  118 | `  t = pCtx->s[pCtx->i];` |
| 356910 |  119 | `  pCtx->j += t;` |
| 356910 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 356910 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 356910 |  122 | `  t += pCtx->s[pCtx->i];` |
| 356910 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  36282 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  36284 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  36284 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  36284 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  53965 |  137 | `	for(;;){` |
| 107932 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 106796 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 106796 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  71674 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  36284 |  143 | `	return SXRET_OK;` |
|  18143 |  144 |  |
|      - |  145 |  |
