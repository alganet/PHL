...
|       - |   32 | `#endif` |
|    2614 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2616 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2614 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2614 |   55 | `	if (fd >= 0 ){` |
|    2614 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2614 |   57 | `			close(fd);` |
|    2614 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1309 |   73 |  |
|    2614 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2616 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2616 |   88 | `	if( xSeed == 0 ){` |
|    2616 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1307 |   90 | `	}` |
|    2616 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2616 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2616 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  671800 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  669186 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  334594 |   98 | `    }` |
|  671800 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  669186 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  669186 |  101 | `      t = pCtx->s[pCtx->j];` |
|  669186 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  669186 |  103 | `      pCtx->s[i] = t;` |
|  334594 |  104 | `    }` |
|    2616 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2616 |  107 | `	return SXRET_OK;` |
|    1309 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1270788 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1270790 |  117 | `  pCtx->i++;` |
| 1270790 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1270790 |  119 | `  pCtx->j += t;` |
| 1270790 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1270790 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1270790 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1270790 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  128517 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  128519 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  128519 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  128519 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  191614 |  137 | `	for(;;){` |
|  383224 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  380679 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  380679 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  254731 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  128519 |  143 | `	return SXRET_OK;` |
|   64262 |  144 |  |
|       - |  145 |  |
