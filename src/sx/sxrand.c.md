...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3800 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3805 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3800 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3800 |   70 | `	if (fd >= 0 ){` |
|     3800 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3800 |   72 | `			close(fd);` |
|     3800 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3800 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3805 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3805 |  106 | `	if( xSeed == 0 ){` |
|     3805 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1900 |  108 | `	}` |
|     3805 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3805 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3805 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   976605 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   972805 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   486405 |  116 | `    }` |
|   976605 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   972805 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   972805 |  119 | `      t = pCtx->s[pCtx->j];` |
|   972805 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   972805 |  121 | `      pCtx->s[i] = t;` |
|   486405 |  122 | `    }` |
|     3805 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3805 |  125 | `	return SXRET_OK;` |
|     1905 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23751512 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23751517 |  135 | `  pCtx->i++;` |
| 23751517 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23751517 |  137 | `  pCtx->j += t;` |
| 23751517 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23751517 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23751517 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23751517 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2378161 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2378166 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2378166 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2378166 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3564767 |  155 | `	for(;;){` |
|  7129545 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7124370 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7124370 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4751408 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2378166 |  161 | `	return SXRET_OK;` |
|  1189084 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      377 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      379 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      196 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      196 |  204 | `	return SXRET_OK;` |
