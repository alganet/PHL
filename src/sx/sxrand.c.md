...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3304 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3309 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3304 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3304 |   70 | `	if (fd >= 0 ){` |
|    3304 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3304 |   72 | `			close(fd);` |
|    3304 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3304 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3309 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3309 |  106 | `	if( xSeed == 0 ){` |
|    3309 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1652 |  108 | `	}` |
|    3309 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3309 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3309 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  849133 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  845829 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  422917 |  116 | `    }` |
|  849133 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  845829 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  845829 |  119 | `      t = pCtx->s[pCtx->j];` |
|  845829 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  845829 |  121 | `      pCtx->s[i] = t;` |
|  422917 |  122 | `    }` |
|    3309 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3309 |  125 | `	return SXRET_OK;` |
|    1657 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2500888 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2500893 |  135 | `  pCtx->i++;` |
| 2500893 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2500893 |  137 | `  pCtx->j += t;` |
| 2500893 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2500893 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2500893 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2500893 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  251896 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  251901 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  251901 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  251901 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  376372 |  155 | `	for(;;){` |
|  752749 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  749589 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  749589 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  500877 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  251901 |  161 | `	return SXRET_OK;` |
|  125953 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
