...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3818 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 | `{` |
|    3823 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3818 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3818 |   70 | `	if (fd >= 0 ){` |
|    3818 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3818 |   72 | `			close(fd);` |
|    3818 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3818 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 | `{` |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3823 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3823 |  106 | `	if( xSeed == 0 ){` |
|    3823 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1909 |  108 | `	}` |
|    3823 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3823 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3823 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  981231 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  977413 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  488709 |  116 | `    }` |
|  981231 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  977413 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  977413 |  119 | `      t = pCtx->s[pCtx->j];` |
|  977413 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  977413 |  121 | `      pCtx->s[i] = t;` |
|  488709 |  122 | `    }` |
|    3823 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3823 |  125 | `	return SXRET_OK;` |
|    1914 |  126 | `}` |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2890900 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2890905 |  135 | `  pCtx->i++;` |
| 2890905 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2890905 |  137 | `  pCtx->j += t;` |
| 2890905 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2890905 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2890905 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2890905 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  291186 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  291191 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  291191 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  291191 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  435069 |  155 | `	for(;;){` |
|  870139 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  866495 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  866495 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  578977 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  291191 |  161 | `	return SXRET_OK;` |
|  145599 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
