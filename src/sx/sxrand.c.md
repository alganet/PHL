...
|       - |   32 | `#endif` |
|    2776 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2778 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2776 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2776 |   55 | `	if (fd >= 0 ){` |
|    2776 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2776 |   57 | `			close(fd);` |
|    2776 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1390 |   73 |  |
|    2776 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2778 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2778 |   88 | `	if( xSeed == 0 ){` |
|    2778 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1388 |   90 | `	}` |
|    2778 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2778 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2778 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  713434 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  710658 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  355330 |   98 | `    }` |
|  713434 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  710658 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  710658 |  101 | `      t = pCtx->s[pCtx->j];` |
|  710658 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  710658 |  103 | `      pCtx->s[i] = t;` |
|  355330 |  104 | `    }` |
|    2778 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2778 |  107 | `	return SXRET_OK;` |
|    1390 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1348716 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1348718 |  117 | `  pCtx->i++;` |
| 1348718 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1348718 |  119 | `  pCtx->j += t;` |
| 1348718 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1348718 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1348718 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1348718 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  136407 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  136409 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  136409 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  136409 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  203364 |  137 | `	for(;;){` |
|  406732 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  404025 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  404025 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  270349 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  136409 |  143 | `	return SXRET_OK;` |
|   68205 |  144 |  |
|       - |  145 |  |
