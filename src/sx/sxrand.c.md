...
|        - |  129 | ` */` |
| 28942428 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28942433 |  135 | `  pCtx->i++;` |
| 28942433 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28942433 |  137 | `  pCtx->j += t;` |
| 28942433 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28942433 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28942433 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28942433 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2897290 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2897295 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2897295 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2897295 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4343435 |  155 | `	for(;;){` |
|  8686871 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681631 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681631 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5789605 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2897295 |  161 | `	return SXRET_OK;` |
|  1448651 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
