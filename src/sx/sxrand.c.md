...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3894 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3899 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3894 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3894 |   70 | `	if (fd >= 0 ){` |
|     3894 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3894 |   72 | `			close(fd);` |
|     3894 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3894 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3899 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3899 |  106 | `	if( xSeed == 0 ){` |
|     3899 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1947 |  108 | `	}` |
|     3899 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3899 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3899 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1000763 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   996869 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   498437 |  116 | `    }` |
|  1000763 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   996869 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   996869 |  119 | `      t = pCtx->s[pCtx->j];` |
|   996869 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   996869 |  121 | `      pCtx->s[i] = t;` |
|   498437 |  122 | `    }` |
|     3899 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3899 |  125 | `	return SXRET_OK;` |
|     1952 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 29091668 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29091673 |  135 | `  pCtx->i++;` |
| 29091673 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29091673 |  137 | `  pCtx->j += t;` |
| 29091673 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29091673 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29091673 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29091673 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2912220 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2912225 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2912225 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2912225 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4365823 |  155 | `	for(;;){` |
|  8731651 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8726401 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8726401 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5819455 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2912225 |  161 | `	return SXRET_OK;` |
|  1456115 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      378 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      380 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      187 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      187 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      191 |  207 | `		sxu32 nDone = 0;` |
|      382 |  208 | `		while( nDone < nLen ){` |
|      191 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      191 |  210 | `			if( n > 0 ){` |
|      191 |  211 | `				nDone += (sxu32)n;` |
|      191 |  212 | `				continue;` |
|        - |  213 | `			}` |
