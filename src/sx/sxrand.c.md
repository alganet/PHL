...
|      - |   32 | `#endif` |
|   2060 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   2062 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   2060 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   2060 |   55 | `	if (fd >= 0 ){` |
|   2060 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   2060 |   57 | `			close(fd);` |
|   2060 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|   1032 |   73 |  |
|   2060 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   2062 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   2062 |   88 | `	if( xSeed == 0 ){` |
|   2062 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|   1030 |   90 | `	}` |
|   2062 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   2062 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   2062 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 529422 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 527362 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 263682 |   98 | `    }` |
| 529422 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 527362 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 527362 |  101 | `      t = pCtx->s[pCtx->j];` |
| 527362 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 527362 |  103 | `      pCtx->s[i] = t;` |
| 263682 |  104 | `    }` |
|   2062 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   2062 |  107 | `	return SXRET_OK;` |
|   1032 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 608696 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 608698 |  117 | `  pCtx->i++;` |
| 608698 |  118 | `  t = pCtx->s[pCtx->i];` |
| 608698 |  119 | `  pCtx->j += t;` |
| 608698 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 608698 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 608698 |  122 | `  t += pCtx->s[pCtx->i];` |
| 608698 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  61991 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  61993 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  61993 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  61993 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  92083 |  137 | `	for(;;){` |
| 184174 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 182157 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 182157 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 122207 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  61993 |  143 | `	return SXRET_OK;` |
|  30996 |  144 |  |
|      - |  145 |  |
