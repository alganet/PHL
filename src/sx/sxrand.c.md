...
|        - |  129 | ` */` |
| 26492532 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26492537 |  135 | `  pCtx->i++;` |
| 26492537 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26492537 |  137 | `  pCtx->j += t;` |
| 26492537 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26492537 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26492537 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26492537 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2652312 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2652317 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2652317 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2652317 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3975961 |  155 | `	for(;;){` |
|  7951923 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946653 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946653 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5299635 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2652317 |  161 | `	return SXRET_OK;` |
|  1326162 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      179 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      179 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      203 |  207 | `		sxu32 nDone = 0;` |
|      406 |  208 | `		while( nDone < nLen ){` |
|      203 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      203 |  210 | `			if( n > 0 ){` |
|      203 |  211 | `				nDone += (sxu32)n;` |
|      203 |  212 | `				continue;` |
|        - |  213 | `			}` |
