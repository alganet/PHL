...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3860 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3865 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3860 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3860 |   70 | `	if (fd >= 0 ){` |
|     3860 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3860 |   72 | `			close(fd);` |
|     3860 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3860 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3865 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3865 |  106 | `	if( xSeed == 0 ){` |
|     3865 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1930 |  108 | `	}` |
|     3865 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3865 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3865 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   992025 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   988165 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   494085 |  116 | `    }` |
|   992025 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   988165 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   988165 |  119 | `      t = pCtx->s[pCtx->j];` |
|   988165 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   988165 |  121 | `      pCtx->s[i] = t;` |
|   494085 |  122 | `    }` |
|     3865 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3865 |  125 | `	return SXRET_OK;` |
|     1935 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 26096876 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26096881 |  135 | `  pCtx->i++;` |
| 26096881 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26096881 |  137 | `  pCtx->j += t;` |
| 26096881 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26096881 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26096881 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26096881 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2612730 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2612735 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2612735 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2612735 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3916599 |  155 | `	for(;;){` |
|  7833203 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7827963 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7827963 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5220497 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2612735 |  161 | `	return SXRET_OK;` |
|  1306370 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      364 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      366 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      170 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      170 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      194 |  207 | `		sxu32 nDone = 0;` |
|      388 |  208 | `		while( nDone < nLen ){` |
|      194 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      194 |  210 | `			if( n > 0 ){` |
|      194 |  211 | `				nDone += (sxu32)n;` |
|      194 |  212 | `				continue;` |
|        - |  213 | `			}` |
