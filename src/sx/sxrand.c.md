...
|       - |  129 | ` */` |
| 2890912 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2890917 |  135 | `  pCtx->i++;` |
| 2890917 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2890917 |  137 | `  pCtx->j += t;` |
| 2890917 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2890917 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2890917 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2890917 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  291189 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  291194 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  291194 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  291194 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  435067 |  155 | `	for(;;){` |
|  870145 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  866498 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  866498 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  578980 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  291194 |  161 | `	return SXRET_OK;` |
|  145598 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
