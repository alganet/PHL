...
|      - |   32 | `#endif` |
|   1578 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1580 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1578 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1578 |   55 | `	if (fd >= 0 ){` |
|   1578 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1578 |   57 | `			close(fd);` |
|   1578 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    791 |   73 |  |
|   1578 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1580 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1580 |   88 | `	if( xSeed == 0 ){` |
|   1580 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    789 |   90 | `	}` |
|   1580 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1580 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1580 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 405548 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 403970 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 201986 |   98 | `    }` |
| 405548 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 403970 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 403970 |  101 | `      t = pCtx->s[pCtx->j];` |
| 403970 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 403970 |  103 | `      pCtx->s[i] = t;` |
| 201986 |  104 | `    }` |
|   1580 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1580 |  107 | `	return SXRET_OK;` |
|    791 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 466872 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 466874 |  117 | `  pCtx->i++;` |
| 466874 |  118 | `  t = pCtx->s[pCtx->i];` |
| 466874 |  119 | `  pCtx->j += t;` |
| 466874 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 466874 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 466874 |  122 | `  t += pCtx->s[pCtx->i];` |
| 466874 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  47508 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  47510 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  47510 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  47510 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  70612 |  137 | `	for(;;){` |
| 141226 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 139710 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 139710 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  93742 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  47510 |  143 | `	return SXRET_OK;` |
|  23756 |  144 |  |
|      - |  145 |  |
