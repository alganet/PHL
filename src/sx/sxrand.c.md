...
|       - |  129 | ` */` |
| 2900132 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2900137 |  135 | `  pCtx->i++;` |
| 2900137 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2900137 |  137 | `  pCtx->j += t;` |
| 2900137 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2900137 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2900137 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2900137 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  292117 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  292122 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  292122 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  292122 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  436456 |  155 | `	for(;;){` |
|  872919 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  869262 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  869262 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  580826 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  292122 |  161 | `	return SXRET_OK;` |
|  146063 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
