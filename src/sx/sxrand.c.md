...
|        - |  129 | ` */` |
| 26492640 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26492645 |  135 | `  pCtx->i++;` |
| 26492645 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26492645 |  137 | `  pCtx->j += t;` |
| 26492645 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26492645 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26492645 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26492645 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2652320 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2652325 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2652325 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2652325 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3975976 |  155 | `	for(;;){` |
|  7951953 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946685 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946685 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5299657 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2652325 |  161 | `	return SXRET_OK;` |
|  1326166 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  206 | `	{` |
|      205 |  207 | `		sxu32 nDone = 0;` |
|      410 |  208 | `		while( nDone < nLen ){` |
|      205 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      205 |  210 | `			if( n > 0 ){` |
|      205 |  211 | `				nDone += (sxu32)n;` |
|      205 |  212 | `				continue;` |
|        - |  213 | `			}` |
