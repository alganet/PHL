...
|        - |  129 | ` */` |
| 13903460 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13903465 |  135 | `  pCtx->i++;` |
| 13903465 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13903465 |  137 | `  pCtx->j += t;` |
| 13903465 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13903465 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13903465 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13903465 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1393450 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1393455 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1393455 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1393455 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2087623 |  155 | `	for(;;){` |
|  4175251 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4169927 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4169927 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2781825 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1393455 |  161 | `	return SXRET_OK;` |
|   696730 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
