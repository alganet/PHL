...
|       - |   32 | `#endif` |
|    2622 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2624 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2622 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2622 |   55 | `	if (fd >= 0 ){` |
|    2622 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2622 |   57 | `			close(fd);` |
|    2622 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1313 |   73 |  |
|    2622 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2624 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2624 |   88 | `	if( xSeed == 0 ){` |
|    2624 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1311 |   90 | `	}` |
|    2624 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2624 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2624 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  673856 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  671234 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  335618 |   98 | `    }` |
|  673856 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  671234 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  671234 |  101 | `      t = pCtx->s[pCtx->j];` |
|  671234 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  671234 |  103 | `      pCtx->s[i] = t;` |
|  335618 |  104 | `    }` |
|    2624 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2624 |  107 | `	return SXRET_OK;` |
|    1313 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1274632 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1274634 |  117 | `  pCtx->i++;` |
| 1274634 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1274634 |  119 | `  pCtx->j += t;` |
| 1274634 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1274634 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1274634 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1274634 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  128902 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  128904 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  128904 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  128904 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  192188 |  137 | `	for(;;){` |
|  384378 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  381832 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  381832 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  255500 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  128904 |  143 | `	return SXRET_OK;` |
|   64453 |  144 |  |
|       - |  145 |  |
