...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3812 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3817 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3812 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3812 |   70 | `	if (fd >= 0 ){` |
|     3812 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3812 |   72 | `			close(fd);` |
|     3812 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3812 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3817 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3817 |  106 | `	if( xSeed == 0 ){` |
|     3817 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1906 |  108 | `	}` |
|     3817 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3817 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3817 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   979689 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   975877 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   487941 |  116 | `    }` |
|   979689 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   975877 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   975877 |  119 | `      t = pCtx->s[pCtx->j];` |
|   975877 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   975877 |  121 | `      pCtx->s[i] = t;` |
|   487941 |  122 | `    }` |
|     3817 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3817 |  125 | `	return SXRET_OK;` |
|     1911 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23826488 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23826493 |  135 | `  pCtx->i++;` |
| 23826493 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23826493 |  137 | `  pCtx->j += t;` |
| 23826493 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23826493 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23826493 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23826493 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2385666 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2385671 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2385671 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2385671 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3576018 |  155 | `	for(;;){` |
|  7152049 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146859 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7146859 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4766407 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2385671 |  161 | `	return SXRET_OK;` |
|  1192836 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      206 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      206 |  204 | `	return SXRET_OK;` |
