...
|        - |  129 | ` */` |
| 23966452 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23966457 |  135 | `  pCtx->i++;` |
| 23966457 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23966457 |  137 | `  pCtx->j += t;` |
| 23966457 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23966457 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23966457 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23966457 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2399661 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2399666 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2399666 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2399666 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3597016 |  155 | `	for(;;){` |
|  7194039 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188846 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188846 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4794402 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2399666 |  161 | `	return SXRET_OK;` |
|  1199835 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
