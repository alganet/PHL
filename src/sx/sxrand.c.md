...
|        - |  129 | ` */` |
| 13766312 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13766317 |  135 | `  pCtx->i++;` |
| 13766317 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13766317 |  137 | `  pCtx->j += t;` |
| 13766317 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13766317 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13766317 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13766317 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1379716 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1379721 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1379721 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1379721 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2067038 |  155 | `	for(;;){` |
|  4134081 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4128789 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4128789 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2754389 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1379721 |  161 | `	return SXRET_OK;` |
|   689863 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
