...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3306 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3311 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3306 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3306 |   70 | `	if (fd >= 0 ){` |
|    3306 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3306 |   72 | `			close(fd);` |
|    3306 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3306 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3311 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3311 |  106 | `	if( xSeed == 0 ){` |
|    3311 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1653 |  108 | `	}` |
|    3311 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3311 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3311 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  849647 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  846341 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  423173 |  116 | `    }` |
|  849647 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  846341 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  846341 |  119 | `      t = pCtx->s[pCtx->j];` |
|  846341 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  846341 |  121 | `      pCtx->s[i] = t;` |
|  423173 |  122 | `    }` |
|    3311 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3311 |  125 | `	return SXRET_OK;` |
|    1658 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2502596 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2502601 |  135 | `  pCtx->i++;` |
| 2502601 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2502601 |  137 | `  pCtx->j += t;` |
| 2502601 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2502601 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2502601 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2502601 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  252067 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  252072 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  252072 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  252072 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  376630 |  155 | `	for(;;){` |
|  753263 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  750100 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  750100 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  501220 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  252072 |  161 | `	return SXRET_OK;` |
|  126039 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
