...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3586 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 | `{` |
|    3591 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3586 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3586 |   70 | `	if (fd >= 0 ){` |
|    3586 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3586 |   72 | `			close(fd);` |
|    3586 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3586 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 | `{` |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3591 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3591 |  106 | `	if( xSeed == 0 ){` |
|    3591 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1793 |  108 | `	}` |
|    3591 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3591 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3591 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  921607 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  918021 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  459013 |  116 | `    }` |
|  921607 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  918021 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  918021 |  119 | `      t = pCtx->s[pCtx->j];` |
|  918021 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  918021 |  121 | `      pCtx->s[i] = t;` |
|  459013 |  122 | `    }` |
|    3591 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3591 |  125 | `	return SXRET_OK;` |
|    1798 |  126 | `}` |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2715756 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2715761 |  135 | `  pCtx->i++;` |
| 2715761 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2715761 |  137 | `  pCtx->j += t;` |
| 2715761 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2715761 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2715761 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2715761 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  273542 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  273547 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  273547 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  273547 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  408711 |  155 | `	for(;;){` |
|  817423 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  813995 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  813995 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  543905 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  273547 |  161 | `	return SXRET_OK;` |
|  136777 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
