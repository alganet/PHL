...
|        - |  129 | ` */` |
| 14021760 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 14021765 |  135 | `  pCtx->i++;` |
| 14021765 |  136 | `  t = pCtx->s[pCtx->i];` |
| 14021765 |  137 | `  pCtx->j += t;` |
| 14021765 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 14021765 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 14021765 |  140 | `  t += pCtx->s[pCtx->i];` |
| 14021765 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1405280 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1405285 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1405285 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1405285 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2105368 |  155 | `	for(;;){` |
|  4210741 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4205417 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4205417 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2805485 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1405285 |  161 | `	return SXRET_OK;` |
|   702645 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
