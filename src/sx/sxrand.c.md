...
|       - |   32 | `#endif` |
|    2736 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2738 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2736 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2736 |   55 | `	if (fd >= 0 ){` |
|    2736 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2736 |   57 | `			close(fd);` |
|    2736 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1370 |   73 |  |
|    2736 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2738 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2738 |   88 | `	if( xSeed == 0 ){` |
|    2738 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1368 |   90 | `	}` |
|    2738 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2738 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2738 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  703154 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  700418 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  350210 |   98 | `    }` |
|  703154 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  700418 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  700418 |  101 | `      t = pCtx->s[pCtx->j];` |
|  700418 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  700418 |  103 | `      pCtx->s[i] = t;` |
|  350210 |  104 | `    }` |
|    2738 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2738 |  107 | `	return SXRET_OK;` |
|    1370 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1329364 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1329366 |  117 | `  pCtx->i++;` |
| 1329366 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1329366 |  119 | `  pCtx->j += t;` |
| 1329366 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1329366 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1329366 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1329366 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  134449 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  134451 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  134451 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  134451 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  200446 |  137 | `	for(;;){` |
|  400896 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  398227 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  398227 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  266471 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  134451 |  143 | `	return SXRET_OK;` |
|   67226 |  144 |  |
|       - |  145 |  |
