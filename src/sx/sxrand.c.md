...
|        - |  129 | ` */` |
| 29017084 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29017089 |  135 | `  pCtx->i++;` |
| 29017089 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29017089 |  137 | `  pCtx->j += t;` |
| 29017089 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29017089 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29017089 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29017089 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2904752 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2904757 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2904757 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2904757 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4354629 |  155 | `	for(;;){` |
|  8709263 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8704029 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8704029 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5804535 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2904757 |  161 | `	return SXRET_OK;` |
|  1452381 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      172 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      172 |  204 | `	return SXRET_OK;` |
