...
|      - |   32 | `#endif` |
|   1762 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1764 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1762 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1762 |   55 | `	if (fd >= 0 ){` |
|   1762 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1762 |   57 | `			close(fd);` |
|   1762 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    883 |   73 |  |
|   1762 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1764 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1764 |   88 | `	if( xSeed == 0 ){` |
|   1764 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    881 |   90 | `	}` |
|   1764 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1764 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1764 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 452836 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 451074 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 225538 |   98 | `    }` |
| 452836 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 451074 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 451074 |  101 | `      t = pCtx->s[pCtx->j];` |
| 451074 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 451074 |  103 | `      pCtx->s[i] = t;` |
| 225538 |  104 | `    }` |
|   1764 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1764 |  107 | `	return SXRET_OK;` |
|    883 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 520948 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 520950 |  117 | `  pCtx->i++;` |
| 520950 |  118 | `  t = pCtx->s[pCtx->i];` |
| 520950 |  119 | `  pCtx->j += t;` |
| 520950 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 520950 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 520950 |  122 | `  t += pCtx->s[pCtx->i];` |
| 520950 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  53023 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  53025 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  53025 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  53025 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  78796 |  137 | `	for(;;){` |
| 157592 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 155897 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 155897 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 104593 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  53025 |  143 | `	return SXRET_OK;` |
|  26514 |  144 |  |
|      - |  145 |  |
