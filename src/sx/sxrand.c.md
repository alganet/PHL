...
|        - |  129 | ` */` |
| 28942416 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28942421 |  135 | `  pCtx->i++;` |
| 28942421 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28942421 |  137 | `  pCtx->j += t;` |
| 28942421 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28942421 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28942421 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28942421 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2897287 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2897292 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2897292 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2897292 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4343431 |  155 | `	for(;;){` |
|  8686865 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681628 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681628 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5789602 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2897292 |  161 | `	return SXRET_OK;` |
|  1448649 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
