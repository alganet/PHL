...
|        - |  129 | ` */` |
| 29061840 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29061845 |  135 | `  pCtx->i++;` |
| 29061845 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29061845 |  137 | `  pCtx->j += t;` |
| 29061845 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29061845 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29061845 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29061845 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2909239 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2909244 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2909244 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2909244 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4361349 |  155 | `	for(;;){` |
|  8722705 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8717452 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8717452 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5813490 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2909244 |  161 | `	return SXRET_OK;` |
|  1454624 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
