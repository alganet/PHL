...
|      - |   32 | `#endif` |
|   1320 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1322 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1320 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1320 |   55 | `	if (fd >= 0 ){` |
|   1320 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1320 |   57 | `			close(fd);` |
|   1320 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    662 |   73 |  |
|   1320 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1322 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1322 |   88 | `	if( xSeed == 0 ){` |
|   1322 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    660 |   90 | `	}` |
|   1322 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1322 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1322 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 339242 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 337922 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 168962 |   98 | `    }` |
| 339242 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 337922 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 337922 |  101 | `      t = pCtx->s[pCtx->j];` |
| 337922 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 337922 |  103 | `      pCtx->s[i] = t;` |
| 168962 |  104 | `    }` |
|   1322 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1322 |  107 | `	return SXRET_OK;` |
|    662 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 391004 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 391006 |  117 | `  pCtx->i++;` |
| 391006 |  118 | `  t = pCtx->s[pCtx->i];` |
| 391006 |  119 | `  pCtx->j += t;` |
| 391006 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 391006 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 391006 |  122 | `  t += pCtx->s[pCtx->i];` |
| 391006 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  39764 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  39766 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  39766 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  39766 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  59129 |  137 | `	for(;;){` |
| 118256 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 117002 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 117002 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  78516 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  39766 |  143 | `	return SXRET_OK;` |
|  19885 |  144 |  |
|      - |  145 |  |
