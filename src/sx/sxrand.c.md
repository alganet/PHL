...
|        - |  129 | ` */` |
| 28942412 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28942417 |  135 | `  pCtx->i++;` |
| 28942417 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28942417 |  137 | `  pCtx->j += t;` |
| 28942417 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28942417 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28942417 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28942417 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2897286 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2897291 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2897291 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2897291 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4343427 |  155 | `	for(;;){` |
|  8686863 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681627 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8681627 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5789601 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2897291 |  161 | `	return SXRET_OK;` |
|  1448647 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      344 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      346 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      173 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      173 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      171 |  207 | `		sxu32 nDone = 0;` |
|      342 |  208 | `		while( nDone < nLen ){` |
|      171 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      171 |  210 | `			if( n > 0 ){` |
|      171 |  211 | `				nDone += (sxu32)n;` |
|      171 |  212 | `				continue;` |
|        - |  213 | `			}` |
