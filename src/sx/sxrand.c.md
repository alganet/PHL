...
|      - |   32 | `#endif` |
|   1682 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1684 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1682 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1682 |   55 | `	if (fd >= 0 ){` |
|   1682 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1682 |   57 | `			close(fd);` |
|   1682 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    843 |   73 |  |
|   1682 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1684 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1684 |   88 | `	if( xSeed == 0 ){` |
|   1684 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    841 |   90 | `	}` |
|   1684 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1684 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1684 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 432276 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 430594 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 215298 |   98 | `    }` |
| 432276 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 430594 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 430594 |  101 | `      t = pCtx->s[pCtx->j];` |
| 430594 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 430594 |  103 | `      pCtx->s[i] = t;` |
| 215298 |  104 | `    }` |
|   1684 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1684 |  107 | `	return SXRET_OK;` |
|    843 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 497440 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 497442 |  117 | `  pCtx->i++;` |
| 497442 |  118 | `  t = pCtx->s[pCtx->i];` |
| 497442 |  119 | `  pCtx->j += t;` |
| 497442 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 497442 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 497442 |  122 | `  t += pCtx->s[pCtx->i];` |
| 497442 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  50626 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  50628 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  50628 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  50628 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  75238 |  137 | `	for(;;){` |
| 150478 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 148860 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 148860 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  99876 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  50628 |  143 | `	return SXRET_OK;` |
|  25315 |  144 |  |
|      - |  145 |  |
