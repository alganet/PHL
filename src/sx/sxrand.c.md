...
|       - |  129 | ` */` |
| 2715768 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2715773 |  135 | `  pCtx->i++;` |
| 2715773 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2715773 |  137 | `  pCtx->j += t;` |
| 2715773 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2715773 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2715773 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2715773 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  273545 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  273550 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  273550 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  273550 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  408709 |  155 | `	for(;;){` |
|  817429 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  813998 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  813998 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  543908 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  273550 |  161 | `	return SXRET_OK;` |
|  136776 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
