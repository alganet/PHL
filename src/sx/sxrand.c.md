...
|        - |  129 | ` */` |
| 28957348 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28957353 |  135 | `  pCtx->i++;` |
| 28957353 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28957353 |  137 | `  pCtx->j += t;` |
| 28957353 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28957353 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28957353 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28957353 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2898782 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2898787 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2898787 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2898787 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4345673 |  155 | `	for(;;){` |
|  8691347 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8686107 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8686107 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5792589 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2898787 |  161 | `	return SXRET_OK;` |
|  1449397 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      389 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      391 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      201 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      201 |  204 | `	return SXRET_OK;` |
