...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3164 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 |  |
|    3169 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3164 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3164 |   70 | `	if (fd >= 0 ){` |
|    3164 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3164 |   72 | `			close(fd);` |
|    3164 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3164 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3169 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3169 |  106 | `	if( xSeed == 0 ){` |
|    3169 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1582 |  108 | `	}` |
|    3169 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3169 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3169 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  813153 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  809989 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  404997 |  116 | `    }` |
|  813153 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  809989 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  809989 |  119 | `      t = pCtx->s[pCtx->j];` |
|  809989 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  809989 |  121 | `      pCtx->s[i] = t;` |
|  404997 |  122 | `    }` |
|    3169 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3169 |  125 | `	return SXRET_OK;` |
|    1587 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2395008 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2395013 |  135 | `  pCtx->i++;` |
| 2395013 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2395013 |  137 | `  pCtx->j += t;` |
| 2395013 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2395013 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2395013 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2395013 |  141 | `  return pCtx->s[t];` |
|       5 |  142 |  |
|  241239 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 |  |
|  241244 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  241244 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  241244 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  360445 |  155 | `	for(;;){` |
|  720893 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  717848 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  717848 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  479678 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  241244 |  161 | `	return SXRET_OK;` |
|  120625 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
...
|       1 |  190 |  |
|     351 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
