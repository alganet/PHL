...
|      - |   32 | `#endif` |
|   1292 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1294 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1292 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1292 |   55 | `	if (fd >= 0 ){` |
|   1292 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1292 |   57 | `			close(fd);` |
|   1292 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    648 |   73 |  |
|   1292 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1294 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1294 |   88 | `	if( xSeed == 0 ){` |
|   1294 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    646 |   90 | `	}` |
|   1294 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1294 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1294 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 332046 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 330754 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 165378 |   98 | `    }` |
| 332046 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 330754 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 330754 |  101 | `      t = pCtx->s[pCtx->j];` |
| 330754 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 330754 |  103 | `      pCtx->s[i] = t;` |
| 165378 |  104 | `    }` |
|   1294 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1294 |  107 | `	return SXRET_OK;` |
|    648 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 382784 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 382786 |  117 | `  pCtx->i++;` |
| 382786 |  118 | `  t = pCtx->s[pCtx->i];` |
| 382786 |  119 | `  pCtx->j += t;` |
| 382786 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 382786 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 382786 |  122 | `  t += pCtx->s[pCtx->i];` |
| 382786 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  38927 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  38929 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  38929 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  38929 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  57881 |  137 | `	for(;;){` |
| 115770 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 114541 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 114541 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  76867 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  38929 |  143 | `	return SXRET_OK;` |
|  19464 |  144 |  |
|      - |  145 |  |
