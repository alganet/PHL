...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3820 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3825 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3820 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3820 |   70 | `	if (fd >= 0 ){` |
|     3820 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3820 |   72 | `			close(fd);` |
|     3820 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3820 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3825 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3825 |  106 | `	if( xSeed == 0 ){` |
|     3825 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1910 |  108 | `	}` |
|     3825 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3825 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3825 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   981745 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   977925 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   488965 |  116 | `    }` |
|   981745 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   977925 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   977925 |  119 | `      t = pCtx->s[pCtx->j];` |
|   977925 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   977925 |  121 | `      pCtx->s[i] = t;` |
|   488965 |  122 | `    }` |
|     3825 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3825 |  125 | `	return SXRET_OK;` |
|     1915 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23991728 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23991733 |  135 | `  pCtx->i++;` |
| 23991733 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23991733 |  137 | `  pCtx->j += t;` |
| 23991733 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23991733 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23991733 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23991733 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2402191 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2402196 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2402196 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2402196 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3600809 |  155 | `	for(;;){` |
|  7201625 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7196428 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7196428 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4799458 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2402196 |  161 | `	return SXRET_OK;` |
|  1201100 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
