...
|        - |  129 | ` */` |
| 26492644 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26492649 |  135 | `  pCtx->i++;` |
| 26492649 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26492649 |  137 | `  pCtx->j += t;` |
| 26492649 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26492649 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26492649 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26492649 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2652321 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2652326 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2652326 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2652326 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3975978 |  155 | `	for(;;){` |
|  7951955 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946686 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7946686 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5299658 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2652326 |  161 | `	return SXRET_OK;` |
|  1326167 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
