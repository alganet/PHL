...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3140 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   49 |  |
|    3142 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3140 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3140 |   70 | `	if (fd >= 0 ){` |
|    3140 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3140 |   72 | `			close(fd);` |
|    3140 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3140 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   93 |  |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3142 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3142 |  106 | `	if( xSeed == 0 ){` |
|    3142 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1570 |  108 | `	}` |
|    3142 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3142 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3142 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  806982 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  803842 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  401922 |  116 | `    }` |
|  806982 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  803842 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  803842 |  119 | `      t = pCtx->s[pCtx->j];` |
|  803842 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  803842 |  121 | `      pCtx->s[i] = t;` |
|  401922 |  122 | `    }` |
|    3142 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3142 |  125 | `	return SXRET_OK;` |
|    1572 |  126 |  |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2376620 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  131 |  |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2376622 |  135 | `  pCtx->i++;` |
| 2376622 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2376622 |  137 | `  pCtx->j += t;` |
| 2376622 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2376622 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2376622 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2376622 |  141 | `  return pCtx->s[t];` |
|       2 |  142 |  |
|  239387 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  144 |  |
|  239389 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  239389 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  239389 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  357678 |  155 | `	for(;;){` |
|  715356 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  712333 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  712333 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  475993 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  160 | `	}` |
|  239389 |  161 | `	return SXRET_OK;` |
|  119696 |  162 |  |
|       - |  163 | `/* SPDX-SnippetEnd */` |
...
|       - |  187 |  |
|       - |  188 | `#endif` |
|     352 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|       1 |  190 |  |
|     353 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|     161 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|     161 |  204 | `	return SXRET_OK;` |
