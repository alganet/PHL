...
|      - |   32 | `#endif` |
|   1606 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1608 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1606 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1606 |   55 | `	if (fd >= 0 ){` |
|   1606 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1606 |   57 | `			close(fd);` |
|   1606 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    805 |   73 |  |
|   1606 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1608 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1608 |   88 | `	if( xSeed == 0 ){` |
|   1608 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    803 |   90 | `	}` |
|   1608 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1608 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1608 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 412744 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 411138 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 205570 |   98 | `    }` |
| 412744 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 411138 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 411138 |  101 | `      t = pCtx->s[pCtx->j];` |
| 411138 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 411138 |  103 | `      pCtx->s[i] = t;` |
| 205570 |  104 | `    }` |
|   1608 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1608 |  107 | `	return SXRET_OK;` |
|    805 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 475096 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 475098 |  117 | `  pCtx->i++;` |
| 475098 |  118 | `  t = pCtx->s[pCtx->i];` |
| 475098 |  119 | `  pCtx->j += t;` |
| 475098 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 475098 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 475098 |  122 | `  t += pCtx->s[pCtx->i];` |
| 475098 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  48346 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  48348 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  48348 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  48348 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  71858 |  137 | `	for(;;){` |
| 143714 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 142172 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 142172 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  95392 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  48348 |  143 | `	return SXRET_OK;` |
|  24176 |  144 |  |
|      - |  145 |  |
