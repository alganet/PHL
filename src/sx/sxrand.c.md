...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3816 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3821 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3816 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3816 |   70 | `	if (fd >= 0 ){` |
|     3816 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3816 |   72 | `			close(fd);` |
|     3816 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3816 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3821 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3821 |  106 | `	if( xSeed == 0 ){` |
|     3821 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1908 |  108 | `	}` |
|     3821 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3821 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3821 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   980717 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   976901 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   488453 |  116 | `    }` |
|   980717 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   976901 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   976901 |  119 | `      t = pCtx->s[pCtx->j];` |
|   976901 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   976901 |  121 | `      pCtx->s[i] = t;` |
|   488453 |  122 | `    }` |
|     3821 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3821 |  125 | `	return SXRET_OK;` |
|     1913 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 23965972 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23965977 |  135 | `  pCtx->i++;` |
| 23965977 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23965977 |  137 | `  pCtx->j += t;` |
| 23965977 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23965977 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23965977 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23965977 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2399614 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2399619 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2399619 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2399619 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3596943 |  155 | `	for(;;){` |
|  7193895 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188703 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188703 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4794305 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2399619 |  161 | `	return SXRET_OK;` |
|  1199811 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      384 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      386 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
