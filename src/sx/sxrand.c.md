...
|       - |   32 | `#endif` |
|    2796 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2798 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2796 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2796 |   55 | `	if (fd >= 0 ){` |
|    2796 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2796 |   57 | `			close(fd);` |
|    2796 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1400 |   73 |  |
|    2796 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2798 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2798 |   88 | `	if( xSeed == 0 ){` |
|    2798 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1398 |   90 | `	}` |
|    2798 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2798 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2798 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  718574 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  715778 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  357890 |   98 | `    }` |
|  718574 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  715778 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  715778 |  101 | `      t = pCtx->s[pCtx->j];` |
|  715778 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  715778 |  103 | `      pCtx->s[i] = t;` |
|  357890 |  104 | `    }` |
|    2798 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2798 |  107 | `	return SXRET_OK;` |
|    1400 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1358528 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1358530 |  117 | `  pCtx->i++;` |
| 1358530 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1358530 |  119 | `  pCtx->j += t;` |
| 1358530 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1358530 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1358530 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1358530 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  137402 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  137404 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  137404 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  137404 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  204848 |  137 | `	for(;;){` |
|  409694 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  406964 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  406964 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  272316 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  137404 |  143 | `	return SXRET_OK;` |
|   68704 |  144 |  |
|       - |  145 |  |
