...
|      - |   32 | `#endif` |
|   2304 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   2306 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   2304 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   2304 |   55 | `	if (fd >= 0 ){` |
|   2304 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   2304 |   57 | `			close(fd);` |
|   2304 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|   1154 |   73 |  |
|   2304 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   2306 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   2306 |   88 | `	if( xSeed == 0 ){` |
|   2306 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|   1152 |   90 | `	}` |
|   2306 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   2306 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   2306 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 592130 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 589826 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 294914 |   98 | `    }` |
| 592130 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 589826 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 589826 |  101 | `      t = pCtx->s[pCtx->j];` |
| 589826 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 589826 |  103 | `      pCtx->s[i] = t;` |
| 294914 |  104 | `    }` |
|   2306 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   2306 |  107 | `	return SXRET_OK;` |
|   1154 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 680708 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 680710 |  117 | `  pCtx->i++;` |
| 680710 |  118 | `  t = pCtx->s[pCtx->i];` |
| 680710 |  119 | `  pCtx->j += t;` |
| 680710 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 680710 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 680710 |  122 | `  t += pCtx->s[pCtx->i];` |
| 680710 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  69341 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  69343 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  69343 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  69343 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
| 102988 |  137 | `	for(;;){` |
| 205976 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 203711 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 203711 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 136659 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  69343 |  143 | `	return SXRET_OK;` |
|  34673 |  144 |  |
|      - |  145 |  |
