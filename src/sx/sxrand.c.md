...
|        - |  129 | ` */` |
| 28987192 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28987197 |  135 | `  pCtx->i++;` |
| 28987197 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28987197 |  137 | `  pCtx->j += t;` |
| 28987197 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28987197 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28987197 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28987197 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2901767 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2901772 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2901772 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2901772 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4350147 |  155 | `	for(;;){` |
|  8700301 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8695060 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8695060 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5798558 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2901772 |  161 | `	return SXRET_OK;` |
|  1450888 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
