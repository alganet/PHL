...
|        - |  129 | ` */` |
| 13845128 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13845133 |  135 | `  pCtx->i++;` |
| 13845133 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13845133 |  137 | `  pCtx->j += t;` |
| 13845133 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13845133 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13845133 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13845133 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1387615 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1387620 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1387620 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1387620 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2078871 |  155 | `	for(;;){` |
|  4157749 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4152428 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4152428 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2770158 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1387620 |  161 | `	return SXRET_OK;` |
|   693812 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        1 |  190 | `{` |
|      361 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
