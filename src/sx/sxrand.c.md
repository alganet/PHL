...
|       - |   46 | `#endif` |
|       - |   47 | `#endif` |
|    3844 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       5 |   49 | `{` |
|    3849 |   50 | `	char *zBuf = (char *)pBuf;` |
|       - |   51 | `#ifdef __WINNT__` |
...
|    3844 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    3844 |   70 | `	if (fd >= 0 ){` |
|    3844 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    3844 |   72 | `			close(fd);` |
|    3844 |   73 | `			return SXRET_OK;` |
|       - |   74 | `		}` |
|       - |   75 | `		/* FALL THRU */` |
|     ! 0 |   76 | `	}` |
|     ! 0 |   77 | `	close(fd);` |
...
|    3844 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       5 |   93 | `{` |
|       - |   94 | `	char zSeed[256];` |
|       - |   95 | `	sxu8 t;` |
|       - |   96 | `	sxi32 rc;` |
|       - |   97 | `	sxu32 i;` |
|    3849 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |  105 | `  */` |
|    3849 |  106 | `	if( xSeed == 0 ){` |
|    3849 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|    1922 |  108 | `	}` |
|    3849 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    3849 |  110 | `	if( rc != SXRET_OK ){` |
|     ! 0 |  111 | `		return rc;` |
|       - |  112 | `	}` |
|    3849 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  987913 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  984069 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|  492037 |  116 | `    }` |
|  987913 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  984069 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  984069 |  119 | `      t = pCtx->s[pCtx->j];` |
|  984069 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  984069 |  121 | `      pCtx->s[i] = t;` |
|  492037 |  122 | `    }` |
|    3849 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  124 |  |
|    3849 |  125 | `	return SXRET_OK;` |
|    1927 |  126 | `}` |
|       - |  127 | `/*` |
|       - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  129 | ` */` |
| 2917416 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       5 |  131 | `{` |
|       - |  132 | `  sxu8 t;` |
|       - |  133 |  |
|       - |  134 | `  /* Generate and return single random byte */` |
| 2917421 |  135 | `  pCtx->i++;` |
| 2917421 |  136 | `  t = pCtx->s[pCtx->i];` |
| 2917421 |  137 | `  pCtx->j += t;` |
| 2917421 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 2917421 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 2917421 |  140 | `  t += pCtx->s[pCtx->i];` |
| 2917421 |  141 | `  return pCtx->s[t];` |
|       5 |  142 | `}` |
|  294827 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       5 |  144 | `{` |
|  294832 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  294832 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  147 | `#if defined(UNTRUST)` |
|       - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  149 | `		return SXERR_EMPTY;` |
|       - |  150 | `	}` |
|       - |  151 | `#endif` |
|  294832 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  153 | `		return SXERR_CORRUPT;` |
|       - |  154 | `	}` |
|  439705 |  155 | `	for(;;){` |
|  879413 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  874120 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  874120 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  584610 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       5 |  160 | `	}` |
|  294832 |  161 | `	return SXRET_OK;` |
|  147419 |  162 | `}` |
|       - |  163 | `/* SPDX-SnippetEnd */` |
