...
|      - |   32 | `#endif` |
|   1538 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1540 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1538 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1538 |   55 | `	if (fd >= 0 ){` |
|   1538 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1538 |   57 | `			close(fd);` |
|   1538 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    771 |   73 |  |
|   1538 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1540 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1540 |   88 | `	if( xSeed == 0 ){` |
|   1540 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    769 |   90 | `	}` |
|   1540 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1540 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1540 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 395268 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 393730 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 196866 |   98 | `    }` |
| 395268 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 393730 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 393730 |  101 | `      t = pCtx->s[pCtx->j];` |
| 393730 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 393730 |  103 | `      pCtx->s[i] = t;` |
| 196866 |  104 | `    }` |
|   1540 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1540 |  107 | `	return SXRET_OK;` |
|    771 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 455108 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 455110 |  117 | `  pCtx->i++;` |
| 455110 |  118 | `  t = pCtx->s[pCtx->i];` |
| 455110 |  119 | `  pCtx->j += t;` |
| 455110 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 455110 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 455110 |  122 | `  t += pCtx->s[pCtx->i];` |
| 455110 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  46307 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  46309 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  46309 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  46309 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  68834 |  137 | `	for(;;){` |
| 137664 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 136189 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 136189 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  91381 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  46309 |  143 | `	return SXRET_OK;` |
|  23157 |  144 |  |
|      - |  145 |  |
