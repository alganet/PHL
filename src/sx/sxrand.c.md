...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3824 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3829 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3824 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3824 |   70 | `	if (fd >= 0 ){` |
|     3824 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3824 |   72 | `			close(fd);` |
|     3824 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3824 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3829 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3829 |  106 | `	if( xSeed == 0 ){` |
|     3829 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1912 |  108 | `	}` |
|     3829 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3829 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3829 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   982773 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   978949 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   489477 |  116 | `    }` |
|   982773 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   978949 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   978949 |  119 | `      t = pCtx->s[pCtx->j];` |
|   978949 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   978949 |  121 | `      pCtx->s[i] = t;` |
|   489477 |  122 | `    }` |
|     3829 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3829 |  125 | `	return SXRET_OK;` |
|     1917 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 24017568 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 24017573 |  135 | `  pCtx->i++;` |
| 24017573 |  136 | `  t = pCtx->s[pCtx->i];` |
| 24017573 |  137 | `  pCtx->j += t;` |
| 24017573 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 24017573 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 24017573 |  140 | `  t += pCtx->s[pCtx->i];` |
| 24017573 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2404778 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2404783 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2404783 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2404783 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3604690 |  155 | `	for(;;){` |
|  7209381 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7204179 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7204179 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4804627 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2404783 |  161 | `	return SXRET_OK;` |
|  1202395 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      394 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      396 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      186 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      186 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      208 |  207 | `		sxu32 nDone = 0;` |
|      416 |  208 | `		while( nDone < nLen ){` |
|      208 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      208 |  210 | `			if( n > 0 ){` |
|      208 |  211 | `				nDone += (sxu32)n;` |
|      208 |  212 | `				continue;` |
|        - |  213 | `			}` |
