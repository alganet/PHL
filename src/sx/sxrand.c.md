...
|        - |  129 | ` */` |
| 23965964 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23965969 |  135 | `  pCtx->i++;` |
| 23965969 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23965969 |  137 | `  pCtx->j += t;` |
| 23965969 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23965969 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23965969 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23965969 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2399616 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2399621 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2399621 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2399621 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3596945 |  155 | `	for(;;){` |
|  7193895 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188701 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188701 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4794303 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2399621 |  161 | `	return SXRET_OK;` |
|  1199813 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
