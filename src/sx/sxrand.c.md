...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3846 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3851 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3846 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3846 |   70 | `	if (fd >= 0 ){` |
|     3846 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3846 |   72 | `			close(fd);` |
|     3846 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3846 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3851 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3851 |  106 | `	if( xSeed == 0 ){` |
|     3851 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1923 |  108 | `	}` |
|     3851 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3851 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3851 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   988427 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   984581 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   492293 |  116 | `    }` |
|   988427 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   984581 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   984581 |  119 | `      t = pCtx->s[pCtx->j];` |
|   984581 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   984581 |  121 | `      pCtx->s[i] = t;` |
|   492293 |  122 | `    }` |
|     3851 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3851 |  125 | `	return SXRET_OK;` |
|     1928 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 13765644 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13765649 |  135 | `  pCtx->i++;` |
| 13765649 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13765649 |  137 | `  pCtx->j += t;` |
| 13765649 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13765649 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13765649 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13765649 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1379651 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1379656 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1379656 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1379656 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2066938 |  155 | `	for(;;){` |
|  4133883 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4128588 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4128588 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2754256 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1379656 |  161 | `	return SXRET_OK;` |
|   689830 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
