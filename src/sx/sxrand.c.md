...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3898 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3903 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3898 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3898 |   70 | `	if (fd >= 0 ){` |
|     3898 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3898 |   72 | `			close(fd);` |
|     3898 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3898 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3903 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3903 |  106 | `	if( xSeed == 0 ){` |
|     3903 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1949 |  108 | `	}` |
|     3903 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3903 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3903 |  113 | `	pCtx->i = pCtx->j = 0;` |
|  1001791 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   997893 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   498949 |  116 | `    }` |
|  1001791 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   997893 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   997893 |  119 | `      t = pCtx->s[pCtx->j];` |
|   997893 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   997893 |  121 | `      pCtx->s[i] = t;` |
|   498949 |  122 | `    }` |
|     3903 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3903 |  125 | `	return SXRET_OK;` |
|     1954 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 29121568 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29121573 |  135 | `  pCtx->i++;` |
| 29121573 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29121573 |  137 | `  pCtx->j += t;` |
| 29121573 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29121573 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29121573 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29121573 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2915207 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2915212 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2915212 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2915212 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4370307 |  155 | `	for(;;){` |
|  8740617 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8735372 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8735372 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5825434 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2915212 |  161 | `	return SXRET_OK;` |
|  1457609 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      382 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      384 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
...
|      190 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      190 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      192 |  207 | `		sxu32 nDone = 0;` |
|      384 |  208 | `		while( nDone < nLen ){` |
|      192 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      192 |  210 | `			if( n > 0 ){` |
|      192 |  211 | `				nDone += (sxu32)n;` |
|      192 |  212 | `				continue;` |
|        - |  213 | `			}` |
