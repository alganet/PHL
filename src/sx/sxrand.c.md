...
|        - |  129 | ` */` |
| 25867060 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 25867065 |  135 | `  pCtx->i++;` |
| 25867065 |  136 | `  t = pCtx->s[pCtx->i];` |
| 25867065 |  137 | `  pCtx->j += t;` |
| 25867065 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 25867065 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 25867065 |  140 | `  t += pCtx->s[pCtx->i];` |
| 25867065 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2589729 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2589734 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2589734 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2589734 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3882112 |  155 | `	for(;;){` |
|  7764231 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7759026 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7759026 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5174526 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2589734 |  161 | `	return SXRET_OK;` |
|  1294869 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
