...
|        - |  129 | ` */` |
| 29031940 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 29031945 |  135 | `  pCtx->i++;` |
| 29031945 |  136 | `  t = pCtx->s[pCtx->i];` |
| 29031945 |  137 | `  pCtx->j += t;` |
| 29031945 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 29031945 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 29031945 |  140 | `  t += pCtx->s[pCtx->i];` |
| 29031945 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2906240 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2906245 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2906245 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2906245 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4356859 |  155 | `	for(;;){` |
|  8713723 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708485 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8708485 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5807507 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2906245 |  161 | `	return SXRET_OK;` |
|  1453125 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      205 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      205 |  204 | `	return SXRET_OK;` |
