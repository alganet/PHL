...
|      - |   32 | `#endif` |
|   1240 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1242 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1240 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1240 |   55 | `	if (fd >= 0 ){` |
|   1240 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1240 |   57 | `			close(fd);` |
|   1240 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    622 |   73 |  |
|   1240 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1242 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1242 |   88 | `	if( xSeed == 0 ){` |
|   1242 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    620 |   90 | `	}` |
|   1242 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1242 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1242 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 318682 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 317442 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 158722 |   98 | `    }` |
| 318682 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 317442 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 317442 |  101 | `      t = pCtx->s[pCtx->j];` |
| 317442 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 317442 |  103 | `      pCtx->s[i] = t;` |
| 158722 |  104 | `    }` |
|   1242 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1242 |  107 | `	return SXRET_OK;` |
|    622 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 367488 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 367490 |  117 | `  pCtx->i++;` |
| 367490 |  118 | `  t = pCtx->s[pCtx->i];` |
| 367490 |  119 | `  pCtx->j += t;` |
| 367490 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 367490 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 367490 |  122 | `  t += pCtx->s[pCtx->i];` |
| 367490 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  37365 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  37367 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  37367 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  37367 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  55567 |  137 | `	for(;;){` |
| 111138 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 109963 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 109963 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  73797 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  37367 |  143 | `	return SXRET_OK;` |
|  18684 |  144 |  |
|      - |  145 |  |
