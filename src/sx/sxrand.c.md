...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3850 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3855 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3850 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3850 |   70 | `	if (fd >= 0 ){` |
|     3850 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3850 |   72 | `			close(fd);` |
|     3850 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3850 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3855 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3855 |  106 | `	if( xSeed == 0 ){` |
|     3855 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1925 |  108 | `	}` |
|     3855 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3855 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3855 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   989455 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   985605 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   492805 |  116 | `    }` |
|   989455 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   985605 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   985605 |  119 | `      t = pCtx->s[pCtx->j];` |
|   985605 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   985605 |  121 | `      pCtx->s[i] = t;` |
|   492805 |  122 | `    }` |
|     3855 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3855 |  125 | `	return SXRET_OK;` |
|     1930 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 26029176 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26029181 |  135 | `  pCtx->i++;` |
| 26029181 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26029181 |  137 | `  pCtx->j += t;` |
| 26029181 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26029181 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26029181 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26029181 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2605954 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2605959 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2605959 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2605959 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3906440 |  155 | `	for(;;){` |
|  7812885 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7807655 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7807655 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5206955 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2605959 |  161 | `	return SXRET_OK;` |
|  1302982 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      396 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
|      398 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
