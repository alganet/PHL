...
|        - |  129 | ` */` |
| 26519792 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26519797 |  135 | `  pCtx->i++;` |
| 26519797 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26519797 |  137 | `  pCtx->j += t;` |
| 26519797 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26519797 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26519797 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26519797 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2655041 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2655046 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2655046 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2655046 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3980047 |  155 | `	for(;;){` |
|  7960105 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7954830 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7954830 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5305088 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2655046 |  161 | `	return SXRET_OK;` |
|  1327524 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
