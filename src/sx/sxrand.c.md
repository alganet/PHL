...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3854 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3859 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3854 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3854 |   70 | `	if (fd >= 0 ){` |
|     3854 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3854 |   72 | `			close(fd);` |
|     3854 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3854 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3859 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3859 |  106 | `	if( xSeed == 0 ){` |
|     3859 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1927 |  108 | `	}` |
|     3859 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3859 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3859 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   990483 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   986629 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   493317 |  116 | `    }` |
|   990483 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   986629 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   986629 |  119 | `      t = pCtx->s[pCtx->j];` |
|   986629 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   986629 |  121 | `      pCtx->s[i] = t;` |
|   493317 |  122 | `    }` |
|     3859 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3859 |  125 | `	return SXRET_OK;` |
|     1932 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 26056248 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26056253 |  135 | `  pCtx->i++;` |
| 26056253 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26056253 |  137 | `  pCtx->j += t;` |
| 26056253 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26056253 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26056253 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26056253 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2608663 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2608668 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2608668 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2608668 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3910501 |  155 | `	for(;;){` |
|  7821009 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7815776 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7815776 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5212370 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2608668 |  161 | `	return SXRET_OK;` |
|  1304336 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
