...
|        - |  129 | ` */` |
| 23966056 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23966061 |  135 | `  pCtx->i++;` |
| 23966061 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23966061 |  137 | `  pCtx->j += t;` |
| 23966061 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23966061 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23966061 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23966061 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2399623 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2399628 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2399628 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2399628 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3596957 |  155 | `	for(;;){` |
|  7193921 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188728 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188728 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4794322 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2399628 |  161 | `	return SXRET_OK;` |
|  1199816 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|      171 |  203 | `	arc4random_buf(zBuf,(size_t)nLen);` |
|      171 |  204 | `	return SXRET_OK;` |
|        - |  205 | `#elif defined(SX_HAVE_GETRANDOM)` |
|        - |  206 | `	{` |
|      197 |  207 | `		sxu32 nDone = 0;` |
|      394 |  208 | `		while( nDone < nLen ){` |
|      197 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      197 |  210 | `			if( n > 0 ){` |
|      197 |  211 | `				nDone += (sxu32)n;` |
|      197 |  212 | `				continue;` |
|        - |  213 | `			}` |
