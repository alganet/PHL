...
|       - |   32 | `#endif` |
|    2694 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|       2 |   34 |  |
|    2696 |   35 | `	char *zBuf = (char *)pBuf;` |
|       - |   36 | `#ifdef __WINNT__` |
...
|    2694 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|    2694 |   55 | `	if (fd >= 0 ){` |
|    2694 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|    2694 |   57 | `			close(fd);` |
|    2694 |   58 | `			return SXRET_OK;` |
|       - |   59 | `		}` |
|       - |   60 | `		/* FALL THRU */` |
|     ! 0 |   61 | `	}` |
|     ! 0 |   62 | `	close(fd);` |
...
|    1349 |   73 |  |
|    2694 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|       2 |   75 |  |
|       - |   76 | `	char zSeed[256];` |
|       - |   77 | `	sxu8 t;` |
|       - |   78 | `	sxi32 rc;` |
|       - |   79 | `	sxu32 i;` |
|    2696 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|       - |   87 | `  */` |
|    2696 |   88 | `	if( xSeed == 0 ){` |
|    2696 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    1347 |   90 | `	}` |
|    2696 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|    2696 |   92 | `	if( rc != SXRET_OK ){` |
|     ! 0 |   93 | `		return rc;` |
|       - |   94 | `	}` |
|    2696 |   95 | `	pCtx->i = pCtx->j = 0;` |
|  692360 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|  689666 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
|  344834 |   98 | `    }` |
|  692360 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|  689666 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|  689666 |  101 | `      t = pCtx->s[pCtx->j];` |
|  689666 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|  689666 |  103 | `      pCtx->s[i] = t;` |
|  344834 |  104 | `    }` |
|    2696 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|       - |  106 |  |
|    2696 |  107 | `	return SXRET_OK;` |
|    1349 |  108 |  |
|       - |  109 | `/*` |
|       - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|       - |  111 | ` */` |
| 1066564 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|       2 |  113 |  |
|       - |  114 | `  sxu8 t;` |
|       - |  115 |  |
|       - |  116 | `  /* Generate and return single random byte */` |
| 1066566 |  117 | `  pCtx->i++;` |
| 1066566 |  118 | `  t = pCtx->s[pCtx->i];` |
| 1066566 |  119 | `  pCtx->j += t;` |
| 1066566 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 1066566 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 1066566 |  122 | `  t += pCtx->s[pCtx->i];` |
| 1066566 |  123 | `  return pCtx->s[t];` |
|       2 |  124 |  |
|  108142 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|       2 |  126 |  |
|  108144 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  108144 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|       - |  129 | `#if defined(UNTRUST)` |
|       - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|       - |  131 | `		return SXERR_EMPTY;` |
|       - |  132 | `	}` |
|       - |  133 | `#endif` |
|  108144 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|     ! 0 |  135 | `		return SXERR_CORRUPT;` |
|       - |  136 | `	}` |
|  161011 |  137 | `	for(;;){` |
|  322020 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  319396 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  319396 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  213902 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|       2 |  142 | `	}` |
|  108144 |  143 | `	return SXRET_OK;` |
|   54074 |  144 |  |
|       - |  145 |  |
