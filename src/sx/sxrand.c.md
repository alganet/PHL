...
|      - |   32 | `#endif` |
|   1562 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1564 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1562 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1562 |   55 | `	if (fd >= 0 ){` |
|   1562 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1562 |   57 | `			close(fd);` |
|   1562 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    783 |   73 |  |
|   1562 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1564 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1564 |   88 | `	if( xSeed == 0 ){` |
|   1564 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    781 |   90 | `	}` |
|   1564 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1564 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1564 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 401436 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 399874 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 199938 |   98 | `    }` |
| 401436 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 399874 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 399874 |  101 | `      t = pCtx->s[pCtx->j];` |
| 399874 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 399874 |  103 | `      pCtx->s[i] = t;` |
| 199938 |  104 | `    }` |
|   1564 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1564 |  107 | `	return SXRET_OK;` |
|    783 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 462156 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 462158 |  117 | `  pCtx->i++;` |
| 462158 |  118 | `  t = pCtx->s[pCtx->i];` |
| 462158 |  119 | `  pCtx->j += t;` |
| 462158 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 462158 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 462158 |  122 | `  t += pCtx->s[pCtx->i];` |
| 462158 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  47025 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  47027 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  47027 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  47027 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  69896 |  137 | `	for(;;){` |
| 139796 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 138299 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 138299 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  92795 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  47027 |  143 | `	return SXRET_OK;` |
|  23514 |  144 |  |
|      - |  145 |  |
