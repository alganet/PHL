...
|        - |   46 | `#endif` |
|        - |   47 | `#endif` |
|     3880 |   48 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|        5 |   49 | `{` |
|     3885 |   50 | `	char *zBuf = (char *)pBuf;` |
|        - |   51 | `#ifdef __WINNT__` |
...
|     3880 |   69 | `	fd = open("/dev/urandom",O_RDONLY);` |
|     3880 |   70 | `	if (fd >= 0 ){` |
|     3880 |   71 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|     3880 |   72 | `			close(fd);` |
|     3880 |   73 | `			return SXRET_OK;` |
|        - |   74 | `		}` |
|        - |   75 | `		/* FALL THRU */` |
|      ! 0 |   76 | `	}` |
|      ! 0 |   77 | `	close(fd);` |
...
|     3880 |   92 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|        5 |   93 | `{` |
|        - |   94 | `	char zSeed[256];` |
|        - |   95 | `	sxu8 t;` |
|        - |   96 | `	sxi32 rc;` |
|        - |   97 | `	sxu32 i;` |
|     3885 |   98 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|        - |  105 | `  */` |
|     3885 |  106 | `	if( xSeed == 0 ){` |
|     3885 |  107 | `		xSeed = SyOSUtilRandomSeed;` |
|     1940 |  108 | `	}` |
|     3885 |  109 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|     3885 |  110 | `	if( rc != SXRET_OK ){` |
|      ! 0 |  111 | `		return rc;` |
|        - |  112 | `	}` |
|     3885 |  113 | `	pCtx->i = pCtx->j = 0;` |
|   997165 |  114 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
|   993285 |  115 | `		pCtx->s[i] = (unsigned char)i;` |
|   496645 |  116 | `    }` |
|   997165 |  117 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
|   993285 |  118 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
|   993285 |  119 | `      t = pCtx->s[pCtx->j];` |
|   993285 |  120 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
|   993285 |  121 | `      pCtx->s[i] = t;` |
|   496645 |  122 | `    }` |
|     3885 |  123 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|        - |  124 |  |
|     3885 |  125 | `	return SXRET_OK;` |
|     1945 |  126 | `}` |
|        - |  127 | `/*` |
|        - |  128 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|        - |  129 | ` */` |
| 28987200 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 28987205 |  135 | `  pCtx->i++;` |
| 28987205 |  136 | `  t = pCtx->s[pCtx->i];` |
| 28987205 |  137 | `  pCtx->j += t;` |
| 28987205 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 28987205 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 28987205 |  140 | `  t += pCtx->s[pCtx->i];` |
| 28987205 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2901769 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2901774 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2901774 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2901774 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  4350147 |  155 | `	for(;;){` |
|  8700305 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8695062 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  8695062 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5798560 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2901774 |  161 | `	return SXRET_OK;` |
|  1450888 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
