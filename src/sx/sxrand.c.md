...
|        - |  129 | ` */` |
| 13903988 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13903993 |  135 | `  pCtx->i++;` |
| 13903993 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13903993 |  137 | `  pCtx->j += t;` |
| 13903993 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13903993 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13903993 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13903993 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1393504 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1393509 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1393509 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1393509 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2087703 |  155 | `	for(;;){` |
|  4175411 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4170085 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4170085 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2781931 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1393509 |  161 | `	return SXRET_OK;` |
|   696757 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
