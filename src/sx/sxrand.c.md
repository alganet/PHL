...
|        - |  129 | ` */` |
| 23726384 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23726389 |  135 | `  pCtx->i++;` |
| 23726389 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23726389 |  137 | `  pCtx->j += t;` |
| 23726389 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23726389 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23726389 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23726389 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2375646 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2375651 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2375651 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2375651 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3561000 |  155 | `	for(;;){` |
|  7122001 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7116835 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7116835 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4746379 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2375651 |  161 | `	return SXRET_OK;` |
|  1187829 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
