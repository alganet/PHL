...
|      - |   32 | `#endif` |
|   1662 |   33 | `static sxi32 SyOSUtilRandomSeed(void *pBuf,sxu32 nLen,void *pUnused)` |
|      2 |   34 |  |
|   1664 |   35 | `	char *zBuf = (char *)pBuf;` |
|      - |   36 | `#ifdef __WINNT__` |
...
|   1662 |   54 | `	fd = open("/dev/urandom",O_RDONLY);` |
|   1662 |   55 | `	if (fd >= 0 ){` |
|   1662 |   56 | `		if( read(fd,zBuf,nLen) > 0 ){` |
|   1662 |   57 | `			close(fd);` |
|   1662 |   58 | `			return SXRET_OK;` |
|      - |   59 | `		}` |
|      - |   60 | `		/* FALL THRU */` |
|    ! 0 |   61 | `	}` |
|    ! 0 |   62 | `	close(fd);` |
...
|    833 |   73 |  |
|   1662 |   74 | `PH7_PRIVATE sxi32 SyRandomnessInit(SyPRNGCtx *pCtx,ProcRandomSeed xSeed,void * pUserData)` |
|      2 |   75 |  |
|      - |   76 | `	char zSeed[256];` |
|      - |   77 | `	sxu8 t;` |
|      - |   78 | `	sxi32 rc;` |
|      - |   79 | `	sxu32 i;` |
|   1664 |   80 | `	if( pCtx->nMagic == SXPRNG_MAGIC ){` |
...
|      - |   87 | `  */` |
|   1664 |   88 | `	if( xSeed == 0 ){` |
|   1664 |   89 | `		xSeed = SyOSUtilRandomSeed;` |
|    831 |   90 | `	}` |
|   1664 |   91 | `	rc = xSeed(zSeed,sizeof(zSeed),pUserData);` |
|   1664 |   92 | `	if( rc != SXRET_OK ){` |
|    ! 0 |   93 | `		return rc;` |
|      - |   94 | `	}` |
|   1664 |   95 | `	pCtx->i = pCtx->j = 0;` |
| 427136 |   96 | `	for(i=0; i < SX_ARRAYSIZE(pCtx->s) ; i++){` |
| 425474 |   97 | `		pCtx->s[i] = (unsigned char)i;` |
| 212738 |   98 | `    }` |
| 427136 |   99 | `    for(i=0; i < sizeof(zSeed) ; i++){` |
| 425474 |  100 | `      pCtx->j += pCtx->s[i] + zSeed[i];` |
| 425474 |  101 | `      t = pCtx->s[pCtx->j];` |
| 425474 |  102 | `      pCtx->s[pCtx->j] = pCtx->s[i];` |
| 425474 |  103 | `      pCtx->s[i] = t;` |
| 212738 |  104 | `    }` |
|   1664 |  105 | `	pCtx->nMagic = SXPRNG_MAGIC;` |
|      - |  106 |  |
|   1664 |  107 | `	return SXRET_OK;` |
|    833 |  108 |  |
|      - |  109 | `/*` |
|      - |  110 | ` * Get a single 8-bit random value using the RC4 PRNG.` |
|      - |  111 | ` */` |
| 491564 |  112 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|      2 |  113 |  |
|      - |  114 | `  sxu8 t;` |
|      - |  115 |  |
|      - |  116 | `  /* Generate and return single random byte */` |
| 491566 |  117 | `  pCtx->i++;` |
| 491566 |  118 | `  t = pCtx->s[pCtx->i];` |
| 491566 |  119 | `  pCtx->j += t;` |
| 491566 |  120 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 491566 |  121 | `  pCtx->s[pCtx->j] = t;` |
| 491566 |  122 | `  t += pCtx->s[pCtx->i];` |
| 491566 |  123 | `  return pCtx->s[t];` |
|      2 |  124 |  |
|  50027 |  125 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|      2 |  126 |  |
|  50029 |  127 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  50029 |  128 | `	unsigned char *zEnd = &zBuf[nLen];` |
|      - |  129 | `#if defined(UNTRUST)` |
|      - |  130 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|      - |  131 | `		return SXERR_EMPTY;` |
|      - |  132 | `	}` |
|      - |  133 | `#endif` |
|  50029 |  134 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|    ! 0 |  135 | `		return SXERR_CORRUPT;` |
|      - |  136 | `	}` |
|  74348 |  137 | `	for(;;){` |
| 148700 |  138 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 147101 |  139 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
| 147101 |  140 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  98697 |  141 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|      2 |  142 | `	}` |
|  50029 |  143 | `	return SXRET_OK;` |
|  25015 |  144 |  |
|      - |  145 |  |
