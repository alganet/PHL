...
|        - |  129 | ` */` |
| 26097000 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 26097005 |  135 | `  pCtx->i++;` |
| 26097005 |  136 | `  t = pCtx->s[pCtx->i];` |
| 26097005 |  137 | `  pCtx->j += t;` |
| 26097005 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 26097005 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 26097005 |  140 | `  t += pCtx->s[pCtx->i];` |
| 26097005 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2612743 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2612748 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2612748 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2612748 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3916617 |  155 | `	for(;;){` |
|  7833241 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7828000 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7828000 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  5220522 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2612748 |  161 | `	return SXRET_OK;` |
|  1306376 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      398 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        2 |  190 | `{` |
...
|        - |  206 | `	{` |
|      207 |  207 | `		sxu32 nDone = 0;` |
|      414 |  208 | `		while( nDone < nLen ){` |
|      207 |  209 | `			ssize_t n = getrandom(&zBuf[nDone],nLen - nDone,0);` |
|      207 |  210 | `			if( n > 0 ){` |
|      207 |  211 | `				nDone += (sxu32)n;` |
|      207 |  212 | `				continue;` |
|        - |  213 | `			}` |
