...
|        - |  129 | ` */` |
| 23966240 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 23966245 |  135 | `  pCtx->i++;` |
| 23966245 |  136 | `  t = pCtx->s[pCtx->i];` |
| 23966245 |  137 | `  pCtx->j += t;` |
| 23966245 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 23966245 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 23966245 |  140 | `  t += pCtx->s[pCtx->i];` |
| 23966245 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  2399642 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  2399647 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  2399647 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  2399647 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  3596988 |  155 | `	for(;;){` |
|  7193977 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188783 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  7188783 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4794359 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  2399647 |  161 | `	return SXRET_OK;` |
|  1199827 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
