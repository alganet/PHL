...
|        - |  129 | ` */` |
| 13978620 |  130 | `static sxu8 randomByte(SyPRNGCtx *pCtx)` |
|        5 |  131 | `{` |
|        - |  132 | `  sxu8 t;` |
|        - |  133 |  |
|        - |  134 | `  /* Generate and return single random byte */` |
| 13978625 |  135 | `  pCtx->i++;` |
| 13978625 |  136 | `  t = pCtx->s[pCtx->i];` |
| 13978625 |  137 | `  pCtx->j += t;` |
| 13978625 |  138 | `  pCtx->s[pCtx->i] = pCtx->s[pCtx->j];` |
| 13978625 |  139 | `  pCtx->s[pCtx->j] = t;` |
| 13978625 |  140 | `  t += pCtx->s[pCtx->i];` |
| 13978625 |  141 | `  return pCtx->s[t];` |
|        5 |  142 | `}` |
|  1400957 |  143 | `PH7_PRIVATE sxi32 SyRandomness(SyPRNGCtx *pCtx,void *pBuf,sxu32 nLen)` |
|        5 |  144 | `{` |
|  1400962 |  145 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
|  1400962 |  146 | `	unsigned char *zEnd = &zBuf[nLen];` |
|        - |  147 | `#if defined(UNTRUST)` |
|        - |  148 | `	if( pCtx == 0 \|\| pBuf == 0 \|\| nLen <= 0 ){` |
|        - |  149 | `		return SXERR_EMPTY;` |
|        - |  150 | `	}` |
|        - |  151 | `#endif` |
|  1400962 |  152 | `	if(pCtx->nMagic != SXPRNG_MAGIC ){` |
|      ! 0 |  153 | `		return SXERR_CORRUPT;` |
|        - |  154 | `	}` |
|  2098892 |  155 | `	for(;;){` |
|  4197787 |  156 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4192478 |  157 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  4192478 |  158 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|  2796854 |  159 | `		if( zBuf >= zEnd ){break;}	zBuf[0] = randomByte(pCtx);	zBuf++;` |
|        5 |  160 | `	}` |
|  1400962 |  161 | `	return SXRET_OK;` |
|   700484 |  162 | `}` |
|        - |  163 | `/* SPDX-SnippetEnd */` |
...
|        - |  187 | `}` |
|        - |  188 | `#endif` |
|      349 |  189 | `PH7_PRIVATE sxi32 SyOSCSPRNG(void *pBuf,sxu32 nLen)` |
|        1 |  190 | `{` |
|      350 |  191 | `	unsigned char *zBuf = (unsigned char *)pBuf;` |
