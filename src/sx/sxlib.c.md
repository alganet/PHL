...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  49703828 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  49703833 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  49703833 |   21 | `	sxu32 nH = 5381;` |
|  49703833 |   22 | `	zEnd = &zIn[nLen];` |
|  89605611 |   23 | `	for(;;){` |
| 179208507 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 165576083 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 151891057 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 140181099 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  49703833 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|    913158 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|    913163 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|    913163 |   35 | `	sxu32 nH = 5381;` |
|    913163 |   36 | `	zEnd = &zIn[nLen];` |
|   1793562 |   37 | `	for(;;){` |
|   3587129 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3446327 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3260031 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   2865151 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|    913163 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     63166 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     63171 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     63171 |  154 | `	if( pSet ){` |
|     63171 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     63171 |  158 | `		pLex->pTokenSet = pSet;` |
|     31583 |  159 | `	}` |
|     63171 |  160 | `	pStream = &pLex->sStream;` |
|     63171 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     63171 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     63171 |  164 | `	pStream->nLine = 1;` |
|     63171 |  165 | `	pStream->nIgn  = 0;` |
|     63171 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     63171 |  167 | `	pStream->pSet  = pSet;` |
|     63171 |  168 | `	return SXRET_OK;` |
|     31588 |  169 | `}` |
|     63156 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     63161 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     63161 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     63161 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  37834126 |  186 | `	for(;;){` |
|  75668257 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     63127 |  189 | `			break;` |
|         - |  190 | `		}` |
|  75605135 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
|  75605135 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
|  75605135 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
|  75605101 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    106011 |  203 | `			pStream->nIgn++;` |
|  75552098 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
|  75499095 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
|  75499095 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  37749545 |  210 | `		}` |
|  75605101 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     63127 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     63127 |  225 | `	return SXRET_OK;` |
|     31583 |  226 | `}` |
|     63166 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     63171 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     31583 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     63171 |  237 | `	return rc;` |
|         5 |  238 | `}` |
