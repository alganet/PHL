...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  78665697 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  78665702 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  78665702 |   21 | `	sxu32 nH = 5381;` |
|  78665702 |   22 | `	zEnd = &zIn[nLen];` |
| 137081059 |   23 | `	for(;;){` |
| 274159417 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 252197116 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 230556610 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 213297294 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  78665702 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1540564 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1540569 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1540569 |   35 | `	sxu32 nH = 5381;` |
|   1540569 |   36 | `	zEnd = &zIn[nLen];` |
|   3070992 |   37 | `	for(;;){` |
|   6141989 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5817679 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5419401 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4901897 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1540569 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     84648 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     84653 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     84653 |  154 | `	if( pSet ){` |
|     84653 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     84653 |  158 | `		pLex->pTokenSet = pSet;` |
|     42324 |  159 | `	}` |
|     84653 |  160 | `	pStream = &pLex->sStream;` |
|     84653 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     84653 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     84653 |  164 | `	pStream->nLine = 1;` |
|     84653 |  165 | `	pStream->nIgn  = 0;` |
|     84653 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     84653 |  167 | `	pStream->pSet  = pSet;` |
|     84653 |  168 | `	return SXRET_OK;` |
|     42329 |  169 | `}` |
|     84638 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     84643 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     84643 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     84643 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  72643815 |  186 | `	for(;;){` |
| 145287635 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     84609 |  189 | `			break;` |
|         - |  190 | `		}` |
| 145203031 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 145203031 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 145203031 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 145202997 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    157519 |  203 | `			pStream->nIgn++;` |
| 145124240 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 145045483 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 145045483 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  72522739 |  210 | `		}` |
| 145202997 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     84609 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     84609 |  225 | `	return SXRET_OK;` |
|     42324 |  226 | `}` |
|     84648 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     84653 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     42324 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     84653 |  237 | `	return rc;` |
|         5 |  238 | `}` |
