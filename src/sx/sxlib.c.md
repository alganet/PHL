...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  74768537 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  74768542 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  74768542 |   21 | `	sxu32 nH = 5381;` |
|  74768542 |   22 | `	zEnd = &zIn[nLen];` |
| 130117611 |   23 | `	for(;;){` |
| 260232501 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 239429094 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 218496166 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 202353570 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  74768542 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1482066 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1482071 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1482071 |   35 | `	sxu32 nH = 5381;` |
|   1482071 |   36 | `	zEnd = &zIn[nLen];` |
|   2930475 |   37 | `	for(;;){` |
|   5860955 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5544629 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5178743 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4675611 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1482071 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     83902 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     83907 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     83907 |  154 | `	if( pSet ){` |
|     83907 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     83907 |  158 | `		pLex->pTokenSet = pSet;` |
|     41951 |  159 | `	}` |
|     83907 |  160 | `	pStream = &pLex->sStream;` |
|     83907 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     83907 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     83907 |  164 | `	pStream->nLine = 1;` |
|     83907 |  165 | `	pStream->nIgn  = 0;` |
|     83907 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     83907 |  167 | `	pStream->pSet  = pSet;` |
|     83907 |  168 | `	return SXRET_OK;` |
|     41956 |  169 | `}` |
|     83892 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     83897 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     83897 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     83897 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  69521062 |  186 | `	for(;;){` |
| 139042129 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     83863 |  189 | `			break;` |
|         - |  190 | `		}` |
| 138958271 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 138958271 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 138958271 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 138958237 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    131971 |  203 | `			pStream->nIgn++;` |
| 138892254 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 138826271 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 138826271 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  69413133 |  210 | `		}` |
| 138958237 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     83863 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     83863 |  225 | `	return SXRET_OK;` |
|     41951 |  226 | `}` |
|     83902 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     83907 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     41951 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     83907 |  237 | `	return rc;` |
|         5 |  238 | `}` |
