...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  76764897 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  76764902 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  76764902 |   21 | `	sxu32 nH = 5381;` |
|  76764902 |   22 | `	zEnd = &zIn[nLen];` |
| 132868641 |   23 | `	for(;;){` |
| 265735873 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 244384340 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 223513156 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 206323898 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  76764902 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1479810 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1479815 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1479815 |   35 | `	sxu32 nH = 5381;` |
|   1479815 |   36 | `	zEnd = &zIn[nLen];` |
|   2949521 |   37 | `	for(;;){` |
|   5899047 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5587505 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5204831 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4707849 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1479815 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     81544 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     81549 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     81549 |  154 | `	if( pSet ){` |
|     81549 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     81549 |  158 | `		pLex->pTokenSet = pSet;` |
|     40772 |  159 | `	}` |
|     81549 |  160 | `	pStream = &pLex->sStream;` |
|     81549 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     81549 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     81549 |  164 | `	pStream->nLine = 1;` |
|     81549 |  165 | `	pStream->nIgn  = 0;` |
|     81549 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     81549 |  167 | `	pStream->pSet  = pSet;` |
|     81549 |  168 | `	return SXRET_OK;` |
|     40777 |  169 | `}` |
|     81534 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     81539 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     81539 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     81539 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  69749851 |  186 | `	for(;;){` |
| 139499707 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     81505 |  189 | `			break;` |
|         - |  190 | `		}` |
| 139418207 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 139418207 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 139418207 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 139418173 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    151497 |  203 | `			pStream->nIgn++;` |
| 139342427 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 139266681 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 139266681 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  69633338 |  210 | `		}` |
| 139418173 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     81505 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     81505 |  225 | `	return SXRET_OK;` |
|     40772 |  226 | `}` |
|     81544 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     81549 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     40772 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     81549 |  237 | `	return rc;` |
|         5 |  238 | `}` |
