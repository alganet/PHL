...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  84458685 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  84458690 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  84458690 |   21 | `	sxu32 nH = 5381;` |
|  84458690 |   22 | `	zEnd = &zIn[nLen];` |
| 141948468 |   23 | `	for(;;){` |
| 283895451 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 261195890 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 237271558 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 218359458 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  84458690 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1498850 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1498855 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1498855 |   35 | `	sxu32 nH = 5381;` |
|   1498855 |   36 | `	zEnd = &zIn[nLen];` |
|   2983550 |   37 | `	for(;;){` |
|   5967105 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5652953 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5259431 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4758657 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1498855 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     82176 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     82181 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     82181 |  154 | `	if( pSet ){` |
|     82181 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     82181 |  158 | `		pLex->pTokenSet = pSet;` |
|     41088 |  159 | `	}` |
|     82181 |  160 | `	pStream = &pLex->sStream;` |
|     82181 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     82181 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     82181 |  164 | `	pStream->nLine = 1;` |
|     82181 |  165 | `	pStream->nIgn  = 0;` |
|     82181 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     82181 |  167 | `	pStream->pSet  = pSet;` |
|     82181 |  168 | `	return SXRET_OK;` |
|     41093 |  169 | `}` |
|     82166 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     82171 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     82171 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     82171 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  81978647 |  186 | `	for(;;){` |
| 163957299 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     82137 |  189 | `			break;` |
|         - |  190 | `		}` |
| 163875167 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 163875167 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 163875167 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 163875133 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    217877 |  203 | `			pStream->nIgn++;` |
| 163766197 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 163657261 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 163657261 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  81828628 |  210 | `		}` |
| 163875133 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     82137 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     82137 |  225 | `	return SXRET_OK;` |
|     41088 |  226 | `}` |
|     82176 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     82181 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     41088 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     82181 |  237 | `	return rc;` |
|         5 |  238 | `}` |
