...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  51158425 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  51158430 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  51158430 |   21 | `	sxu32 nH = 5381;` |
|  51158430 |   22 | `	zEnd = &zIn[nLen];` |
|  91793941 |   23 | `	for(;;){` |
| 183585161 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 169419774 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 155360294 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 143374380 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  51158430 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|    924798 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|    924803 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|    924803 |   35 | `	sxu32 nH = 5381;` |
|    924803 |   36 | `	zEnd = &zIn[nLen];` |
|   1814876 |   37 | `	for(;;){` |
|   3629757 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3486935 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3298159 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   2898817 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|    924803 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     63974 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     63979 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     63979 |  154 | `	if( pSet ){` |
|     63979 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     63979 |  158 | `		pLex->pTokenSet = pSet;` |
|     31987 |  159 | `	}` |
|     63979 |  160 | `	pStream = &pLex->sStream;` |
|     63979 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     63979 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     63979 |  164 | `	pStream->nLine = 1;` |
|     63979 |  165 | `	pStream->nIgn  = 0;` |
|     63979 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     63979 |  167 | `	pStream->pSet  = pSet;` |
|     63979 |  168 | `	return SXRET_OK;` |
|     31992 |  169 | `}` |
|     63964 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     63969 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     63969 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     63969 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  39131775 |  186 | `	for(;;){` |
|  78263555 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     63935 |  189 | `			break;` |
|         - |  190 | `		}` |
|  78199625 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
|  78199625 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
|  78199625 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
|  78199591 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    107865 |  203 | `			pStream->nIgn++;` |
|  78145661 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
|  78091731 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
|  78091731 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  39045863 |  210 | `		}` |
|  78199591 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     63935 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     63935 |  225 | `	return SXRET_OK;` |
|     31987 |  226 | `}` |
|     63974 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     63979 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     31987 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     63979 |  237 | `	return rc;` |
|         5 |  238 | `}` |
