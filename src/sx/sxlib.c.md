...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  77613315 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  77613320 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  77613320 |   21 | `	sxu32 nH = 5381;` |
|  77613320 |   22 | `	zEnd = &zIn[nLen];` |
| 135532081 |   23 | `	for(;;){` |
| 271061441 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 249371568 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 227774038 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 211019290 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  77613320 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1537478 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1537483 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1537483 |   35 | `	sxu32 nH = 5381;` |
|   1537483 |   36 | `	zEnd = &zIn[nLen];` |
|   3064816 |   37 | `	for(;;){` |
|   6129637 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5805977 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5408495 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4892031 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1537483 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     84704 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     84709 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     84709 |  154 | `	if( pSet ){` |
|     84709 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     84709 |  158 | `		pLex->pTokenSet = pSet;` |
|     42352 |  159 | `	}` |
|     84709 |  160 | `	pStream = &pLex->sStream;` |
|     84709 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     84709 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     84709 |  164 | `	pStream->nLine = 1;` |
|     84709 |  165 | `	pStream->nIgn  = 0;` |
|     84709 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     84709 |  167 | `	pStream->pSet  = pSet;` |
|     84709 |  168 | `	return SXRET_OK;` |
|     42357 |  169 | `}` |
|     84694 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     84699 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     84699 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     84699 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  72492363 |  186 | `	for(;;){` |
| 144984731 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     84665 |  189 | `			break;` |
|         - |  190 | `		}` |
| 144900071 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 144900071 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 144900071 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 144900037 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    153051 |  203 | `			pStream->nIgn++;` |
| 144823514 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 144746991 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 144746991 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  72373493 |  210 | `		}` |
| 144900037 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     84665 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     84665 |  225 | `	return SXRET_OK;` |
|     42352 |  226 | `}` |
|     84704 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     84709 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     42352 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     84709 |  237 | `	return rc;` |
|         5 |  238 | `}` |
