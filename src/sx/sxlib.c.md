...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  95865591 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  95865596 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  95865596 |   21 | `	sxu32 nH = 5381;` |
|  95865596 |   22 | `	zEnd = &zIn[nLen];` |
| 162361832 |   23 | `	for(;;){` |
| 324683675 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 298913642 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 272142218 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 250613404 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  95865596 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1779474 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1779479 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1779479 |   35 | `	sxu32 nH = 5381;` |
|   1779479 |   36 | `	zEnd = &zIn[nLen];` |
|   3489869 |   37 | `	for(;;){` |
|   6979743 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   6621763 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   6179013 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5596557 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1779479 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|    102242 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|    102247 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|    102247 |  154 | `	if( pSet ){` |
|    102247 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|    102247 |  158 | `		pLex->pTokenSet = pSet;` |
|     51121 |  159 | `	}` |
|    102247 |  160 | `	pStream = &pLex->sStream;` |
|    102247 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|    102247 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|    102247 |  164 | `	pStream->nLine = 1;` |
|    102247 |  165 | `	pStream->nIgn  = 0;` |
|    102247 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|    102247 |  167 | `	pStream->pSet  = pSet;` |
|    102247 |  168 | `	return SXRET_OK;` |
|     51126 |  169 | `}` |
|    102242 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|    102247 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|    102247 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|    102247 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  92078343 |  186 | `	for(;;){` |
| 184156691 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|    102229 |  189 | `			break;` |
|         - |  190 | `		}` |
| 184054467 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 184054467 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 184054467 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 184054449 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    233039 |  203 | `			pStream->nIgn++;` |
| 183937932 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 183821415 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 183821415 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  91910705 |  210 | `		}` |
| 184054449 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|    102229 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|    102229 |  225 | `	return SXRET_OK;` |
|     51126 |  226 | `}` |
|    102242 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|    102247 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     51121 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|    102247 |  237 | `	return rc;` |
|         5 |  238 | `}` |
