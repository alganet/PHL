...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  50190466 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  50190471 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  50190471 |   21 | `	sxu32 nH = 5381;` |
|  50190471 |   22 | `	zEnd = &zIn[nLen];` |
|  90481305 |   23 | `	for(;;){` |
| 180959895 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 167200171 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 153404307 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 141550505 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  50190471 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|    919596 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|    919601 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|    919601 |   35 | `	sxu32 nH = 5381;` |
|    919601 |   36 | `	zEnd = &zIn[nLen];` |
|   1805774 |   37 | `	for(;;){` |
|   3611553 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3469781 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3281967 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   2884471 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|    919601 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     63594 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     63599 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     63599 |  154 | `	if( pSet ){` |
|     63599 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     63599 |  158 | `		pLex->pTokenSet = pSet;` |
|     31797 |  159 | `	}` |
|     63599 |  160 | `	pStream = &pLex->sStream;` |
|     63599 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     63599 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     63599 |  164 | `	pStream->nLine = 1;` |
|     63599 |  165 | `	pStream->nIgn  = 0;` |
|     63599 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     63599 |  167 | `	pStream->pSet  = pSet;` |
|     63599 |  168 | `	return SXRET_OK;` |
|     31802 |  169 | `}` |
|     63584 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     63589 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     63589 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     63589 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  38078647 |  186 | `	for(;;){` |
|  76157299 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     63555 |  189 | `			break;` |
|         - |  190 | `		}` |
|  76093749 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
|  76093749 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
|  76093749 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
|  76093715 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    107037 |  203 | `			pStream->nIgn++;` |
|  76040199 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
|  75986683 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
|  75986683 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  37993339 |  210 | `		}` |
|  76093715 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     63555 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     63555 |  225 | `	return SXRET_OK;` |
|     31797 |  226 | `}` |
|     63594 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     63599 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     31797 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     63599 |  237 | `	return rc;` |
|         5 |  238 | `}` |
