...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  96200328 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  96200333 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  96200333 |   21 | `	sxu32 nH = 5381;` |
|  96200333 |   22 | `	zEnd = &zIn[nLen];` |
| 163402557 |   23 | `	for(;;){` |
| 326764763 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 301254394 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 274126295 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 252245757 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  96200333 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1850490 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1850495 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1850495 |   35 | `	sxu32 nH = 5381;` |
|   1850495 |   36 | `	zEnd = &zIn[nLen];` |
|   3608593 |   37 | `	for(;;){` |
|   7217191 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   6838063 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   6377141 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5772787 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1850495 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|    106058 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|    106063 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|    106063 |  154 | `	if( pSet ){` |
|    106063 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|    106063 |  158 | `		pLex->pTokenSet = pSet;` |
|     53029 |  159 | `	}` |
|    106063 |  160 | `	pStream = &pLex->sStream;` |
|    106063 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|    106063 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|    106063 |  164 | `	pStream->nLine = 1;` |
|    106063 |  165 | `	pStream->nIgn  = 0;` |
|    106063 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|    106063 |  167 | `	pStream->pSet  = pSet;` |
|    106063 |  168 | `	return SXRET_OK;` |
|     53034 |  169 | `}` |
|    106058 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|    106063 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|    106063 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|    106063 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  93554966 |  186 | `	for(;;){` |
| 187109937 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|    106043 |  189 | `			break;` |
|         - |  190 | `		}` |
| 187003899 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 187003899 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 187003899 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|         3 |  199 | `			break;` |
|         - |  200 | `		}` |
| 187003879 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    233755 |  203 | `			pStream->nIgn++;` |
| 186887004 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 186770129 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 186770129 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  93385062 |  210 | `		}` |
| 187003879 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|    106045 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|    106045 |  225 | `	return SXRET_OK;` |
|     53034 |  226 | `}` |
|    106058 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|    106063 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     53029 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|    106063 |  237 | `	return rc;` |
|         5 |  238 | `}` |
