...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  77786407 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  77786412 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  77786412 |   21 | `	sxu32 nH = 5381;` |
|  77786412 |   22 | `	zEnd = &zIn[nLen];` |
| 134031813 |   23 | `	for(;;){` |
| 268062217 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 246527694 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 225291416 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 207819642 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  77786412 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1487922 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1487927 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1487927 |   35 | `	sxu32 nH = 5381;` |
|   1487927 |   36 | `	zEnd = &zIn[nLen];` |
|   2962075 |   37 | `	for(;;){` |
|   5924155 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5612439 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5222253 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4724995 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1487927 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     81498 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     81503 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     81503 |  154 | `	if( pSet ){` |
|     81503 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     81503 |  158 | `		pLex->pTokenSet = pSet;` |
|     40749 |  159 | `	}` |
|     81503 |  160 | `	pStream = &pLex->sStream;` |
|     81503 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     81503 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     81503 |  164 | `	pStream->nLine = 1;` |
|     81503 |  165 | `	pStream->nIgn  = 0;` |
|     81503 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     81503 |  167 | `	pStream->pSet  = pSet;` |
|     81503 |  168 | `	return SXRET_OK;` |
|     40754 |  169 | `}` |
|     81488 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     81493 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     81493 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     81493 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  70720835 |  186 | `	for(;;){` |
| 141441675 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     81459 |  189 | `			break;` |
|         - |  190 | `		}` |
| 141360221 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 141360221 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 141360221 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 141360187 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    151587 |  203 | `			pStream->nIgn++;` |
| 141284396 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 141208605 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 141208605 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  70604300 |  210 | `		}` |
| 141360187 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     81459 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     81459 |  225 | `	return SXRET_OK;` |
|     40749 |  226 | `}` |
|     81498 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     81503 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     40749 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     81503 |  237 | `	return rc;` |
|         5 |  238 | `}` |
