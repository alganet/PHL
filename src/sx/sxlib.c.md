...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  73829223 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  73829228 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  73829228 |   21 | `	sxu32 nH = 5381;` |
|  73829228 |   22 | `	zEnd = &zIn[nLen];` |
| 128692450 |   23 | `	for(;;){` |
| 257382115 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 236746672 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 216111780 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 200205390 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  73829228 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1454664 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1454669 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1454669 |   35 | `	sxu32 nH = 5381;` |
|   1454669 |   36 | `	zEnd = &zIn[nLen];` |
|   2885456 |   37 | `	for(;;){` |
|   5770917 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5454609 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5104387 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4601255 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1454669 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     83896 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     83901 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     83901 |  154 | `	if( pSet ){` |
|     83901 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     83901 |  158 | `		pLex->pTokenSet = pSet;` |
|     41948 |  159 | `	}` |
|     83901 |  160 | `	pStream = &pLex->sStream;` |
|     83901 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     83901 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     83901 |  164 | `	pStream->nLine = 1;` |
|     83901 |  165 | `	pStream->nIgn  = 0;` |
|     83901 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     83901 |  167 | `	pStream->pSet  = pSet;` |
|     83901 |  168 | `	return SXRET_OK;` |
|     41953 |  169 | `}` |
|     83886 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     83891 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     83891 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     83891 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  68130727 |  186 | `	for(;;){` |
| 136261459 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     83857 |  189 | `			break;` |
|         - |  190 | `		}` |
| 136177607 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 136177607 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 136177607 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 136177573 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    131967 |  203 | `			pStream->nIgn++;` |
| 136111592 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 136045611 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 136045611 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  68022803 |  210 | `		}` |
| 136177573 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     83857 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     83857 |  225 | `	return SXRET_OK;` |
|     41948 |  226 | `}` |
|     83896 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     83901 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     41948 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     83901 |  237 | `	return rc;` |
|         5 |  238 | `}` |
