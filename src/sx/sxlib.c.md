...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  78385075 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  78385080 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  78385080 |   21 | `	sxu32 nH = 5381;` |
|  78385080 |   22 | `	zEnd = &zIn[nLen];` |
| 136590095 |   23 | `	for(;;){` |
| 273178781 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 251288182 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 229713416 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 212529678 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  78385080 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1535224 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1535229 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1535229 |   35 | `	sxu32 nH = 5381;` |
|   1535229 |   36 | `	zEnd = &zIn[nLen];` |
|   3060306 |   37 | `	for(;;){` |
|   6120617 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5797439 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5400511 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4884815 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1535229 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     84334 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     84339 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     84339 |  154 | `	if( pSet ){` |
|     84339 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     84339 |  158 | `		pLex->pTokenSet = pSet;` |
|     42167 |  159 | `	}` |
|     84339 |  160 | `	pStream = &pLex->sStream;` |
|     84339 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     84339 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     84339 |  164 | `	pStream->nLine = 1;` |
|     84339 |  165 | `	pStream->nIgn  = 0;` |
|     84339 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     84339 |  167 | `	pStream->pSet  = pSet;` |
|     84339 |  168 | `	return SXRET_OK;` |
|     42172 |  169 | `}` |
|     84324 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     84329 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     84329 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     84329 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  72387279 |  186 | `	for(;;){` |
| 144774563 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     84295 |  189 | `			break;` |
|         - |  190 | `		}` |
| 144690273 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 144690273 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 144690273 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 144690239 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    156983 |  203 | `			pStream->nIgn++;` |
| 144611750 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 144533261 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 144533261 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  72266628 |  210 | `		}` |
| 144690239 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     84295 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     84295 |  225 | `	return SXRET_OK;` |
|     42167 |  226 | `}` |
|     84334 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     84339 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     42167 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     84339 |  237 | `	return rc;` |
|         5 |  238 | `}` |
