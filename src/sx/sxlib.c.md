...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  77126511 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  77126516 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  77126516 |   21 | `	sxu32 nH = 5381;` |
|  77126516 |   22 | `	zEnd = &zIn[nLen];` |
| 133445479 |   23 | `	for(;;){` |
| 266889549 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 245443700 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 224581778 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 207210510 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  77126516 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1479832 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1479837 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1479837 |   35 | `	sxu32 nH = 5381;` |
|   1479837 |   36 | `	zEnd = &zIn[nLen];` |
|   2949551 |   37 | `	for(;;){` |
|   5899107 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5587565 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5204881 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4707887 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1479837 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     81480 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     81485 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     81485 |  154 | `	if( pSet ){` |
|     81485 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     81485 |  158 | `		pLex->pTokenSet = pSet;` |
|     40740 |  159 | `	}` |
|     81485 |  160 | `	pStream = &pLex->sStream;` |
|     81485 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     81485 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     81485 |  164 | `	pStream->nLine = 1;` |
|     81485 |  165 | `	pStream->nIgn  = 0;` |
|     81485 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     81485 |  167 | `	pStream->pSet  = pSet;` |
|     81485 |  168 | `	return SXRET_OK;` |
|     40745 |  169 | `}` |
|     81470 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     81475 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     81475 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     81475 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  69749500 |  186 | `	for(;;){` |
| 139499005 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     81441 |  189 | `			break;` |
|         - |  190 | `		}` |
| 139417569 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 139417569 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 139417569 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 139417535 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    151503 |  203 | `			pStream->nIgn++;` |
| 139341786 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 139266037 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 139266037 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  69633016 |  210 | `		}` |
| 139417535 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     81441 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     81441 |  225 | `	return SXRET_OK;` |
|     40740 |  226 | `}` |
|     81480 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     81485 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     40740 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     81485 |  237 | `	return rc;` |
|         5 |  238 | `}` |
