...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  82732970 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  82732975 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  82732975 |   21 | `	sxu32 nH = 5381;` |
|  82732975 |   22 | `	zEnd = &zIn[nLen];` |
| 142833618 |   23 | `	for(;;){` |
| 285626509 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 262992210 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 238006977 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 219598505 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  82732975 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|  19452834 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|  19452839 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|  19452839 |   35 | `	sxu32 nH = 5381;` |
|  19452839 |   36 | `	zEnd = &zIn[nLen];` |
|  30228040 |   37 | `	for(;;){` |
|  60456085 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  56103511 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  52645245 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  47672015 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|  19452839 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|    105890 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|    105895 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|    105895 |  154 | `	if( pSet ){` |
|    105895 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|    105895 |  158 | `		pLex->pTokenSet = pSet;` |
|     52945 |  159 | `	}` |
|    105895 |  160 | `	pStream = &pLex->sStream;` |
|    105895 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|    105895 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|    105895 |  164 | `	pStream->nLine = 1;` |
|    105895 |  165 | `	pStream->nIgn  = 0;` |
|    105895 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|    105895 |  167 | `	pStream->pSet  = pSet;` |
|    105895 |  168 | `	return SXRET_OK;` |
|     52950 |  169 | `}` |
|    105890 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|    105895 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|    105895 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|    105895 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  93412165 |  186 | `	for(;;){` |
| 186824335 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|    105875 |  189 | `			break;` |
|         - |  190 | `		}` |
| 186718465 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 186718465 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 186718465 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|         3 |  199 | `			break;` |
|         - |  200 | `		}` |
| 186718445 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    233781 |  203 | `			pStream->nIgn++;` |
| 186601557 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 186484669 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 186484669 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  93242332 |  210 | `		}` |
| 186718445 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|    105877 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|    105877 |  225 | `	return SXRET_OK;` |
|     52950 |  226 | `}` |
|    105890 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|    105895 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     52945 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|    105895 |  237 | `	return rc;` |
|         5 |  238 | `}` |
