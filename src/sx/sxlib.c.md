...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  64175827 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  64175832 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  64175832 |   21 | `	sxu32 nH = 5381;` |
|  64175832 |   22 | `	zEnd = &zIn[nLen];` |
| 112554955 |   23 | `	for(;;){` |
| 225107189 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 206813398 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 189169104 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 174878238 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  64175832 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1273254 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1273259 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1273259 |   35 | `	sxu32 nH = 5381;` |
|   1273259 |   36 | `	zEnd = &zIn[nLen];` |
|   2581186 |   37 | `	for(;;){` |
|   5162377 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4898837 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4582155 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4113773 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1273259 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     75562 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     75567 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     75567 |  154 | `	if( pSet ){` |
|     75567 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     75567 |  158 | `		pLex->pTokenSet = pSet;` |
|     37781 |  159 | `	}` |
|     75567 |  160 | `	pStream = &pLex->sStream;` |
|     75567 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     75567 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     75567 |  164 | `	pStream->nLine = 1;` |
|     75567 |  165 | `	pStream->nIgn  = 0;` |
|     75567 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     75567 |  167 | `	pStream->pSet  = pSet;` |
|     75567 |  168 | `	return SXRET_OK;` |
|     37786 |  169 | `}` |
|     75552 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     75557 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     75557 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     75557 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  55061290 |  186 | `	for(;;){` |
| 110122585 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     75523 |  189 | `			break;` |
|         - |  190 | `		}` |
| 110047067 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 110047067 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 110047067 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 110047033 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    107785 |  203 | `			pStream->nIgn++;` |
| 109993143 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 109939253 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 109939253 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  54969624 |  210 | `		}` |
| 110047033 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     75523 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     75523 |  225 | `	return SXRET_OK;` |
|     37781 |  226 | `}` |
|     75562 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     75567 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     37781 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     75567 |  237 | `	return rc;` |
|         5 |  238 | `}` |
