...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  51145571 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  51145576 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  51145576 |   21 | `	sxu32 nH = 5381;` |
|  51145576 |   22 | `	zEnd = &zIn[nLen];` |
|  91773878 |   23 | `	for(;;){` |
| 183545035 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 169383040 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 155328730 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 143345374 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  51145576 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|    924592 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|    924597 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|    924597 |   35 | `	sxu32 nH = 5381;` |
|    924597 |   36 | `	zEnd = &zIn[nLen];` |
|   1814716 |   37 | `	for(;;){` |
|   3629437 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3486619 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   3297907 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   2898567 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|    924597 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     63970 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     63975 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     63975 |  154 | `	if( pSet ){` |
|     63975 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     63975 |  158 | `		pLex->pTokenSet = pSet;` |
|     31985 |  159 | `	}` |
|     63975 |  160 | `	pStream = &pLex->sStream;` |
|     63975 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     63975 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     63975 |  164 | `	pStream->nLine = 1;` |
|     63975 |  165 | `	pStream->nIgn  = 0;` |
|     63975 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     63975 |  167 | `	pStream->pSet  = pSet;` |
|     63975 |  168 | `	return SXRET_OK;` |
|     31990 |  169 | `}` |
|     63960 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     63965 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     63965 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     63965 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  39129855 |  186 | `	for(;;){` |
|  78259715 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     63931 |  189 | `			break;` |
|         - |  190 | `		}` |
|  78195789 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
|  78195789 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
|  78195789 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
|  78195755 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    107815 |  203 | `			pStream->nIgn++;` |
|  78141850 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
|  78087945 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
|  78087945 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  39043970 |  210 | `		}` |
|  78195755 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     63931 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     63931 |  225 | `	return SXRET_OK;` |
|     31985 |  226 | `}` |
|     63970 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     63975 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     31985 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     63975 |  237 | `	return rc;` |
|         5 |  238 | `}` |
