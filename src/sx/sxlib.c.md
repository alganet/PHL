...
|        - |   15 | `#include "sxstr.h"` |
|        - |   16 |  |
| 25515104 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|        5 |   18 | `{` |
| 25515109 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|        - |   20 | `	unsigned char *zEnd;` |
| 25515109 |   21 | `	sxu32 nH = 5381;` |
| 25515109 |   22 | `	zEnd = &zIn[nLen];` |
| 45418621 |   23 | `	for(;;){` |
| 90834527 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 83985973 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 77906601 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 71297381 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|        5 |   28 | `	}` |
| 25515109 |   29 | `	return nH;` |
|        5 |   30 | `}` |
|   549346 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|        5 |   32 | `{` |
|   549351 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|        - |   34 | `	unsigned char *zEnd;` |
|   549351 |   35 | `	sxu32 nH = 5381;` |
|   549351 |   36 | `	zEnd = &zIn[nLen];` |
|  1014527 |   37 | `	for(;;){` |
|  2029059 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  1911695 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  1775681 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|  1565761 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|        5 |   42 | `	}` |
|   549351 |   43 | `	return nH;` |
|        5 |   44 | `}` |
...
|  6387624 |  186 | `	for(;;){` |
| 12775253 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|        - |  188 | `			/* End of the input reached */` |
|    20667 |  189 | `			break;` |
|        - |  190 | `		}` |
| 12754591 |  191 | `		zCur = pStream->zText;` |
|        - |  192 | `		/* Call the tokenizer callback */` |
| 12754591 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 12754591 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|      ! 0 |  199 | `			break;` |
|        - |  200 | `		}` |
| 12754557 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|        - |  202 | `			/* Request to ignore this token */` |
|    98195 |  203 | `			pStream->nIgn++;` |
| 12705462 |  204 | `		}else if( pLex->pTokenSet  ){` |
|        - |  205 | `			/* Put the token in the set */` |
| 12656367 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 12656367 |  207 | `			if( rc != SXRET_OK ){` |
|      ! 0 |  208 | `				break;` |
|        - |  209 | `			}` |
|  6328181 |  210 | `		}` |
| 12754557 |  211 | `		if( zCur >= pStream->zText ){` |
