...
|         - |   15 | `#include "sxstr.h"` |
|         - |   16 |  |
|  77916051 |   17 | `PH7_PRIVATE sxu32 SyBinHash(const void *pSrc,sxu32 nLen)` |
|         5 |   18 | `{` |
|  77916056 |   19 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   20 | `	unsigned char *zEnd;` |
|  77916056 |   21 | `	sxu32 nH = 5381;` |
|  77916056 |   22 | `	zEnd = &zIn[nLen];` |
| 134192064 |   23 | `	for(;;){` |
| 268382559 |   24 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 246829276 |   25 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 225526076 |   26 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
| 208039010 |   27 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + zIn[0] ; zIn++;` |
|         5 |   28 | `	}` |
|  77916056 |   29 | `	return nH;` |
|         5 |   30 | `}` |
|   1488706 |   31 | `PH7_PRIVATE sxu32 SyStrHash(const void *pSrc,sxu32 nLen)` |
|         5 |   32 | `{` |
|   1488711 |   33 | `	register unsigned char *zIn = (unsigned char *)pSrc;` |
|         - |   34 | `	unsigned char *zEnd;` |
|   1488711 |   35 | `	sxu32 nH = 5381;` |
|   1488711 |   36 | `	zEnd = &zIn[nLen];` |
|   2963635 |   37 | `	for(;;){` |
|   5927275 |   38 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5615397 |   39 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   5224999 |   40 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|   4727481 |   41 | `		if( zIn >= zEnd ){ break; } nH = nH * 33 + SyToLower(zIn[0]); zIn++;` |
|         5 |   42 | `	}` |
|   1488711 |   43 | `	return nH;` |
|         5 |   44 | `}` |
...
|     81542 |  144 | `PH7_PRIVATE sxi32 SyLexInit(SyLex *pLex,SySet *pSet,ProcTokenizer xTokenizer,void *pUserData)` |
|         5 |  145 | `{` |
...
|         - |  150 | `	}` |
|         - |  151 | `#endif` |
|     81547 |  152 | `	pLex->pTokenSet = 0;` |
|         - |  153 | `	/* Initialize lexer fields */` |
|     81547 |  154 | `	if( pSet ){` |
|     81547 |  155 | `		if ( SySetElemSize(pSet) != sizeof(SyToken) ){` |
|       ! 0 |  156 | `			return SXERR_INVALID;` |
|         - |  157 | `		}` |
|     81547 |  158 | `		pLex->pTokenSet = pSet;` |
|     40771 |  159 | `	}` |
|     81547 |  160 | `	pStream = &pLex->sStream;` |
|     81547 |  161 | `	pLex->xTokenizer = xTokenizer;` |
|     81547 |  162 | `	pLex->pUserData = pUserData;` |
|         - |  163 |  |
|     81547 |  164 | `	pStream->nLine = 1;` |
|     81547 |  165 | `	pStream->nIgn  = 0;` |
|     81547 |  166 | `	pStream->zText = pStream->zEnd = 0;` |
|     81547 |  167 | `	pStream->pSet  = pSet;` |
|     81547 |  168 | `	return SXRET_OK;` |
|     40776 |  169 | `}` |
|     81532 |  170 | `PH7_PRIVATE sxi32 SyLexTokenizeInput(SyLex *pLex,const char *zInput,sxu32 nLen,void *pCtxData,ProcSort xSort,ProcCmp xCmp)` |
|         5 |  171 | `{` |
...
|         - |  179 | `	}` |
|         - |  180 | `#endif` |
|     81537 |  181 | `	pStream = &pLex->sStream;` |
|         - |  182 | `	/* Point to the head of the input */` |
|     81537 |  183 | `	pStream->zText = pStream->zInput = (const unsigned char *)zInput;` |
|         - |  184 | `	/* Point to the end of the input */` |
|     81537 |  185 | `	pStream->zEnd = &pStream->zInput[nLen];` |
|  70927188 |  186 | `	for(;;){` |
| 141854381 |  187 | `		if( pStream->zText >= pStream->zEnd ){` |
|         - |  188 | `			/* End of the input reached */` |
|     81503 |  189 | `			break;` |
|         - |  190 | `		}` |
| 141772883 |  191 | `		zCur = pStream->zText;` |
|         - |  192 | `		/* Call the tokenizer callback */` |
| 141772883 |  193 | `		rc = pLex->xTokenizer(pStream,&sToken,pLex->pUserData,pCtxData);` |
| 141772883 |  194 | `		if( rc != SXRET_OK && rc != SXERR_CONTINUE ){` |
...
|       ! 0 |  199 | `			break;` |
|         - |  200 | `		}` |
| 141772849 |  201 | `		if( rc == SXERR_CONTINUE ){` |
|         - |  202 | `			/* Request to ignore this token */` |
|    155463 |  203 | `			pStream->nIgn++;` |
| 141695120 |  204 | `		}else if( pLex->pTokenSet  ){` |
|         - |  205 | `			/* Put the token in the set */` |
| 141617391 |  206 | `			rc = SySetPut(pLex->pTokenSet,(const void *)&sToken);` |
| 141617391 |  207 | `			if( rc != SXRET_OK ){` |
|       ! 0 |  208 | `				break;` |
|         - |  209 | `			}` |
|  70808693 |  210 | `		}` |
| 141772849 |  211 | `		if( zCur >= pStream->zText ){` |
|         - |  212 | `			/* Automatic advance of the stream cursor */` |
|       ! 0 |  213 | `			pStream->zText = &zCur[1];` |
|       ! 0 |  214 | `		}` |
|         5 |  215 | `	}` |
|     81503 |  216 | `	if( xSort &&  pLex->pTokenSet ){` |
...
|       ! 0 |  224 | `	}` |
|     81503 |  225 | `	return SXRET_OK;` |
|     40771 |  226 | `}` |
|     81542 |  227 | `PH7_PRIVATE sxi32 SyLexRelease(SyLex *pLex)` |
|         5 |  228 | `{` |
|     81547 |  229 | `	sxi32 rc = SXRET_OK;` |
|         - |  230 | `#if defined (UNTRUST)` |
|         - |  231 | `	if ( INVALID_LEXER(pLex) ){` |
|         - |  232 | `		return SXERR_CORRUPT;` |
|         - |  233 | `	}` |
|         - |  234 | `#else` |
|     40771 |  235 | `	SXUNUSED(pLex); /* Prevent compiler warning */` |
|         - |  236 | `#endif` |
|     81547 |  237 | `	return rc;` |
|         5 |  238 | `}` |
