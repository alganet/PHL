...
|        - |    8 | `#include "sxstr.h"` |
|        - |    9 |  |
| 22837098 |   10 | `PH7_PRIVATE sxu32 SyStrlen(const char *zSrc)` |
|        5 |   11 | `{` |
| 22837103 |   12 | `	register const char *zIn = zSrc;` |
...
|        - |   17 | `#endif` |
| 36612821 |   18 | `	for(;;){` |
| 73196050 |   19 | `		if( !zIn[0] ){ break; } zIn++;` |
| 67185939 |   20 | `		if( !zIn[0] ){ break; } zIn++;` |
| 60883337 |   21 | `		if( !zIn[0] ){ break; } zIn++;` |
| 55438210 |   22 | `		if( !zIn[0] ){ break; } zIn++;` |
|        5 |   23 | `	}` |
| 22837103 |   24 | `	return (sxu32)(zIn - zSrc);` |
|        5 |   25 | `}` |
...
|  1030719 |   72 | `PH7_PRIVATE sxi32 SyStrncmp(const char *zLeft,const char *zRight,sxu32 nLen)` |
|        5 |   73 | `{` |
|  1030724 |   74 | `	const unsigned char *zP = (const unsigned char *)zLeft;` |
|  1030724 |   75 | `	const unsigned char *zQ = (const unsigned char *)zRight;` |
|        - |   76 |  |
...
|  1030724 |   81 | `	if( nLen <= 0 ){` |
|       17 |   82 | `		return 0;` |
|        - |   83 | `	}` |
|  1030708 |   84 | `	if( SX_EMPTY_STR(zP) \|\| SX_EMPTY_STR(zQ)  ){` |
|      ! 0 |   85 | `			return SX_EMPTY_STR(zP) ? (SX_EMPTY_STR(zQ) ? 0 : -1) :1;` |
|        - |   86 | `	}` |
|   535196 |   87 | `	for(;;){` |
|  1069656 |   88 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    43739 |   89 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    42679 |   90 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    42631 |   91 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|        5 |   92 | `	}` |
|  1023746 |   93 | `	return (sxi32)(zP[0] - zQ[0]);` |
|   515735 |   94 | `}` |
| 15043024 |   95 | `PH7_PRIVATE sxi32 SyStrnicmp(const char *zLeft, const char *zRight,sxu32 SLen)` |
|        5 |   96 | `{` |
| 15043029 |   97 | `  	register unsigned char *p = (unsigned char *)zLeft;` |
| 15043029 |   98 | `	register unsigned char *q = (unsigned char *)zRight;` |
|        - |   99 |  |
|        - |  100 | `	/* Zero bytes compared is always equal -- see the note in SyStrncmp. */` |
| 15043029 |  101 | `	if( !SLen ){` |
|      ! 0 |  102 | `		return 0;` |
|        - |  103 | `	}` |
| 15043029 |  104 | `	if( SX_EMPTY_STR(p) \|\| SX_EMPTY_STR(q) ){` |
|      ! 0 |  105 | `		return SX_EMPTY_STR(p)? SX_EMPTY_STR(q) ? 0 : -1 :1;` |
|        - |  106 | `	}` |
| 12030656 |  107 | `	for(;;){` |
| 24061317 |  108 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
| 14223091 |  109 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
| 12299737 |  110 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
| 10897715 |  111 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
|        - |  112 |  |
|        5 |  113 | `	}` |
|  8061357 |  114 | `	return (sxi32)(SyCharToLower(p[0]) - SyCharToLower(q[0]));` |
|  7521517 |  115 | `}` |
|  3250884 |  116 | `PH7_PRIVATE sxi32 SyStrnmicmp(const void *pLeft, const void *pRight,sxu32 SLen)` |
|        5 |  117 | `{` |
|  3250889 |  118 | `	return SyStrnicmp((const char *)pLeft,(const char *)pRight,SLen);` |
|        5 |  119 | `}` |
|  9853194 |  120 | `PH7_PRIVATE sxu32 Systrcpy(char *zDest,sxu32 nDestLen,const char *zSrc,sxu32 nLen)` |
|        5 |  121 | `{` |
|  9853199 |  122 | `	unsigned char *zBuf = (unsigned char *)zDest;` |
|  9853199 |  123 | `	unsigned char *zIn = (unsigned char *)zSrc;` |
...
|        - |  128 | `	}` |
|        - |  129 | `#endif` |
|  9853199 |  130 | `	if( nLen <= 0 ){` |
|      ! 0 |  131 | `		nLen = SyStrlen(zSrc);` |
|      ! 0 |  132 | `	}` |
|  9853199 |  133 | `	zEnd = &zBuf[nDestLen - 1]; /* reserve a room for the null terminator */` |
| 14975781 |  134 | `	for(;;){` |
| 29949236 |  135 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
| 27293323 |  136 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
| 24580848 |  137 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
| 22292767 |  138 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
|        5 |  139 | `	}` |
|  9853199 |  140 | `	zBuf[0] = 0;` |
|  9853199 |  141 | `	return (sxu32)(zBuf-(unsigned char *)zDest);` |
|        5 |  142 | `}` |
|        - |  143 |  |
