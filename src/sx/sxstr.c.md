...
|        - |    8 | `#include "sxstr.h"` |
|        - |    9 |  |
|  6416629 |   10 | `PH7_PRIVATE sxu32 SyStrlen(const char *zSrc)` |
|        5 |   11 | `{` |
|  6416634 |   12 | `	register const char *zIn = zSrc;` |
...
|        - |   17 | `#endif` |
| 11196370 |   18 | `	for(;;){` |
| 22388496 |   19 | `		if( !zIn[0] ){ break; } zIn++;` |
| 20944969 |   20 | `		if( !zIn[0] ){ break; } zIn++;` |
| 19303899 |   21 | `		if( !zIn[0] ){ break; } zIn++;` |
| 17232042 |   22 | `		if( !zIn[0] ){ break; } zIn++;` |
|        5 |   23 | `	}` |
|  6416634 |   24 | `	return (sxu32)(zIn - zSrc);` |
|        5 |   25 | `}` |
...
|    24422 |   72 | `PH7_PRIVATE sxi32 SyStrncmp(const char *zLeft,const char *zRight,sxu32 nLen)` |
|        5 |   73 | `{` |
|    24427 |   74 | `	const unsigned char *zP = (const unsigned char *)zLeft;` |
|    24427 |   75 | `	const unsigned char *zQ = (const unsigned char *)zRight;` |
|        - |   76 |  |
|    24427 |   77 | `	if( SX_EMPTY_STR(zP) \|\| SX_EMPTY_STR(zQ)  ){` |
...
|        - |   82 | `	}` |
|    24379 |   83 | `	for(;;){` |
|    48763 |   84 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    24389 |   85 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    24369 |   86 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|    24359 |   87 | `		if( nLen <= 0 ){ return 0; } if( zP[0] == 0 \|\| zQ[0] == 0 \|\| zP[0] != zQ[0] ){ break; } zP++; zQ++; nLen--;` |
|        5 |   88 | `	}` |
|    24379 |   89 | `	return (sxi32)(zP[0] - zQ[0]);` |
|    12216 |   90 | `}` |
|        - |   91 | `#endif` |
|   846578 |   92 | `PH7_PRIVATE sxi32 SyStrnicmp(const char *zLeft, const char *zRight,sxu32 SLen)` |
|        5 |   93 | `{` |
|   846583 |   94 | `  	register unsigned char *p = (unsigned char *)zLeft;` |
|   846583 |   95 | `	register unsigned char *q = (unsigned char *)zRight;` |
|        - |   96 |  |
|   846583 |   97 | `	if( SX_EMPTY_STR(p) \|\| SX_EMPTY_STR(q) ){` |
|      ! 0 |   98 | `		return SX_EMPTY_STR(p)? SX_EMPTY_STR(q) ? 0 : -1 :1;` |
|        - |   99 | `	}` |
|   746005 |  100 | `	for(;;){` |
|  1492015 |  101 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
|   954043 |  102 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
|   817163 |  103 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
|   709353 |  104 | `		if( !SLen ){ return 0; }if( !*p \|\| !*q \|\| SyCharToLower(*p) != SyCharToLower(*q) ){ break; }p++;q++;--SLen;` |
|        - |  105 |  |
|        5 |  106 | `	}` |
|   444805 |  107 | `	return (sxi32)(SyCharToLower(p[0]) - SyCharToLower(q[0]));` |
|   423294 |  108 | `}` |
...
|        5 |  112 | `}` |
|  3245660 |  113 | `PH7_PRIVATE sxu32 Systrcpy(char *zDest,sxu32 nDestLen,const char *zSrc,sxu32 nLen)` |
|        5 |  114 | `{` |
|  3245665 |  115 | `	unsigned char *zBuf = (unsigned char *)zDest;` |
|  3245665 |  116 | `	unsigned char *zIn = (unsigned char *)zSrc;` |
...
|        - |  121 | `	}` |
|        - |  122 | `#endif` |
|  3245665 |  123 | `	if( nLen <= 0 ){` |
|      ! 0 |  124 | `		nLen = SyStrlen(zSrc);` |
|      ! 0 |  125 | `	}` |
|  3245665 |  126 | `	zEnd = &zBuf[nDestLen - 1]; /* reserve a room for the null terminator */` |
|  5682476 |  127 | `	for(;;){` |
| 11364720 |  128 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
| 10519211 |  129 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
|  9712730 |  130 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
|  8921126 |  131 | `		if( zBuf >= zEnd \|\| nLen == 0 ){ break;} zBuf[0] = zIn[0]; zIn++; zBuf++; nLen--;` |
|        5 |  132 | `	}` |
|  3245665 |  133 | `	zBuf[0] = 0;` |
|  3245665 |  134 | `	return (sxu32)(zBuf-(unsigned char *)zDest);` |
|        5 |  135 | `}` |
|        - |  136 |  |
