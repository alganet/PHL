...
|    - |  139 | ` */` |
| 3066 |  140 | `static const char * PHL_ResolveBinaryPath(const char *zArgv0)` |
|    5 |  141 |  |
|    - |  142 | `#ifdef __WINNT__` |
...
|    - |  146 | `	}` |
|    - |  147 | `#else` |
| 3066 |  148 | `	if( realpath(zArgv0,zPhlBinaryPath) != 0 ){` |
| 3066 |  149 | `		return zPhlBinaryPath;` |
|    - |  150 | `	}` |
|    - |  151 | `#endif` |
|  ! 0 |  152 | `	return zArgv0;` |
| 1538 |  153 |  |
...
|    - |  162 | ` */` |
| 9512 |  163 | `static int Output_Consumer(const void *pOutput,unsigned int nOutputLen,void *pUserData /* Unused */)` |
|    5 |  164 |  |
| 4756 |  165 | `	(void)pUserData;` |
|    - |  166 | `#ifdef __WINNT__` |
|    - |  167 | `	BOOL rc;` |
...
|    - |  173 | `#else` |
|    - |  174 | `	ssize_t nWr;` |
| 9512 |  175 | `	nWr = write(STDOUT_FILENO,pOutput,nOutputLen);` |
| 9512 |  176 | `	if( nWr < 0 ){` |
...
| 9517 |  182 | `	return PH7_OK;` |
| 4761 |  183 |  |
|    - |  184 | `/*` |
|    - |  185 | ` * Main program: Compile and execute the PHP file.` |
|    - |  186 | ` */` |
| 3557 |  187 | `int main(int argc,char **argv)` |
|    5 |  188 |  |
|    - |  189 | `	ph7 *pEngine; /* PH7 engine */` |
|    - |  190 | `	ph7_vm *pVm;  /* Compiled PHP program */` |
| 3562 |  191 | `	int dump_vm = 0;    /* Dump VM instructions if TRUE */` |
| 3562 |  192 | `	int run_code = 0;    /* Run inline code if TRUE */` |
| 3562 |  193 | `	int lint_mode = 0;   /* Syntax-check only (-l) if TRUE */` |
| 3562 |  194 | `	const char *zRunCode = 0; /* Inline code string */` |
|    - |  195 | `#ifdef PHL_ENABLE_SERVER` |
| 3562 |  196 | `	int server_mode = 0;        /* Start built-in server if TRUE */` |
| 3562 |  197 | `	const char *zServerAddr = 0; /* host:port string */` |
| 3562 |  198 | `	const char *zDocRoot = ".";  /* Document root */` |
|    - |  199 | `#endif` |
|    - |  200 | `	int n;              /* Script arguments */` |
|    - |  201 | `	int rc;` |
|    - |  202 | `	/* Process interpreter arguments first*/` |
| 3625 |  203 | `	for(n = 1 ; n < argc ; ++n ){` |
|    - |  204 | `		int c;` |
| 3455 |  205 | `		if( argv[n][0] != '-' ){` |
...
| 3223 |  306 | `	rc = ph7_init(&pEngine);` |
| 3223 |  307 | `	if( rc != PH7_OK ){` |
|    - |  308 | `		/*` |
...
|    - |  316 | `	 */` |
| 3223 |  317 | `	ph7_config(pEngine,PH7_CONFIG_ERR_OUTPUT,` |
...
|    - |  325 | `	{` |
| 3223 |  326 | `		const char *zMaxAlloc = getenv("PHL_MAX_ALLOC");` |
| 3223 |  327 | `		if( zMaxAlloc ){` |
...
|    - |  342 | `	{` |
| 3223 |  343 | `		const char *zMaxInput = getenv("PHL_MAX_INPUT");` |
| 3223 |  344 | `		if( zMaxInput ){` |
...
|  ! 0 |  396 | `		}` |
|    6 |  397 | `	}else{` |
| 3211 |  398 | `		rc = ph7_compile_file(` |
| 1518 |  399 | `			pEngine, /* PH7 Engine */` |
| 3206 |  400 | `			argv[n], /* Path to the PHP file to compile */` |
|    - |  401 | `			&pVm,    /* OUT: Compiled PHP program */` |
|    - |  402 |  |
|    - |  403 | `			);` |
| 3211 |  404 | `		if( rc != PH7_OK ){ /* Compile error */` |
...
| 4571 |  429 | `	ph7_create_constant(pVm,"PHP_BINARY",PHL_PhpBinaryConst,` |
| 3044 |  430 | `		(void *)PHL_ResolveBinaryPath(argv[0]));` |
...
|    - |  435 | `	 */` |
|    - |  436 | `	{` |
| 3049 |  437 | `		const char *zScriptName = run_code ? "Standard input code" : argv[n];` |
| 3049 |  438 | `		int argv_count = 0;` |
|    - |  439 | `		ph7_value *pArgc;` |
...
| 3049 |  443 | `		if( ph7_vm_config(pVm,PH7_VM_CONFIG_ARGV_ENTRY,zScriptName) == PH7_OK ){` |
| 3049 |  444 | `			argv_count++;` |
| 1522 |  445 | `		}` |
...
| 3079 |  448 | `		for( n = run_code ? n : n + 1; n < argc ; ++n ){` |
|   35 |  449 | `			if( ph7_vm_config(pVm,PH7_VM_CONFIG_ARGV_ENTRY,argv[n]) == PH7_OK ){` |
|   33 |  450 | `				argv_count++;` |
|   14 |  451 | `			}` |
|   20 |  452 | `		}` |
|    - |  453 | `		/* $argc: a plain integer global equal to count($argv). */` |
| 3049 |  454 | `		pArgc = ph7_new_scalar(pVm);` |
| 3049 |  455 | `		if( pArgc ){` |
| 3049 |  456 | `			ph7_value_int(pArgc,argv_count);` |
| 3049 |  457 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_CREATE_VAR,"argc",pArgc);` |
| 3049 |  458 | `			ph7_release_value(pVm,pArgc);` |
| 1522 |  459 | `		}` |
...
| 3049 |  462 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"SCRIPT_NAME",zScriptName,-1);` |
| 3049 |  463 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"PHP_SELF",zScriptName,-1);` |
| 3049 |  464 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"DOCUMENT_ROOT","",0);` |
|    - |  465 | `		{` |
|    - |  466 | `			char zTime[32];` |
| 3049 |  467 | `			snprintf(zTime,sizeof(zTime),"%ld",(long)time(0));` |
| 3049 |  468 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"REQUEST_TIME",zTime,-1);` |
|    - |  469 | `		}` |
|    - |  470 | `#ifndef __WINNT__` |
|    - |  471 | `		{` |
|    - |  472 | `			char zCwd[PATH_MAX];` |
| 3044 |  473 | `			if( getcwd(zCwd,sizeof(zCwd)) ){` |
| 3044 |  474 | `				ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"PWD",zCwd,-1);` |
| 1522 |  475 | `			}` |
|    - |  476 | `		}` |
|    - |  477 | `#endif` |
|    - |  478 | `	}` |
|    - |  479 | `	/* Report script run-time errors (now default behavior) */` |
| 3049 |  480 | `	ph7_vm_config(pVm,PH7_VM_CONFIG_ERR_REPORT);` |
| 3049 |  481 | `	if( dump_vm ){` |
...
|    - |  491 | `	 */` |
|    - |  492 | `	{` |
| 3049 |  493 | `		int iExitStatus = 0;` |
| 3049 |  494 | `		ph7_vm_exec(pVm,&iExitStatus);` |
|    - |  495 | `		/* All done, cleanup the mess left behind.` |
|    - |  496 | `		*/` |
| 3049 |  497 | `		ph7_vm_release(pVm);` |
| 3049 |  498 | `		ph7_release(pEngine);` |
|    - |  499 | `		/* Propagate the script exit status (set via exit()/die()) */` |
| 3049 |  500 | `		return iExitStatus;` |
|    - |  501 | `	}` |
| 1540 |  502 |  |
|    - |  503 |  |
