...
|     - |  172 | ` */` |
| 11870 |  173 | `static int Output_Consumer(const void *pOutput,unsigned int nOutputLen,void *pUserData /* Unused */)` |
|     5 |  174 | `{` |
|  5935 |  175 | `	(void)pUserData;` |
|     - |  176 | `#ifdef __WINNT__` |
|     - |  177 | `	BOOL rc;` |
...
|     - |  183 | `#else` |
|     - |  184 | `	ssize_t nWr;` |
| 11870 |  185 | `	nWr = write(STDOUT_FILENO,pOutput,nOutputLen);` |
| 11870 |  186 | `	if( nWr < 0 ){` |
...
|     - |  202 | ` */` |
| 14616 |  203 | `static int PHL_EnvULong(const char *zName,unsigned long uFloor,unsigned long uCeil,unsigned long *pOut)` |
|     5 |  204 | `{` |
| 14621 |  205 | `	const char *zVal = getenv(zName);` |
| 14621 |  206 | `	char *zEnd = 0;` |
|     - |  207 | `	unsigned long uMax;` |
| 14621 |  208 | `	if( zVal == 0 \|\| zVal[0] == 0 ){` |
| 14607 |  209 | `		return 0;` |
|     - |  210 | `	}` |
...
|     - |  232 | ` */` |
|  4027 |  233 | `int main(int argc,char **argv)` |
|     5 |  234 | `{` |
|     - |  235 | `	ph7 *pEngine; /* PH7 engine */` |
|     - |  236 | `	ph7_vm *pVm;  /* Compiled PHP program */` |
|  4032 |  237 | `	int dump_vm = 0;    /* Dump VM instructions if TRUE */` |
|  4032 |  238 | `	int run_code = 0;    /* Run inline code if TRUE */` |
|  4032 |  239 | `	int lint_mode = 0;   /* Syntax-check only (-l) if TRUE */` |
|  4032 |  240 | `	const char *zRunCode = 0; /* Inline code string */` |
|     - |  241 | `#ifdef PHL_ENABLE_SERVER` |
|  4032 |  242 | `	int server_mode = 0;        /* Start built-in server if TRUE */` |
|  4032 |  243 | `	const char *zServerAddr = 0; /* host:port string */` |
|  4032 |  244 | `	const char *zDocRoot = ".";  /* Document root */` |
|     - |  245 | `#endif` |
|     - |  246 | `	int n;              /* Script arguments */` |
|     - |  247 | `	int rc;` |
|     - |  248 | `	/* Process interpreter arguments first*/` |
|  4095 |  249 | `	for(n = 1 ; n < argc ; ++n ){` |
|     - |  250 | `		int c;` |
|  3909 |  251 | `		if( argv[n][0] != '-' ){` |
...
|  3661 |  352 | `	rc = ph7_init(&pEngine);` |
|  3661 |  353 | `	if( rc != PH7_OK ){` |
|     - |  354 | `		/*` |
...
|     - |  362 | `	 */` |
|  3661 |  363 | `	ph7_config(pEngine,PH7_CONFIG_ERR_OUTPUT,` |
...
|  3661 |  374 | `		if( PHL_EnvULong("PHL_MAX_ALLOC",65536UL,0xFFFFFFFFUL,&uMax) ){` |
...
|  3661 |  382 | `		if( PHL_EnvULong("PHL_MAX_INPUT",1UL,0xFFFFFFFFUL,&uMax) ){` |
...
|   ! 0 |  428 | `		}` |
|     6 |  429 | `	}else{` |
|  3649 |  430 | `		rc = ph7_compile_file(` |
|  1729 |  431 | `			pEngine, /* PH7 Engine */` |
|  3644 |  432 | `			argv[n], /* Path to the PHP file to compile */` |
|     - |  433 | `			&pVm,    /* OUT: Compiled PHP program */` |
|     - |  434 | `			0        /* IN: Compile flags */` |
|     - |  435 | `			);` |
|  3649 |  436 | `		if( rc != PH7_OK ){ /* Compile error */` |
...
|  3471 |  470 | `		if( PHL_EnvULong("PHL_MAX_RECURSION",1UL,0x7FFFFFFFUL,&uMax) ){` |
|     5 |  471 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_RECURSION_DEPTH,(int)uMax);` |
|     2 |  472 | `		}` |
|  3471 |  473 | `		if( PHL_EnvULong("PHL_MAX_NATIVE_DEPTH",2UL,0x7FFFFFFFUL,&uMax) ){` |
|    12 |  474 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_NATIVE_DEPTH,(int)uMax);` |
|     5 |  475 | `		}` |
|     - |  476 | `	}` |
|     - |  477 | `	/* Define PHP_BINARY: absolute path of this interpreter */` |
|  5204 |  478 | `	ph7_create_constant(pVm,"PHP_BINARY",PHL_PhpBinaryConst,` |
|  3466 |  479 | `		(void *)PHL_ResolveBinaryPath(argv[0]));` |
...
|     - |  484 | `	 */` |
|     - |  485 | `	{` |
|  3471 |  486 | `		const char *zScriptName = run_code ? "Standard input code" : argv[n];` |
|  3471 |  487 | `		int argv_count = 0;` |
|     - |  488 | `		ph7_value *pArgc;` |
...
|  3471 |  492 | `		if( ph7_vm_config(pVm,PH7_VM_CONFIG_ARGV_ENTRY,zScriptName) == PH7_OK ){` |
|  3471 |  493 | `			argv_count++;` |
|  1733 |  494 | `		}` |
...
|  3501 |  497 | `		for( n = run_code ? n : n + 1; n < argc ; ++n ){` |
|    35 |  498 | `			if( ph7_vm_config(pVm,PH7_VM_CONFIG_ARGV_ENTRY,argv[n]) == PH7_OK ){` |
|    33 |  499 | `				argv_count++;` |
|    14 |  500 | `			}` |
|    20 |  501 | `		}` |
|     - |  502 | `		/* $argc: a plain integer global equal to count($argv). */` |
|  3471 |  503 | `		pArgc = ph7_new_scalar(pVm);` |
|  3471 |  504 | `		if( pArgc ){` |
|  3471 |  505 | `			ph7_value_int(pArgc,argv_count);` |
|  3471 |  506 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_CREATE_VAR,"argc",pArgc);` |
|  3471 |  507 | `			ph7_release_value(pVm,pArgc);` |
|  1733 |  508 | `		}` |
...
|  3471 |  511 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"SCRIPT_NAME",zScriptName,-1);` |
|  3471 |  512 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"PHP_SELF",zScriptName,-1);` |
|  3471 |  513 | `		ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"DOCUMENT_ROOT","",0);` |
|     - |  514 | `		{` |
|     - |  515 | `			char zTime[32];` |
|  3471 |  516 | `			snprintf(zTime,sizeof(zTime),"%ld",(long)time(0));` |
|  3471 |  517 | `			ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"REQUEST_TIME",zTime,-1);` |
|     - |  518 | `		}` |
|     - |  519 | `#ifndef __WINNT__` |
|     - |  520 | `		{` |
|     - |  521 | `			char zCwd[PATH_MAX];` |
|  3466 |  522 | `			if( getcwd(zCwd,sizeof(zCwd)) ){` |
|  3466 |  523 | `				ph7_vm_config(pVm,PH7_VM_CONFIG_SERVER_ATTR,"PWD",zCwd,-1);` |
|  1733 |  524 | `			}` |
|     - |  525 | `		}` |
|     - |  526 | `#endif` |
|     - |  527 | `	}` |
|     - |  528 | `	/* Report script run-time errors (now default behavior) */` |
|  3471 |  529 | `	ph7_vm_config(pVm,PH7_VM_CONFIG_ERR_REPORT);` |
|  3471 |  530 | `	if( dump_vm ){` |
...
|     - |  540 | `	 */` |
|     - |  541 | `	{` |
|  3471 |  542 | `		int iExitStatus = 0;` |
|  3471 |  543 | `		ph7_vm_exec(pVm,&iExitStatus);` |
|     - |  544 | `		/* All done, cleanup the mess left behind.` |
|     - |  545 | `		*/` |
|  3471 |  546 | `		ph7_vm_release(pVm);` |
|  3471 |  547 | `		ph7_release(pEngine);` |
|     - |  548 | `		/* Propagate the script exit status (set via exit()/die()) */` |
|  3471 |  549 | `		return iExitStatus;` |
|     - |  550 | `	}` |
|  1751 |  551 | `}` |
|     - |  552 |  |
