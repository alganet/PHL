...
|    - |   84 | ` */` |
| 1102 |   85 | `static sxi32 VmJsonEncode(` |
|    - |   86 | `	ph7_value *pIn,          /* Encode this value */` |
|    - |   87 | `	json_private_data *pData /* Context data */` |
|    2 |   88 | `	){` |
| 1104 |   89 | `		ph7_context *pCtx = pData->pCtx;` |
| 1104 |   90 | `		int iFlags = pData->iFlags;` |
|    - |   91 | `		int nByte;` |
| 1104 |   92 | `		if( ph7_value_is_null(pIn) \|\| ph7_value_is_resource(pIn)){` |
|    - |   93 | `			/* null */` |
|    7 |   94 | `			JSON_EMIT(pData,ph7_result_string(pCtx,"null",(int)sizeof("null")-1));` |
| 1100 |   95 | `		}else if( ph7_value_is_bool(pIn) ){` |
...
| 1090 |  101 | `		}else if(  ph7_value_is_numeric(pIn) && !ph7_value_is_string(pIn) ){` |
|  529 |  102 | `			if( ph7_value_is_float(pIn) ){` |
...
|  337 |  112 | `				zNum = ph7_value_to_string(pIn,&nByte);` |
|  337 |  113 | `				JSON_EMIT(pData,ph7_result_string(pCtx,zNum,nByte));` |
|    1 |  114 | `			}` |
|  908 |  115 | `		}else if( ph7_value_is_string(pIn) ){` |
|  360 |  116 | `			if( (iFlags & JSON_NUMERIC_CHECK) &&  ph7_value_is_numeric(pIn) ){` |
...
|  356 |  124 | `				zIn = ph7_value_to_string(pIn,&nByte);` |
|  356 |  125 | `				zEnd = &zIn[nByte];` |
|    - |  126 | `				/* Append the double quote */` |
|  356 |  127 | `				JSON_EMIT(pData,ph7_result_string(pCtx,"\"",(int)sizeof(char)));` |
|  663 |  128 | `				for(;;){` |
| 1334 |  129 | `					if( zIn >= zEnd ){` |
...
|  980 |  135 | `					zIn++;` |
|  980 |  136 | `					if( (c == '<' \|\| c == '>') && (iFlags & JSON_HEX_TAG) ){` |
...
|  974 |  186 | `					JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&c,(int)sizeof(char)));` |
|    2 |  187 | `				}` |
|    - |  188 | `				/* Append the double quote */` |
|  356 |  189 | `				JSON_EMIT(pData,ph7_result_string(pCtx,"\"",(int)sizeof(char)));` |
|    2 |  190 | `			}` |
|  553 |  191 | `		}else if( ph7_value_is_array(pIn) ){` |
...
|  608 |  195 | `			int isObject = (iFlags & JSON_FORCE_OBJECT)` |
|  304 |  196 | `				\|\| !PH7_HashmapIsList((ph7_hashmap *)pIn->x.pOther);` |
|  305 |  197 | `			int savedObject = pData->isObject; /* restore for sibling entries after recursion */` |
|  305 |  198 | `			int c = isObject ? '{' : '[';` |
|  305 |  199 | `			int d = isObject ? '}' : ']';` |
|    - |  200 | `			/* Encode the array */` |
|  305 |  201 | `			pData->isObject = isObject;` |
|  305 |  202 | `			pData->isFirst = 1;` |
|    - |  203 | `			/* Append the square bracket or curly braces */` |
|  305 |  204 | `			JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&c,(int)sizeof(char)));` |
|    - |  205 | `			/* Iterate throw array entries */` |
|  305 |  206 | `			ph7_array_walk(pIn,VmJsonArrayEncode,pData);` |
|    - |  207 | `			/* Bail if a nested append ran out of memory before the closer */` |
|  305 |  208 | `			if( pData->oom ){` |
|  ! 0 |  209 | `				return PH7_OK;` |
|    - |  210 | `			}` |
|    - |  211 | `			/* Append the closing square bracket or curly braces */` |
|  305 |  212 | `			JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&d,(int)sizeof(char)));` |
|  305 |  213 | `			pData->isObject = savedObject;` |
|  222 |  214 | `		}else if( ph7_value_is_object(pIn) ){` |
...
|   68 |  221 | `			if( pVm->pJsonSerializableClass` |
|   70 |  222 | `				&& PH7_VmInstanceOf(pThis->pClass,pVm->pJsonSerializableClass) ){` |
|   17 |  223 | `				pMethod = PH7_ClassExtractMethod(pThis->pClass,"jsonSerialize",sizeof("jsonSerialize")-1);` |
|    8 |  224 | `			}` |
|   70 |  225 | `			if( pMethod == 0 && (pThis->pClass->iFlags & PH7_CLASS_ENUM) != 0 ){` |
...
|    - |  241 | `				sxi32 rc;` |
|   17 |  242 | `				PH7_MemObjInit(pVm,&sResult);` |
|   17 |  243 | `				rc = PH7_VmCallClassMethod(pVm,pThis,pMethod,&sResult,0,0);` |
|   17 |  244 | `				if( rc == PH7_EXCEPTION ){` |
...
|   13 |  251 | `				pData->nRecCount++;` |
|   13 |  252 | `				VmJsonEncode(&sResult,pData);` |
|   13 |  253 | `				pData->nRecCount--;` |
|   13 |  254 | `				PH7_MemObjRelease(&sResult);` |
|   13 |  255 | `				if( pData->exc ){` |
...
|    - |  343 | `		}` |
|    - |  344 | `		/* All done */` |
| 1092 |  345 | `		return PH7_OK;` |
|  553 |  346 | `}` |
|    - |  347 | `/*` |
|    - |  348 | ` * The following walker callback is invoked each time we need` |
|    - |  349 | ` * to encode an array to JSON.` |
|    - |  350 | ` */` |
|  688 |  351 | `static int VmJsonArrayEncode(ph7_value *pKey,ph7_value *pValue,void *pUserData)` |
|    1 |  352 | `{` |
|  689 |  353 | `	json_private_data *pJson = (json_private_data *)pUserData;` |
|  689 |  354 | `	if( pJson->nRecCount > 31 \|\| pJson->exc \|\| pJson->oom ){` |
|    - |  355 | `		/* Recursion limit reached, a callback threw, or OOM — return immediately */` |
|  ! 0 |  356 | `		return PH7_OK;` |
|    - |  357 | `	}` |
|  689 |  358 | `	if( !pJson->isFirst ){` |
|    - |  359 | `		/* Append the colon first */` |
|  415 |  360 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,",",(int)sizeof(char)));` |
|  207 |  361 | `	}` |
|  689 |  362 | `	if( pJson->isObject ){` |
|    - |  363 | `		/* Outputs an object rather than an array */` |
|    - |  364 | `		const char *zKey;` |
|    - |  365 | `		int nByte;` |
|    - |  366 | `		/* Extract a string representation of the key */` |
|  343 |  367 | `		zKey = ph7_value_to_string(pKey,&nByte);` |
...
|  343 |  371 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,"\"",(int)sizeof(char)));` |
|  343 |  372 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,zKey,nByte));` |
|  343 |  373 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,"\":",(int)sizeof("\":")-1));` |
|  171 |  374 | `	}` |
|    - |  375 | `	/* Encode the value */` |
|  689 |  376 | `	pJson->nRecCount++;` |
|  689 |  377 | `	VmJsonEncode(pValue,pJson);` |
|  689 |  378 | `	pJson->nRecCount--;` |
|  689 |  379 | `	pJson->isFirst = 0;` |
|  689 |  380 | `	return PH7_OK;` |
|  345 |  381 | `}` |
|    - |  382 | `/*` |
...
|    - |  429 | ` */` |
|  322 |  430 | `PH7_PRIVATE int vm_builtin_json_encode(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    2 |  431 | `{` |
...
|  324 |  440 | `	sJson.nRecCount = 0;` |
|  324 |  441 | `	sJson.pCtx = pCtx;` |
|  324 |  442 | `	sJson.isFirst = 1;` |
|  324 |  443 | `	sJson.iFlags = 0;` |
|  324 |  444 | `	sJson.exc = 0;` |
|  324 |  445 | `	sJson.oom = 0;` |
|  324 |  446 | `	sJson.fail = 0;` |
|  324 |  447 | `	if( nArg > 1 && ph7_value_is_int(apArg[1]) ){` |
|    - |  448 | `		/* Extract option flags */` |
|    7 |  449 | `		sJson.iFlags = ph7_value_to_int(apArg[1]);` |
|    3 |  450 | `	}` |
|  324 |  451 | `	pCtx->pVm->json_rc = JSON_ERROR_NONE;` |
|    - |  452 | `	/* Perform the encoding operation */` |
|  324 |  453 | `	rc = VmJsonEncode(apArg[0],&sJson);` |
|  324 |  454 | `	if( sJson.oom ){` |
