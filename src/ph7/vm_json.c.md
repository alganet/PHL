...
|    - |   82 | ` */` |
|  808 |   83 | `static sxi32 VmJsonEncode(` |
|    - |   84 | `	ph7_value *pIn,          /* Encode this value */` |
|    - |   85 | `	json_private_data *pData /* Context data */` |
|    2 |   86 | `	){` |
|  810 |   87 | `		ph7_context *pCtx = pData->pCtx;` |
|  810 |   88 | `		int iFlags = pData->iFlags;` |
|    - |   89 | `		int nByte;` |
|  810 |   90 | `		if( ph7_value_is_null(pIn) \|\| ph7_value_is_resource(pIn)){` |
|    - |   91 | `			/* null */` |
|    5 |   92 | `			JSON_EMIT(pData,ph7_result_string(pCtx,"null",(int)sizeof("null")-1));` |
|  807 |   93 | `		}else if( ph7_value_is_bool(pIn) ){` |
...
|  798 |   99 | `		}else if(  ph7_value_is_numeric(pIn) && !ph7_value_is_string(pIn) ){` |
|  400 |  100 | `			if( ph7_value_is_float(pIn) ){` |
...
|  251 |  110 | `				zNum = ph7_value_to_string(pIn,&nByte);` |
|  251 |  111 | `				JSON_EMIT(pData,ph7_result_string(pCtx,zNum,nByte));` |
|    1 |  112 | `			}` |
|  659 |  113 | `		}else if( ph7_value_is_string(pIn) ){` |
|  258 |  114 | `			if( (iFlags & JSON_NUMERIC_CHECK) &&  ph7_value_is_numeric(pIn) ){` |
...
|  254 |  122 | `				zIn = ph7_value_to_string(pIn,&nByte);` |
|  254 |  123 | `				zEnd = &zIn[nByte];` |
|    - |  124 | `				/* Append the double quote */` |
|  254 |  125 | `				JSON_EMIT(pData,ph7_result_string(pCtx,"\"",(int)sizeof(char)));` |
|  477 |  126 | `				for(;;){` |
|  962 |  127 | `					if( zIn >= zEnd ){` |
...
|  710 |  133 | `					zIn++;` |
|  710 |  134 | `					if( (c == '<' \|\| c == '>') && (iFlags & JSON_HEX_TAG) ){` |
...
|  704 |  184 | `					JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&c,(int)sizeof(char)));` |
|    2 |  185 | `				}` |
|    - |  186 | `				/* Append the double quote */` |
|  254 |  187 | `				JSON_EMIT(pData,ph7_result_string(pCtx,"\"",(int)sizeof(char)));` |
|    2 |  188 | `			}` |
|  398 |  189 | `		}else if( ph7_value_is_array(pIn) ){` |
...
|  428 |  193 | `			int isObject = (iFlags & JSON_FORCE_OBJECT)` |
|  214 |  194 | `				\|\| !PH7_HashmapIsList((ph7_hashmap *)pIn->x.pOther);` |
|  215 |  195 | `			int savedObject = pData->isObject; /* restore for sibling entries after recursion */` |
|  215 |  196 | `			int c = isObject ? '{' : '[';` |
|  215 |  197 | `			int d = isObject ? '}' : ']';` |
|    - |  198 | `			/* Encode the array */` |
|  215 |  199 | `			pData->isObject = isObject;` |
|  215 |  200 | `			pData->isFirst = 1;` |
|    - |  201 | `			/* Append the square bracket or curly braces */` |
|  215 |  202 | `			JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&c,(int)sizeof(char)));` |
|    - |  203 | `			/* Iterate throw array entries */` |
|  215 |  204 | `			ph7_array_walk(pIn,VmJsonArrayEncode,pData);` |
|    - |  205 | `			/* Bail if a nested append ran out of memory before the closer */` |
|  215 |  206 | `			if( pData->oom ){` |
|  ! 0 |  207 | `				return PH7_OK;` |
|    - |  208 | `			}` |
|    - |  209 | `			/* Append the closing square bracket or curly braces */` |
|  215 |  210 | `			JSON_EMIT(pData,ph7_result_string(pCtx,(const char *)&d,(int)sizeof(char)));` |
|  215 |  211 | `			pData->isObject = savedObject;` |
|  163 |  212 | `		}else if( ph7_value_is_object(pIn) ){` |
...
|    - |  260 | `		}` |
|    - |  261 | `		/* All done */` |
|  806 |  262 | `		return PH7_OK;` |
|  406 |  263 | `}` |
|    - |  264 | `/*` |
|    - |  265 | ` * The following walker callback is invoked each time we need` |
|    - |  266 | ` * to encode an array to JSON.` |
|    - |  267 | ` */` |
|  492 |  268 | `static int VmJsonArrayEncode(ph7_value *pKey,ph7_value *pValue,void *pUserData)` |
|    1 |  269 | `{` |
|  493 |  270 | `	json_private_data *pJson = (json_private_data *)pUserData;` |
|  493 |  271 | `	if( pJson->nRecCount > 31 \|\| pJson->exc \|\| pJson->oom ){` |
|    - |  272 | `		/* Recursion limit reached, a callback threw, or OOM — return immediately */` |
|  ! 0 |  273 | `		return PH7_OK;` |
|    - |  274 | `	}` |
|  493 |  275 | `	if( !pJson->isFirst ){` |
|    - |  276 | `		/* Append the colon first */` |
|  303 |  277 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,",",(int)sizeof(char)));` |
|  151 |  278 | `	}` |
|  493 |  279 | `	if( pJson->isObject ){` |
|    - |  280 | `		/* Outputs an object rather than an array */` |
|    - |  281 | `		const char *zKey;` |
|    - |  282 | `		int nByte;` |
|    - |  283 | `		/* Extract a string representation of the key */` |
|  255 |  284 | `		zKey = ph7_value_to_string(pKey,&nByte);` |
...
|  255 |  288 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,"\"",(int)sizeof(char)));` |
|  255 |  289 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,zKey,nByte));` |
|  255 |  290 | `		JSON_EMIT(pJson,ph7_result_string(pJson->pCtx,"\":",(int)sizeof("\":")-1));` |
|  127 |  291 | `	}` |
|    - |  292 | `	/* Encode the value */` |
|  493 |  293 | `	pJson->nRecCount++;` |
|  493 |  294 | `	VmJsonEncode(pValue,pJson);` |
|  493 |  295 | `	pJson->nRecCount--;` |
|  493 |  296 | `	pJson->isFirst = 0;` |
|  493 |  297 | `	return PH7_OK;` |
|  247 |  298 | `}` |
|    - |  299 | `/*` |
...
|    - |  346 | ` */` |
|  244 |  347 | `PH7_PRIVATE int vm_builtin_json_encode(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    2 |  348 | `{` |
...
|  246 |  357 | `	sJson.nRecCount = 0;` |
|  246 |  358 | `	sJson.pCtx = pCtx;` |
|  246 |  359 | `	sJson.isFirst = 1;` |
|  246 |  360 | `	sJson.iFlags = 0;` |
|  246 |  361 | `	sJson.exc = 0;` |
|  246 |  362 | `	sJson.oom = 0;` |
|  246 |  363 | `	if( nArg > 1 && ph7_value_is_int(apArg[1]) ){` |
|    - |  364 | `		/* Extract option flags */` |
|    7 |  365 | `		sJson.iFlags = ph7_value_to_int(apArg[1]);` |
|    3 |  366 | `	}` |
|    - |  367 | `	/* Perform the encoding operation */` |
|  246 |  368 | `	rc = VmJsonEncode(apArg[0],&sJson);` |
|  246 |  369 | `	if( sJson.oom ){` |
