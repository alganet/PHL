...
|        - |  463 | `	};` |
|        - |  464 | `	sxu32 i;` |
|       32 |  465 | `	if( nName > 0 && zName[0] == '\\' ){` |
|      ! 0 |  466 | `		zName++;` |
|      ! 0 |  467 | `		nName--;` |
|      ! 0 |  468 | `	}` |
|       46 |  469 | `	for( i = 0; i < SX_ARRAYSIZE(aOk); i++ ){` |
|       46 |  470 | `		if( nName == aOk[i].nLen && SyStrnicmp(zName,aOk[i].zName,nName) == 0 ){` |
|       32 |  471 | `			return 1;` |
|        - |  472 | `		}` |
|        9 |  473 | `	}` |
|      ! 0 |  474 | `	return 0;` |
...
|        - |  495 | `	}` |
|       32 |  496 | `	if( GenStateGenRetNameOk(pName->zString,pName->nByte) ){` |
|       32 |  497 | `		return 1;` |
|        - |  498 | `	}` |
...
|      113 |  887 | `			pOut->nType = SXU32_HIGH;` |
|      113 |  888 | `			pOut->sClass = pIn->sData;` |
|       59 |  889 | `		}else{` |
...
|       37 | 1026 | `		sxu32 g, nGroups = 0;` |
|       37 | 1027 | `		int bFirstGroup = 1;` |
|       79 | 1028 | `		for( g = 0; g <= nMaxGroup; g++ ){ if( aGroupCount[g] > 0 ) nGroups++; }` |
|       79 | 1029 | `		for( g = 0; g <= nMaxGroup; g++ ){` |
|       47 | 1030 | `			int bFirstMember = 1;` |
|        - | 1031 | `			int bWrap;` |
|       47 | 1032 | `			if( aGroupCount[g] == 0 ) continue;` |
...
|       63 | 1037 | `			bWrap = (aGroupCount[g] >= 2 && (nGroups > 1 \|\| bNullable));` |
|       47 | 1038 | `			if( !bFirstGroup ) SyBlobAppend(pBlob, "\|", 1);` |
|       47 | 1039 | `			if( bWrap ) SyBlobAppend(pBlob, "(", 1);` |
|      151 | 1040 | `			for( i = 0; i < nAtoms; i++ ){` |
|      109 | 1041 | `				if( aAtoms[i].nType == UTA_NULL_FLAG \|\| aAtoms[i].nGroup != g ) continue;` |
|       79 | 1042 | `				if( !bFirstMember ) SyBlobAppend(pBlob, "&", 1);` |
|       79 | 1043 | `				if( aAtoms[i].nType == SXU32_HIGH ){` |
|       75 | 1044 | `					GenAppendClassAtom(pBlob, &aAtoms[i], 1);` |
|       40 | 1045 | `				}else{` |
|        6 | 1046 | `					SyBlobAppend(pBlob, aAtoms[i].zCanon, aAtoms[i].nCanon);` |
|        - | 1047 | `				}` |
|       79 | 1048 | `				bFirstMember = 0;` |
|       42 | 1049 | `			}` |
|       47 | 1050 | `			if( bWrap ) SyBlobAppend(pBlob, ")", 1);` |
|       47 | 1051 | `			bFirstGroup = 0;` |
|       26 | 1052 | `		}` |
...
|      109 | 1055 | `		for( i = 0; i < nAtoms; i++ ){` |
|       79 | 1056 | `			if( GenAtomMaskKind(&aAtoms[i]) == GEN_ATOM_ITERABLE ){` |
|        3 | 1057 | `				SyBlobAppend(pBlob, "\|", 1);` |
|        3 | 1058 | `				SyBlobAppend(pBlob, "array", sizeof("array")-1);` |
|        3 | 1059 | `				break;` |
|        - | 1060 | `			}` |
|       41 | 1061 | `		}` |
|       37 | 1062 | `		if( bNullable ){` |
...
|        - | 1289 | `			}` |
|      265 | 1290 | ``			pGen->pIn++; /* skip `\|` */`` |
|      265 | 1291 | `			rc = GenStateParsePart(pGen, aAtoms, &nAtoms, ++iGroup, &nMembers, &bParen, nLine);` |
|      265 | 1292 | `			if( rc != SXRET_OK ){` |
|      ! 0 | 1293 | `				return rc;` |
|        - | 1294 | `			}` |
|        5 | 1295 | `		}` |
...
|       64 | 1337 | `					if( (pC->nByte == 8 && SyMemcmpNoCase(pC->zString,"iterable",8) == 0)` |
|       64 | 1338 | `					 \|\| (pC->nByte == 8 && SyMemcmpNoCase(pC->zString,"callable",8) == 0)` |
|       64 | 1339 | `					 \|\| (pC->nByte == 4 && SyMemcmpNoCase(pC->zString,"true",4) == 0)` |
|       69 | 1340 | `					 \|\| (pC->nByte == 5 && SyMemcmpNoCase(pC->zString,"false",5) == 0) ){` |
|      ! 0 | 1341 | `						bClassLike = 0;` |
|      ! 0 | 1342 | `					}` |
|       32 | 1343 | `				}` |
|       71 | 1344 | `				if( !bClassLike ){` |
...
|      352 | 1407 | `				                   && aGroupCount[aAtoms[j].nGroup] == 1);` |
|      357 | 1408 | `				if( !bSameGroup && !bBothSingleton ) continue;` |
|      337 | 1409 | `				if( aAtoms[i].nType == aAtoms[j].nType ){` |
|       91 | 1410 | `					if( aAtoms[i].nType == SXU32_HIGH ){` |
|       84 | 1411 | `						if( aAtoms[i].sClass.nByte == aAtoms[j].sClass.nByte` |
|       69 | 1412 | `						 && SyMemcmpNoCase(aAtoms[i].sClass.zString,` |
|       22 | 1413 | `								aAtoms[j].sClass.zString,` |
|       44 | 1414 | `								aAtoms[i].sClass.nByte) == 0 ){` |
|      ! 0 | 1415 | `							bDup = 1;` |
|      ! 0 | 1416 | `						}` |
|       47 | 1417 | `					}else{` |
|        3 | 1418 | `						bDup = 1;` |
...
|      495 | 1506 | `				if( aAtoms[i].nType == UTA_NULL_FLAG ) continue;` |
|      483 | 1507 | `				SyZero(&sAlt, sizeof(sAlt));` |
|      483 | 1508 | `				sAlt.nGroup = aAtoms[i].nGroup; /* preserve intersection grouping */` |
|      483 | 1509 | `				if( aAtoms[i].nType == SXU32_HIGH ){` |
|      335 | 1510 | `					char *zDup = SyMemBackendStrDup(&pGen->pVm->sAllocator,` |
|      110 | 1511 | `						aAtoms[i].sClass.zString, aAtoms[i].sClass.nByte);` |
|      225 | 1512 | `					if( zDup == 0 ) return SXERR_ABORT;` |
|      225 | 1513 | `					sAlt.nType = SXU32_HIGH;` |
|      225 | 1514 | `					SyStringInitFromBuf(&sAlt.sClass, zDup, aAtoms[i].sClass.nByte);` |
|      115 | 1515 | `				}else{` |
|      263 | 1516 | `					sAlt.nType = aAtoms[i].nType;` |
|      263 | 1517 | `					SyStringInitFromBuf(&sAlt.sClass, 0, 0);` |
|        - | 1518 | `				}` |
|      483 | 1519 | `				SySetPut(pAlts, (const void *)&sAlt);` |
|      244 | 1520 | `			}` |
|        - | 1521 | `		}` |
|        - | 1522 | `	}` |
