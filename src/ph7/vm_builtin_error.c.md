...
|    5 |  132 | `{` |
|   43 |  133 | `	int nErr = 1024; /* E_USER_NOTICE — php's default level */` |
|   43 |  134 | `	if( nArg > 0 ){` |
|    - |  135 | `		const char *zErr;` |
|    - |  136 | `		int nLen;` |
|    - |  137 | `		/* Extract the error message */` |
|   43 |  138 | `		zErr = ph7_value_to_string(apArg[0],&nLen);` |
|   43 |  139 | `		if( nArg > 1 ){` |
...
|   43 |  144 | `			nErr = ph7_value_to_int(apArg[1]);` |
|   43 |  145 | `			if( nErr != 256 && nErr != 512 && nErr != 1024 && nErr != 16384 ){` |
...
|    - |  148 | `			}` |
|   43 |  149 | `			if( nErr == 256 /* E_USER_ERROR */ ){` |
...
|   41 |  156 | `		PH7_VmThrowError(pCtx->pVm, NULL, nErr, zErr);` |
|   36 |  157 | `		if( nErr == 256 /* E_USER_ERROR */` |
|   23 |  158 | `		 && !ph7_value_is_callable(&pCtx->pVm->aErrCB[1]) ){` |
...
|    - |  190 | ` */` |
|   96 |  191 | `PH7_PRIVATE int vm_builtin_error_reporting(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    4 |  192 | `{` |
|  100 |  193 | `	ph7_vm *pVm = pCtx->pVm;` |
|    - |  194 | `	int nOld;` |
|    - |  195 | `	/* Extract the old reporting level */` |
|  100 |  196 | `	nOld = pVm->bErrReport ? (int)pVm->iErrMask : 0;` |
|  100 |  197 | `	if( pVm->nErrSuppress > 0 ){` |
...
|   34 |  208 | `		nNew = ph7_value_to_int(apArg[0]);` |
|   34 |  209 | `		pVm->iErrMask = (sxi32)nNew;` |
|   34 |  210 | `		pVm->bErrReport = nNew != 0;` |
|   15 |  211 | `	}` |
|    - |  212 | `	/* Return the old level */` |
|  100 |  213 | `	ph7_result_int(pCtx,nOld);` |
|  100 |  214 | `	return PH7_OK;` |
|    4 |  215 | `}` |
|    - |  216 | `/*` |
...
|    - |  357 | ` *  Always TRUE.` |
|    - |  358 | ` */` |
|   64 |  359 | `PH7_PRIVATE int vm_builtin_restore_error_handler(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    5 |  360 | `{` |
|   69 |  361 | `	ph7_vm *pVm = pCtx->pVm;` |
|    - |  362 | `	ph7_value *pOld,*pNew;` |
|    - |  363 | `	/* Point to the old and the new handler */` |
|   69 |  364 | `	pOld = &pVm->aErrCB[0];` |
|   69 |  365 | `	pNew = &pVm->aErrCB[1];` |
|   32 |  366 | `	SXUNUSED(nArg); /* cc warning */` |
|   32 |  367 | `	SXUNUSED(apArg);` |
|   69 |  368 | `	if( pOld->iFlags & MEMOBJ_NULL ){` |
...
|   22 |  375 | `		PH7_MemObjRelease(pNew);` |
|   22 |  376 | `		MemObjSetType(pNew,MEMOBJ_NULL);` |
|   22 |  377 | `		ph7_result_bool(pCtx,1);` |
|   22 |  378 | `		return PH7_OK;` |
|    - |  379 | `	}` |
|    - |  380 | `	/* Copy the old callback */` |
|   47 |  381 | `	PH7_MemObjStore(pOld,pNew);` |
|   47 |  382 | `	PH7_MemObjRelease(pOld);` |
|    - |  383 | `	/* Return TRUE */` |
|   47 |  384 | `	ph7_result_bool(pCtx,1);` |
|   47 |  385 | `	return PH7_OK;` |
|   37 |  386 | `}` |
|    - |  387 | `/*` |
...
|    - |  419 | ` */` |
| 9982 |  420 | `PH7_PRIVATE int vm_builtin_set_error_handler(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    5 |  421 | `{` |
| 9987 |  422 | `	ph7_vm *pVm = pCtx->pVm;` |
|    - |  423 | `	ph7_value *pOld,*pNew;` |
|    - |  424 | `	/* Point to the old and the new handler */` |
| 9987 |  425 | `	pOld = &pVm->aErrCB[0];` |
| 9987 |  426 | `	pNew = &pVm->aErrCB[1];` |
|    - |  427 | `	/* Return the old handler */` |
| 9987 |  428 | `	ph7_result_value(pCtx,pOld); /* Will make it's own copy */` |
| 9987 |  429 | `	if( nArg > 0 ){` |
| 9987 |  430 | `		if( !ph7_value_is_callable(apArg[0])) {` |
|    - |  431 | `			/* Not callable,return TRUE (As requested by the PHP specification) */` |
| 4949 |  432 | `			PH7_MemObjRelease(pNew);` |
| 4949 |  433 | `			ph7_result_bool(pCtx,1);` |
| 2475 |  434 | `		}else{` |
| 5039 |  435 | `			PH7_MemObjStore(pNew,pOld);` |
|    - |  436 | `			/* Install the new handler */` |
| 5039 |  437 | `			PH7_MemObjStore(apArg[0],pNew);` |
|    - |  438 | `		}` |
| 4991 |  439 | `	}` |
| 9987 |  440 | `	return PH7_OK;` |
|    5 |  441 | `}` |
|    - |  442 | `/*` |
...
|    2 |  502 | `{` |
|   12 |  503 | `	ph7_vm *pVm = pCtx->pVm;` |
|    - |  504 | `	ph7_value *pArray,*pValue;` |
|    5 |  505 | `	SXUNUSED(nArg);` |
|    5 |  506 | `	SXUNUSED(apArg);` |
|   12 |  507 | `	if( pVm->nLastErrType == 0 ){` |
|    - |  508 | `		/* No error yet */` |
|    8 |  509 | `		ph7_result_null(pCtx);` |
|    8 |  510 | `		return PH7_OK;` |
|    - |  511 | `	}` |
|    5 |  512 | `	pArray = ph7_context_new_array(pCtx);` |
|    5 |  513 | `	pValue = ph7_context_new_scalar(pCtx);` |
|    5 |  514 | `	if( pArray == 0 \|\| pValue == 0 ){` |
|  ! 0 |  515 | `		ph7_context_throw_error(pCtx,PH7_CTX_ERR,"PH7 is running out of memory");` |
|  ! 0 |  516 | `		ph7_result_null(pCtx);` |
|  ! 0 |  517 | `		return PH7_OK;` |
|    - |  518 | `	}` |
|    5 |  519 | `	ph7_value_int(pValue,(int)pVm->nLastErrType);` |
|    5 |  520 | `	ph7_array_add_strkey_elem(pArray,"type",pValue);` |
|    7 |  521 | `	ph7_value_string(pValue,(const char *)SyBlobData(&pVm->sLastErrMsg),` |
|    4 |  522 | `		(int)SyBlobLength(&pVm->sLastErrMsg));` |
|    5 |  523 | `	ph7_array_add_strkey_elem(pArray,"message",pValue);` |
|    5 |  524 | `	ph7_value_reset_string_cursor(pValue);` |
|    7 |  525 | `	ph7_value_string(pValue,(const char *)SyBlobData(&pVm->sLastErrFile),` |
|    4 |  526 | `		(int)SyBlobLength(&pVm->sLastErrFile));` |
|    5 |  527 | `	ph7_array_add_strkey_elem(pArray,"file",pValue);` |
|    5 |  528 | `	ph7_value_reset_string_cursor(pValue);` |
|    5 |  529 | `	ph7_value_int(pValue,(int)pVm->nLastErrLine);` |
|    5 |  530 | `	ph7_array_add_strkey_elem(pArray,"line",pValue);` |
|    5 |  531 | `	ph7_result_value(pCtx,pArray);` |
|    5 |  532 | `	return PH7_OK;` |
|    7 |  533 | `}` |
|    - |  534 | `/*` |
...
|    7 |  548 | `	return PH7_OK;` |
|    1 |  549 | `}` |
|   10 |  550 | `PH7_PRIVATE int vm_builtin_debug_backtrace(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|    1 |  551 | `{` |
|   11 |  552 | `	ph7_vm *pVm = pCtx->pVm;` |
|    - |  553 | `	ph7_value *pList;` |
...
|   11 |  556 | `	sxi32 iOptions = (nArg > 0 && apArg[0]) ? ph7_value_to_int(apArg[0]) : 1 /*PROVIDE_OBJECT*/;` |
...
|   11 |  561 | `	pList = ph7_context_new_array(pCtx);` |
|   11 |  562 | `	if( pList == 0 ){` |
...
|  ! 0 |  567 | `		return PH7_OK;` |
|    - |  568 | `	}` |
|   11 |  569 | `	VmBuildBacktrace(&(*pVm),iOptions,pList);` |
|    - |  570 | `	/* Return the freshly created list */` |
|   11 |  571 | `	ph7_result_value(pCtx,pList);` |
|    - |  572 | `	/*` |
