...
|     - |  405 | ` */` |
| 10036 |  406 | `PH7_PRIVATE int vm_builtin_set_error_handler(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|     5 |  407 | `{` |
| 10041 |  408 | `	ph7_vm *pVm = pCtx->pVm;` |
|     - |  409 | `	ph7_value *pOld,*pNew;` |
|     - |  410 | `	/* Point to the old and the new handler */` |
| 10041 |  411 | `	pOld = &pVm->aErrCB[0];` |
| 10041 |  412 | `	pNew = &pVm->aErrCB[1];` |
|     - |  413 | `	/* Return the old handler */` |
| 10041 |  414 | `	ph7_result_value(pCtx,pOld); /* Will make it's own copy */` |
| 10041 |  415 | `	if( nArg > 0 ){` |
| 10041 |  416 | `		if( !ph7_value_is_callable(apArg[0])) {` |
|     - |  417 | `			/* Not callable,return TRUE (As requested by the PHP specification) */` |
|  4993 |  418 | `			PH7_MemObjRelease(pNew);` |
|  4993 |  419 | `			ph7_result_bool(pCtx,1);` |
|  2497 |  420 | `		}else{` |
|  5049 |  421 | `			PH7_MemObjStore(pNew,pOld);` |
|     - |  422 | `			/* Install the new handler */` |
|  5049 |  423 | `			PH7_MemObjStore(apArg[0],pNew);` |
|     - |  424 | `		}` |
|  5018 |  425 | `	}` |
| 10041 |  426 | `	return PH7_OK;` |
|     5 |  427 | `}` |
|     - |  428 | `/*` |
