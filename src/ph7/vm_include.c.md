...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90502 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90507 |   32 | `	ProcConsumer xErr = 0;` |
|    90507 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90507 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90507 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90507 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90507 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     9091 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9091 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4544 |   45 | `	}` |
...
|    90507 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90507 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90503 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90507 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90507 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90507 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9091 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4544 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90507 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90507 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90507 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   135757 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90503 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9191 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2437320 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2423543 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2423543 |  104 | `				if( !pClass->bMounted ){` |
|   244121 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   244121 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4592 |  115 | `		}` |
|    90501 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90501 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     9091 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4547 |  126 | `		}else{` |
...
|    90501 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90501 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9189 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4592 |  139 | `		}` |
|    90501 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45251 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90507 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90507 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90507 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90507 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90507 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90507 |  151 | `	if( bNested ){` |
...
|        - |  209 | ` */` |
|     9100 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     9103 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20593491 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20584404 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10292196 |  221 | `	}` |
|     9089 |  222 | `	return FALSE;` |
|     4553 |  223 | `}` |
|        - |  224 | `/*` |
...
|    12977 |  285 | `	if( !bMain ){` |
|     9103 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     9089 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9089 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  313 | ` */` |
|     9106 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     9109 |  328 | `	pVm = pCtx->pVm;` |
|     9109 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9109 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     9109 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     9109 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9109 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     9103 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9103 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     9091 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9091 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     9091 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9091 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4544 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     9103 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     9103 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     9103 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     9066 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        3 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     9069 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     9069 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9069 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     9069 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9069 |  556 | `	if( rc != SXRET_OK ){` |
