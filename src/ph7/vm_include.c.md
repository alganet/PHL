...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90824 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90829 |   32 | `	ProcConsumer xErr = 0;` |
|    90829 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90829 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90829 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90829 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90829 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     9077 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9077 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4537 |   45 | `	}` |
...
|    90829 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90829 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90825 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90829 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90829 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90829 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9077 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4537 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90829 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90829 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90829 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136240 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90825 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9177 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2435801 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2422045 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2422045 |  104 | `				if( !pClass->bMounted ){` |
|   243737 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   243737 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4585 |  115 | `		}` |
|    90823 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90823 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     9077 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4540 |  126 | `		}else{` |
|        - |  127 | `			/* Assume a null return value */` |
|    81749 |  128 | `			PH7_MemObjInit(pVm,&sResult);` |
|        - |  129 | `		}` |
...
|    90823 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90823 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9175 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4585 |  139 | `		}` |
|    90823 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45412 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90829 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90829 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90829 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90829 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90829 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90829 |  151 | `	if( bNested ){` |
...
|        - |  163 | ` */` |
|    73872 |  164 | `PH7_PRIVATE sxi32 PH7_VmEvalBuiltinChunk(ph7_vm *pVm,const char *zSrc,sxu32 nLen)` |
|        5 |  165 | `{` |
|        - |  166 | `	SyString sChunk;` |
|    73877 |  167 | `	SyStringInitFromBuf(&sChunk,zSrc,nLen);` |
|    73877 |  168 | `	return VmEvalChunk(&(*pVm),0,&sChunk,PH7_PHP_ONLY,FALSE);` |
|        5 |  169 | `}` |
|        - |  170 | `/*` |
...
|        - |  209 | ` */` |
|     9086 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     9089 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20530011 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20520938 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10260463 |  221 | `	}` |
|     9075 |  222 | `	return FALSE;` |
|     4546 |  223 | `}` |
|        - |  224 | `/*` |
...
|    12979 |  285 | `	if( !bMain ){` |
|     9089 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     9075 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9075 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  313 | ` */` |
|     9092 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     9095 |  328 | `	pVm = pCtx->pVm;` |
|     9095 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9095 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     9095 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     9095 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9095 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     9089 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9089 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     9077 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9077 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     9077 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9077 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4537 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     9089 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     9089 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     9089 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     9052 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        3 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     9055 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     9055 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9055 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     9055 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9055 |  556 | `	if( rc != SXRET_OK ){` |
