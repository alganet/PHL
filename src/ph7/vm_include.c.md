...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90600 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90605 |   32 | `	ProcConsumer xErr = 0;` |
|    90605 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90605 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90605 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90605 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90605 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     9105 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9105 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4551 |   45 | `	}` |
...
|    90605 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90605 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90601 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90605 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90605 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90605 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9105 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4551 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90605 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90605 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90605 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   135904 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90601 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9205 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2436963 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2423165 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2423165 |  104 | `				if( !pClass->bMounted ){` |
|   244379 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   244379 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4599 |  115 | `		}` |
|    90599 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90599 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     9105 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4554 |  126 | `		}else{` |
...
|    90599 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90599 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9203 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4599 |  139 | `		}` |
|    90599 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45300 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90605 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90605 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90605 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90605 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90605 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90605 |  151 | `	if( bNested ){` |
...
|        - |  209 | ` */` |
|     9114 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     9117 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20657115 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20648014 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10324001 |  221 | `	}` |
|     9103 |  222 | `	return FALSE;` |
|     4560 |  223 | `}` |
|        - |  224 | `/*` |
|        - |  225 | ` * Push a file path in the appropriate VM container.` |
|        - |  226 | ` */` |
|    12990 |  227 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  228 | `{` |
|        - |  229 | `	SyString sPath;` |
|        - |  230 | `	char *zDup;` |
|        - |  231 | `	sxi32 rc;` |
...
|    12995 |  236 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    12995 |  237 | `	if( zDup == 0 ){` |
...
|        - |  246 | `	{` |
|    12990 |  247 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    12990 |  248 | `		if( zReal ){` |
|    12954 |  249 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12954 |  250 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12954 |  251 | `			free(zReal);` |
|    12954 |  252 | `			if( zRealDup ){` |
|    12954 |  253 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12954 |  254 | `				zDup = zRealDup;` |
|    12954 |  255 | `				nLen = (int)nReal;` |
|     6477 |  256 | `			}` |
|     6477 |  257 | `		}` |
|        - |  258 | `	}` |
|        - |  259 | `#endif` |
...
|    12995 |  284 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    12995 |  285 | `	if( !bMain ){` |
|     9117 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     9103 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9103 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  297 | `		}` |
|     4557 |  298 | `	}` |
|    12995 |  299 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    12995 |  300 | `	return SXRET_OK;` |
|     6500 |  301 | `}` |
|        - |  302 | `/*` |
...
|        - |  313 | ` */` |
|     9120 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     9123 |  328 | `	pVm = pCtx->pVm;` |
|     9123 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9123 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     9123 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     9123 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9123 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     9117 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9117 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     9105 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9105 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     9105 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9105 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4551 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     9117 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     9117 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     9117 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     9080 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        3 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     9083 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     9083 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9083 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     9083 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9083 |  556 | `	if( rc != SXRET_OK ){` |
