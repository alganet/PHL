...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90388 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90393 |   32 | `	ProcConsumer xErr = 0;` |
|    90393 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90393 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90393 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90393 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90393 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     8977 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     8977 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4487 |   45 | `	}` |
...
|    90393 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90393 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90389 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90393 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90393 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90393 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     8977 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4487 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90393 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90393 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90393 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   135586 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90389 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9077 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2409285 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2395679 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2395679 |  104 | `				if( !pClass->bMounted ){` |
|   241081 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   241081 |  106 | `					if( rc != SXRET_OK ){` |
...
|        4 |  114 | `			}` |
|     4535 |  115 | `		}` |
|    90387 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90387 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     8977 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4490 |  126 | `		}else{` |
|        - |  127 | `			/* Assume a null return value */` |
|    81413 |  128 | `			PH7_MemObjInit(pVm,&sResult);` |
|        - |  129 | `		}` |
...
|    90387 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90387 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9074 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4535 |  139 | `		}` |
|    90387 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45194 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90393 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90393 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90393 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90393 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90393 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90393 |  151 | `	if( bNested ){` |
...
|        - |  163 | ` */` |
|    73568 |  164 | `PH7_PRIVATE sxi32 PH7_VmEvalBuiltinChunk(ph7_vm *pVm,const char *zSrc,sxu32 nLen)` |
|        5 |  165 | `{` |
|        - |  166 | `	SyString sChunk;` |
|    73573 |  167 | `	SyStringInitFromBuf(&sChunk,zSrc,nLen);` |
|    73573 |  168 | `	return VmEvalChunk(&(*pVm),0,&sChunk,PH7_PHP_ONLY,FALSE);` |
|        5 |  169 | `}` |
|        - |  170 | `/*` |
...
|        - |  209 | ` */` |
|     8986 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     8989 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20079401 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20070428 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10035208 |  221 | `	}` |
|     8975 |  222 | `	return FALSE;` |
|     4496 |  223 | `}` |
|        - |  224 | `/*` |
|        - |  225 | ` * Push a file path in the appropriate VM container.` |
|        - |  226 | ` */` |
|    12858 |  227 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  228 | `{` |
|        - |  229 | `	SyString sPath;` |
|        - |  230 | `	char *zDup;` |
|        - |  231 | `	sxi32 rc;` |
|    12863 |  232 | `	if( nLen < 0 ){` |
|     3877 |  233 | `		nLen = SyStrlen(zPath);` |
|     1936 |  234 | `	}` |
|        - |  235 | `	/* Duplicate the file path first */` |
|    12863 |  236 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    12863 |  237 | `	if( zDup == 0 ){` |
...
|        - |  246 | `	{` |
|    12858 |  247 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    12858 |  248 | `		if( zReal ){` |
|    12822 |  249 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12822 |  250 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12822 |  251 | `			free(zReal);` |
|    12822 |  252 | `			if( zRealDup ){` |
|    12822 |  253 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12822 |  254 | `				zDup = zRealDup;` |
|    12822 |  255 | `				nLen = (int)nReal;` |
|     6411 |  256 | `			}` |
|     6411 |  257 | `		}` |
|        - |  258 | `	}` |
|        - |  259 | `#endif` |
...
|    12863 |  284 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    12863 |  285 | `	if( !bMain ){` |
|     8989 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     8975 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     8975 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  297 | `		}` |
|     4493 |  298 | `	}` |
|    12863 |  299 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    12863 |  300 | `	return SXRET_OK;` |
|     6434 |  301 | `}` |
|        - |  302 | `/*` |
...
|        - |  313 | ` */` |
|     8992 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     8996 |  328 | `	pVm = pCtx->pVm;` |
|     8996 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     8996 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     8996 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     8996 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     8996 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     8989 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     8989 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     8977 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     8977 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     8977 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     8977 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4487 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     8989 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     8989 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     8989 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     8952 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        4 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     8956 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     8956 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     8956 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     8956 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     8956 |  556 | `	if( rc != SXRET_OK ){` |
...
|        - |  626 | `	}` |
|        - |  627 | `	/* File to include */` |
|       16 |  628 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|       16 |  629 | `	if( sFile.nByte < 1 ){` |
...
|       16 |  635 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|       16 |  636 | `	if( rc != SXRET_OK ){` |
...
|        - |  715 | ` */` |
|       40 |  716 | `PH7_PRIVATE int vm_builtin_spl_autoload_register(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        5 |  717 | `{` |
|        - |  718 | `	VmAutoloadCB sEntry;` |
|       45 |  719 | `	ph7_vm *pVm = pCtx->pVm;` |
|       45 |  720 | `	int iPrepend = 0;` |
|        - |  721 | `	sxu32 n;` |
|       45 |  722 | `	if( nArg < 1 ){` |
...
|       37 |  743 | `	if( !PH7_VmIsCallable(pVm,apArg[0],TRUE) ){` |
...
|       55 |  756 | `	for( n = 0 ; n < SySetUsed(&pVm->aAutoload) ; ++n ){` |
...
|       37 |  769 | `	SyZero(&sEntry,sizeof(VmAutoloadCB));` |
|       37 |  770 | `	PH7_MemObjInit(pVm,&sEntry.sCallback);` |
|       37 |  771 | `	PH7_MemObjStore(apArg[0],&sEntry.sCallback);` |
|       38 |  772 | `	if( iPrepend && SySetUsed(&pVm->aAutoload) > 0 ){` |
...
|        1 |  788 | `		}` |
|        2 |  789 | `	}else{` |
|       35 |  790 | `		SySetPut(&pVm->aAutoload,(const void *)&sEntry);` |
|        - |  791 | `	}` |
|       37 |  792 | `	ph7_result_bool(pCtx,1);` |
|       37 |  793 | `	return SXRET_OK;` |
|       25 |  794 | `}` |
|        - |  795 | `/*` |
