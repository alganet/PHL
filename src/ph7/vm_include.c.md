...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90892 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90897 |   32 | `	ProcConsumer xErr = 0;` |
|    90897 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|        - |   36 | `	/* Initialize bytecode container */` |
|    90897 |   37 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90897 |   38 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   39 | `	/* Reset the code generator */` |
|    90897 |   40 | `	if( bTrueReturn ){` |
...
|    90897 |   49 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90897 |   50 | `	if( bNested ){` |
|        5 |   51 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   52 | `	}else{` |
|    90893 |   53 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   54 | `	}` |
...
|    90897 |   58 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90897 |   59 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90897 |   60 | `	if( bTrueReturn ){` |
|        - |   61 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9152 |   62 | `		SyBlobReset(&pVm->sNamespace);` |
|     4574 |   63 | `	}` |
|        - |   64 | `	/* Swap bytecode container */` |
|    90897 |   65 | `	pByteCode = pVm->pByteContainer;` |
|    90897 |   66 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   67 | `	/* Compile the chunk */` |
|    90897 |   68 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136342 |   69 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   84 | `		sxi32 rc;` |
|    90895 |   85 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
...
|        5 |  100 | `			}` |
|     4620 |  101 | `		}` |
|    90893 |  102 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  103 | `			/* Out of memory */` |
|      ! 0 |  104 | `			if( pCtx ){` |
...
|    90893 |  121 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90893 |  122 | `		if( pCtx ){` |
|        - |  123 | `			/* Set the execution result */` |
|     9245 |  124 | `			ph7_result_value(pCtx,&sResult);` |
|     4620 |  125 | `		}` |
|    90893 |  126 | `		PH7_MemObjRelease(&sResult);` |
|        - |  127 | `	}` |
|    45446 |  128 | `Cleanup:` |
|        - |  129 | `	/* Cleanup the mess left behind */` |
|    90897 |  130 | `	pVm->pByteContainer = pByteCode;` |
|    90897 |  131 | `	SySetRelease(&aByteCode);` |
|        - |  132 | `	/* Restore caller's namespace state */` |
|    90897 |  133 | `	SyBlobReset(&pVm->sNamespace);` |
|    90897 |  134 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90897 |  135 | `	SyBlobRelease(&sSavedNs);` |
|        - |  136 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90897 |  137 | `	if( bNested ){` |
|        5 |  138 | `		PH7_CompilerRestoreState(pVm,&sSavedGen);` |
|        2 |  139 | `	}` |
|    90897 |  140 | `	return SXRET_OK;` |
|        5 |  141 | `}` |
|        - |  142 | `/*` |
...
|        - |  146 | ` */` |
|    73872 |  147 | `PH7_PRIVATE sxi32 PH7_VmEvalBuiltinChunk(ph7_vm *pVm,const char *zSrc,sxu32 nLen)` |
|        5 |  148 | `{` |
|        - |  149 | `	SyString sChunk;` |
|    73877 |  150 | `	SyStringInitFromBuf(&sChunk,zSrc,nLen);` |
|    73877 |  151 | `	return VmEvalChunk(&(*pVm),0,&sChunk,PH7_PHP_ONLY,FALSE);` |
|        5 |  152 | `}` |
|        - |  153 | `/*` |
...
|        - |  205 | ` */` |
|    13048 |  206 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  207 | `{` |
|        - |  208 | `	SyString sPath;` |
|        - |  209 | `	char *zDup;` |
|        - |  210 | `	sxi32 rc;` |
|    13053 |  211 | `	if( nLen < 0 ){` |
|     3893 |  212 | `		nLen = SyStrlen(zPath);` |
|     1944 |  213 | `	}` |
|        - |  214 | `	/* Duplicate the file path first */` |
|    13053 |  215 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    13053 |  216 | `	if( zDup == 0 ){` |
...
|        - |  225 | `	{` |
|    13048 |  226 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    13048 |  227 | `		if( zReal ){` |
|    13012 |  228 | `			sxu32 nReal = SyStrlen(zReal);` |
|    13012 |  229 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    13012 |  230 | `			free(zReal);` |
|    13012 |  231 | `			if( zRealDup ){` |
|    13012 |  232 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    13012 |  233 | `				zDup = zRealDup;` |
|    13012 |  234 | `				nLen = (int)nReal;` |
|     6506 |  235 | `			}` |
|     6506 |  236 | `		}` |
|        - |  237 | `	}` |
|        - |  238 | `#endif` |
...
|    13053 |  263 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    13053 |  264 | `	if( !bMain ){` |
...
|        - |  276 | `		}` |
|     4580 |  277 | `	}` |
|    13053 |  278 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    13053 |  279 | `	return SXRET_OK;` |
|     6529 |  280 | `}` |
|        - |  281 | `/*` |
...
|     9170 |  307 | `	pVm = pCtx->pVm;` |
|     9170 |  308 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9170 |  309 | `	isNew = 0;` |
|        - |  310 | `	/* Extract the associated stream */` |
|     9170 |  311 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  312 | `	/*` |
|        - |  313 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  314 | `	 * in a read-only mode.` |
|        - |  315 | `	 */` |
|     9170 |  316 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9170 |  317 | `	if( pHandle == 0 ){` |
