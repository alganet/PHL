...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90802 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90807 |   32 | `	ProcConsumer xErr = 0;` |
|    90807 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90807 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90807 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90807 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90807 |   41 | `	if( bTrueReturn ){` |
...
|    90807 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90807 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90803 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90807 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90807 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90807 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9056 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4526 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90807 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90807 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90807 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136207 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90803 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
...
|        5 |  114 | `			}` |
|     4574 |  115 | `		}` |
|    90801 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|    90801 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90801 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9153 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4574 |  139 | `		}` |
|    90801 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45401 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90807 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90807 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90807 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90807 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90807 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90807 |  151 | `	if( bNested ){` |
...
|        - |  226 | ` */` |
|    12952 |  227 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  228 | `{` |
|        - |  229 | `	SyString sPath;` |
|        - |  230 | `	char *zDup;` |
|        - |  231 | `	sxi32 rc;` |
...
|    12957 |  236 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    12957 |  237 | `	if( zDup == 0 ){` |
...
|        - |  246 | `	{` |
|    12952 |  247 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    12952 |  248 | `		if( zReal ){` |
|    12916 |  249 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12916 |  250 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12916 |  251 | `			free(zReal);` |
|    12916 |  252 | `			if( zRealDup ){` |
|    12916 |  253 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12916 |  254 | `				zDup = zRealDup;` |
|    12916 |  255 | `				nLen = (int)nReal;` |
|     6458 |  256 | `			}` |
|     6458 |  257 | `		}` |
|        - |  258 | `	}` |
|        - |  259 | `#endif` |
...
|    12957 |  284 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    12957 |  285 | `	if( !bMain ){` |
...
|        - |  297 | `		}` |
|     4532 |  298 | `	}` |
|    12957 |  299 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    12957 |  300 | `	return SXRET_OK;` |
|     6481 |  301 | `}` |
|        - |  302 | `/*` |
