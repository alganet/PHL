...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90718 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90723 |   32 | `	ProcConsumer xErr = 0;` |
|    90723 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|        - |   36 | `	/* Initialize bytecode container */` |
|    90723 |   37 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90723 |   38 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   39 | `	/* Reset the code generator */` |
|    90723 |   40 | `	if( bTrueReturn ){` |
|        - |   41 | `		/* Included file,log compile-time errors */` |
|     9145 |   42 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9145 |   43 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4571 |   44 | `	}` |
...
|    90723 |   49 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90723 |   50 | `	if( bNested ){` |
|        5 |   51 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   52 | `	}else{` |
|    90719 |   53 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   54 | `	}` |
...
|    90723 |   58 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90723 |   59 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90723 |   60 | `	if( bTrueReturn ){` |
|        - |   61 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9145 |   62 | `		SyBlobReset(&pVm->sNamespace);` |
|     4571 |   63 | `	}` |
|        - |   64 | `	/* Swap bytecode container */` |
|    90723 |   65 | `	pByteCode = pVm->pByteContainer;` |
|    90723 |   66 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   67 | `	/* Compile the chunk */` |
|    90723 |   68 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136081 |   69 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   84 | `		sxi32 rc;` |
|    90721 |   85 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9241 |   86 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2426671 |   87 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2412819 |   88 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |   89 | `				/* Only mount classes that haven't been mounted yet */` |
|  2412819 |   90 | `				if( !pClass->bMounted ){` |
|   245197 |   91 | `					rc = VmMountUserClass(pVm,pClass);` |
|   245197 |   92 | `					if( rc != SXRET_OK ){` |
...
|        5 |  100 | `			}` |
|     4617 |  101 | `		}` |
|    90719 |  102 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  103 | `			/* Out of memory */` |
|      ! 0 |  104 | `			if( pCtx ){` |
...
|      ! 0 |  107 | `			goto Cleanup;` |
|        - |  108 | `		}` |
|    90719 |  109 | `		if( bTrueReturn ){` |
|        - |  110 | `			/* Assume a boolean true return value */` |
|     9145 |  111 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4574 |  112 | `		}else{` |
|        - |  113 | `			/* Assume a null return value */` |
|    81577 |  114 | `			PH7_MemObjInit(pVm,&sResult);` |
|        - |  115 | `		}` |
...
|    90719 |  121 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90719 |  122 | `		if( pCtx ){` |
|        - |  123 | `			/* Set the execution result */` |
|     9239 |  124 | `			ph7_result_value(pCtx,&sResult);` |
|     4617 |  125 | `		}` |
|    90719 |  126 | `		PH7_MemObjRelease(&sResult);` |
|        - |  127 | `	}` |
|    45359 |  128 | `Cleanup:` |
|        - |  129 | `	/* Cleanup the mess left behind */` |
|    90723 |  130 | `	pVm->pByteContainer = pByteCode;` |
|    90723 |  131 | `	SySetRelease(&aByteCode);` |
|        - |  132 | `	/* Restore caller's namespace state */` |
|    90723 |  133 | `	SyBlobReset(&pVm->sNamespace);` |
|    90723 |  134 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90723 |  135 | `	SyBlobRelease(&sSavedNs);` |
|        - |  136 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90723 |  137 | `	if( bNested ){` |
|        5 |  138 | `		PH7_CompilerRestoreState(pVm,&sSavedGen);` |
|        2 |  139 | `	}` |
|    90723 |  140 | `	return SXRET_OK;` |
|        5 |  141 | `}` |
|        - |  142 | `/*` |
...
|        - |  146 | ` */` |
|    73720 |  147 | `PH7_PRIVATE sxi32 PH7_VmEvalBuiltinChunk(ph7_vm *pVm,const char *zSrc,sxu32 nLen)` |
|        5 |  148 | `{` |
|        - |  149 | `	SyString sChunk;` |
|    73725 |  150 | `	SyStringInitFromBuf(&sChunk,zSrc,nLen);` |
|    73725 |  151 | `	return VmEvalChunk(&(*pVm),0,&sChunk,PH7_PHP_ONLY,FALSE);` |
|        5 |  152 | `}` |
|        - |  153 | `/*` |
...
|        - |  183 | `	}` |
|       32 |  184 | `	return SXRET_OK;` |
|       54 |  185 | `}` |
|        - |  186 | `/*` |
|        - |  187 | ` * Check if a file path is already included.` |
|        - |  188 | ` */` |
|     9154 |  189 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  190 | `{` |
|        - |  191 | `	SyString *aEntries;` |
|        - |  192 | `	sxu32 n;` |
|     9157 |  193 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  194 | `	/* Perform a linear search */` |
| 20839309 |  195 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20830168 |  196 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  197 | `			/* Already included */` |
|       16 |  198 | `			return TRUE;` |
|        - |  199 | `		}` |
| 10415078 |  200 | `	}` |
|     9143 |  201 | `	return FALSE;` |
|     4580 |  202 | `}` |
|        - |  203 | `/*` |
|        - |  204 | ` * Push a file path in the appropriate VM container.` |
|        - |  205 | ` */` |
|    13034 |  206 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  207 | `{` |
|        - |  208 | `	SyString sPath;` |
|        - |  209 | `	char *zDup;` |
|        - |  210 | `	sxi32 rc;` |
|    13039 |  211 | `	if( nLen < 0 ){` |
|     3885 |  212 | `		nLen = SyStrlen(zPath);` |
|     1940 |  213 | `	}` |
|        - |  214 | `	/* Duplicate the file path first */` |
|    13039 |  215 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    13039 |  216 | `	if( zDup == 0 ){` |
...
|        - |  225 | `	{` |
|    13034 |  226 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    13034 |  227 | `		if( zReal ){` |
|    12998 |  228 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12998 |  229 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12998 |  230 | `			free(zReal);` |
|    12998 |  231 | `			if( zRealDup ){` |
|    12998 |  232 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12998 |  233 | `				zDup = zRealDup;` |
|    12998 |  234 | `				nLen = (int)nReal;` |
|     6499 |  235 | `			}` |
|     6499 |  236 | `		}` |
|        - |  237 | `	}` |
|        - |  238 | `#endif` |
...
|    13039 |  263 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    13039 |  264 | `	if( !bMain ){` |
|     9157 |  265 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  266 | `			/* Already included */` |
|       16 |  267 | `			*pNew = 0;` |
|        9 |  268 | `		}else{` |
|        - |  269 | `			/* Insert in the corresponding container */` |
|     9143 |  270 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9143 |  271 | `			if( rc != SXRET_OK ){` |
...
|        - |  276 | `		}` |
|     4577 |  277 | `	}` |
|    13039 |  278 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    13039 |  279 | `	return SXRET_OK;` |
|     6522 |  280 | `}` |
|        - |  281 | `/*` |
...
|        - |  292 | ` */` |
|     9160 |  293 | `static sxi32 VmExecIncludedFile(` |
...
|     9163 |  307 | `	pVm = pCtx->pVm;` |
|     9163 |  308 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9163 |  309 | `	isNew = 0;` |
|        - |  310 | `	/* Extract the associated stream */` |
|     9163 |  311 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  312 | `	/*` |
|        - |  313 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  314 | `	 * in a read-only mode.` |
|        - |  315 | `	 */` |
|     9163 |  316 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9163 |  317 | `	if( pHandle == 0 ){` |
|        8 |  318 | `		return SXERR_IO;` |
|        - |  319 | `	}` |
|     9157 |  320 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9157 |  321 | `	if( IncludeOnce && !isNew ){` |
|        - |  322 | `		/* Already included */` |
|       14 |  323 | `		rc = SXERR_EXISTS;` |
|        8 |  324 | `	}else{` |
|        - |  325 | `		/* Read the whole file contents */` |
|     9145 |  326 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9145 |  327 | `		if( rc == SXRET_OK ){` |
|        - |  328 | `			SyString sScript;` |
|        - |  329 | `			/* Compile and execute the script */` |
|     9145 |  330 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9145 |  331 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4571 |  332 | `		}` |
|        - |  333 | `	}` |
|        - |  334 | `	/* Pop from the set of included file */` |
|     9157 |  335 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  336 | `	/* Close the handle */` |
|     9157 |  337 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  338 | `	/* Release the working buffer */` |
|     9157 |  339 | `	SyBlobRelease(&sContents);` |
|        - |  340 | `#else` |
...
|        - |  516 | ` */` |
|     9120 |  517 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        3 |  518 | `{` |
|        - |  519 | `	SyString sFile;` |
|        - |  520 | `	sxi32 rc;` |
|     9123 |  521 | `	if( nArg < 1 ){` |
...
|        - |  525 | `	}` |
|        - |  526 | `	/* File to include */` |
|     9123 |  527 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9123 |  528 | `	if( sFile.nByte < 1 ){` |
...
|     9123 |  534 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9123 |  535 | `	if( rc != SXRET_OK ){` |
