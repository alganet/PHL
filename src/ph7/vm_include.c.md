...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90768 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90773 |   32 | `	ProcConsumer xErr = 0;` |
|    90773 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90773 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90773 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90773 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90773 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     9063 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9063 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4530 |   45 | `	}` |
...
|    90773 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90773 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90769 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90773 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90773 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90773 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9063 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4530 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90773 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90773 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90773 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136156 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90769 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9163 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2429936 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2416201 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2416201 |  104 | `				if( !pClass->bMounted ){` |
|   243207 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   243207 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4578 |  115 | `		}` |
|    90767 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90767 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     9063 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4533 |  126 | `		}else{` |
|        - |  127 | `			/* Assume a null return value */` |
|    81707 |  128 | `			PH7_MemObjInit(pVm,&sResult);` |
|        - |  129 | `		}` |
...
|    90767 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90767 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9161 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4578 |  139 | `		}` |
|    90767 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45384 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90773 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90773 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90773 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90773 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90773 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90773 |  151 | `	if( bNested ){` |
...
|        - |  163 | ` */` |
|    73834 |  164 | `PH7_PRIVATE sxi32 PH7_VmEvalBuiltinChunk(ph7_vm *pVm,const char *zSrc,sxu32 nLen)` |
|        5 |  165 | `{` |
|        - |  166 | `	SyString sChunk;` |
|    73839 |  167 | `	SyStringInitFromBuf(&sChunk,zSrc,nLen);` |
|    73839 |  168 | `	return VmEvalChunk(&(*pVm),0,&sChunk,PH7_PHP_ONLY,FALSE);` |
|        5 |  169 | `}` |
|        - |  170 | `/*` |
...
|      108 |  190 | `	sChunk.zString = ph7_value_to_string(apArg[0],(int *)&sChunk.nByte);` |
|      108 |  191 | `	if( sChunk.nByte < 1 ){` |
...
|        - |  196 | `	}` |
|        - |  197 | `	/* Eval the chunk */` |
|      106 |  198 | `	rc = VmEvalChunk(pCtx->pVm,&(*pCtx),&sChunk,PH7_PHP_ONLY,FALSE);` |
|      106 |  199 | `	if( pCtx->pVm->bHaltRequested ){` |
...
|        - |  209 | ` */` |
|     9072 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     9075 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20466647 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20457588 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10228788 |  221 | `	}` |
|     9061 |  222 | `	return FALSE;` |
|     4539 |  223 | `}` |
|        - |  224 | `/*` |
|        - |  225 | ` * Push a file path in the appropriate VM container.` |
|        - |  226 | ` */` |
|    12958 |  227 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  228 | `{` |
|        - |  229 | `	SyString sPath;` |
|        - |  230 | `	char *zDup;` |
|        - |  231 | `	sxi32 rc;` |
|    12963 |  232 | `	if( nLen < 0 ){` |
|     3891 |  233 | `		nLen = SyStrlen(zPath);` |
|     1943 |  234 | `	}` |
|        - |  235 | `	/* Duplicate the file path first */` |
|    12963 |  236 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    12963 |  237 | `	if( zDup == 0 ){` |
...
|        - |  246 | `	{` |
|    12958 |  247 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    12958 |  248 | `		if( zReal ){` |
|    12922 |  249 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12922 |  250 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12922 |  251 | `			free(zReal);` |
|    12922 |  252 | `			if( zRealDup ){` |
|    12922 |  253 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12922 |  254 | `				zDup = zRealDup;` |
|    12922 |  255 | `				nLen = (int)nReal;` |
|     6461 |  256 | `			}` |
|     6461 |  257 | `		}` |
|        - |  258 | `	}` |
|        - |  259 | `#endif` |
...
|    12963 |  284 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    12963 |  285 | `	if( !bMain ){` |
|     9075 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     9061 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9061 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  297 | `		}` |
|     4536 |  298 | `	}` |
|    12963 |  299 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    12963 |  300 | `	return SXRET_OK;` |
|     6484 |  301 | `}` |
|        - |  302 | `/*` |
...
|        - |  313 | ` */` |
|     9078 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     9082 |  328 | `	pVm = pCtx->pVm;` |
|     9082 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9082 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     9082 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     9082 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9082 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     9075 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9075 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     9063 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9063 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     9063 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9063 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4530 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     9075 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     9075 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     9075 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     9038 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        4 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     9042 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     9042 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9042 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     9042 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9042 |  556 | `	if( rc != SXRET_OK ){` |
