...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90774 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90779 |   32 | `	ProcConsumer xErr = 0;` |
|    90779 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90779 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90779 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90779 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90779 |   41 | `	if( bTrueReturn ){` |
...
|    90779 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90779 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90775 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90779 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90779 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90779 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9069 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4533 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90779 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90779 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90779 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136165 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90775 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
...
|        5 |  114 | `			}` |
|     4581 |  115 | `		}` |
|    90773 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|    90773 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90773 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9167 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4581 |  139 | `		}` |
|    90773 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45387 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90779 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90779 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90779 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90779 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90779 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90779 |  151 | `	if( bNested ){` |
...
|        - |  226 | ` */` |
|    12964 |  227 | `PH7_PRIVATE sxi32 PH7_VmPushFilePath(ph7_vm *pVm,const char *zPath,int nLen,sxu8 bMain,sxi32 *pNew)` |
|        5 |  228 | `{` |
|        - |  229 | `	SyString sPath;` |
|        - |  230 | `	char *zDup;` |
|        - |  231 | `	sxi32 rc;` |
...
|    12969 |  236 | `	zDup = SyMemBackendStrDup(&pVm->sAllocator,zPath,nLen);` |
|    12969 |  237 | `	if( zDup == 0 ){` |
...
|        - |  246 | `	{` |
|    12964 |  247 | `		char *zReal = realpath(zDup,0); /* POSIX: malloc'd result */` |
|    12964 |  248 | `		if( zReal ){` |
|    12928 |  249 | `			sxu32 nReal = SyStrlen(zReal);` |
|    12928 |  250 | `			char *zRealDup = SyMemBackendStrDup(&pVm->sAllocator,zReal,nReal);` |
|    12928 |  251 | `			free(zReal);` |
|    12928 |  252 | `			if( zRealDup ){` |
|    12928 |  253 | `				SyMemBackendFree(&pVm->sAllocator,zDup);` |
|    12928 |  254 | `				zDup = zRealDup;` |
|    12928 |  255 | `				nLen = (int)nReal;` |
|     6464 |  256 | `			}` |
|     6464 |  257 | `		}` |
|        - |  258 | `	}` |
|        - |  259 | `#endif` |
...
|    12969 |  284 | `	SyStringInitFromBuf(&sPath,zDup,nLen);` |
|    12969 |  285 | `	if( !bMain ){` |
...
|        - |  297 | `		}` |
|     4539 |  298 | `	}` |
|    12969 |  299 | `	SySetPut(&pVm->aFiles,(const void *)&sPath);` |
|    12969 |  300 | `	return SXRET_OK;` |
|     6487 |  301 | `}` |
|        - |  302 | `/*` |
