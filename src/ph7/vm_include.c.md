...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90676 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90681 |   32 | `	ProcConsumer xErr = 0;` |
|    90681 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90681 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90681 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90681 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90681 |   41 | `	if( bTrueReturn ){` |
...
|    90681 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90681 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90677 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90681 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90681 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90681 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9056 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4526 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90681 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90681 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90681 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   136018 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90677 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9155 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2425434 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2411711 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2411711 |  104 | `				if( !pClass->bMounted ){` |
|   242825 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   242825 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4574 |  115 | `		}` |
|    90675 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|    90675 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90675 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9153 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4574 |  139 | `		}` |
|    90675 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45338 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90681 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90681 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90681 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90681 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90681 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90681 |  151 | `	if( bNested ){` |
...
| 20430490 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20421438 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
