...
|        - |   20 | ` * information.` |
|        - |   21 | ` */` |
|    90542 |   22 | `PH7_PRIVATE sxi32 VmEvalChunk(` |
...
|        - |   31 | `	SyBlob sSavedNs;` |
|    90547 |   32 | `	ProcConsumer xErr = 0;` |
|    90547 |   33 | `	void *pErrData = 0;` |
|        - |   34 | `	ph7_gen_state sSavedGen;` |
|        - |   35 | `	int bNested;` |
|    90547 |   36 | `	sxi32 rcThrow = SXRET_OK; /* status of a ParseError raised for a failed eval() compile */` |
|        - |   37 | `	/* Initialize bytecode container */` |
|    90547 |   38 | `	SySetInit(&aByteCode,&pVm->sAllocator,sizeof(VmInstr));` |
|    90547 |   39 | `	SySetAlloc(&aByteCode,0x20);` |
|        - |   40 | `	/* Reset the code generator */` |
|    90547 |   41 | `	if( bTrueReturn ){` |
|        - |   42 | `		/* Included file,log compile-time errors */` |
|     9089 |   43 | `		xErr = pVm->pEngine->xConf.xErr;` |
|     9089 |   44 | `		pErrData = pVm->pEngine->xConf.pErrData;` |
|     4543 |   45 | `	}` |
...
|    90547 |   50 | `	bNested = (pVm->sCodeGen.pIn != 0);` |
|    90547 |   51 | `	if( bNested ){` |
|        5 |   52 | `		PH7_CompilerSaveState(pVm,&sSavedGen,xErr,pErrData);` |
|        3 |   53 | `	}else{` |
|    90543 |   54 | `		PH7_ResetCodeGenerator(pVm,xErr,pErrData);` |
|        - |   55 | `	}` |
...
|    90547 |   59 | `	SyBlobInit(&sSavedNs,&pVm->sAllocator);` |
|    90547 |   60 | `	SyBlobDup(&pVm->sNamespace,&sSavedNs);` |
|    90547 |   61 | `	if( bTrueReturn ){` |
|        - |   62 | `		/* Include/require: start in a fresh (global) namespace scope. */` |
|     9089 |   63 | `		SyBlobReset(&pVm->sNamespace);` |
|     4543 |   64 | `	}` |
|        - |   65 | `	/* Swap bytecode container */` |
|    90547 |   66 | `	pByteCode = pVm->pByteContainer;` |
|    90547 |   67 | `	pVm->pByteContainer = &aByteCode;` |
|        - |   68 | `	/* Compile the chunk */` |
|    90547 |   69 | `	PH7_CompileScript(pVm,pChunk,iFlags);` |
|   135817 |   70 | `	if( pVm->sCodeGen.nErr > 0 ){` |
...
|        - |   98 | `		sxi32 rc;` |
|    90543 |   99 | `		if( pVm->nMagic != PH7_VM_INIT ){` |
|     9189 |  100 | `			SyHashResetLoopCursor(&pVm->hClass);` |
|  2436603 |  101 | `			while((pEntry = SyHashGetNextEntry(&pVm->hClass)) != 0 ){` |
|  2422829 |  102 | `				pClass = (ph7_class *)pEntry->pUserData;` |
|        - |  103 | `				/* Only mount classes that haven't been mounted yet */` |
|  2422829 |  104 | `				if( !pClass->bMounted ){` |
|   244057 |  105 | `					rc = VmMountUserClass(pVm,pClass);` |
|   244057 |  106 | `					if( rc != SXRET_OK ){` |
...
|        5 |  114 | `			}` |
|     4591 |  115 | `		}` |
|    90541 |  116 | `		if( SXRET_OK != PH7_VmEmitInstr(pVm,PH7_OP_DONE,0,0,0,0) ){` |
|        - |  117 | `			/* Out of memory */` |
|      ! 0 |  118 | `			if( pCtx ){` |
...
|      ! 0 |  121 | `			goto Cleanup;` |
|        - |  122 | `		}` |
|    90541 |  123 | `		if( bTrueReturn ){` |
|        - |  124 | `			/* Assume a boolean true return value */` |
|     9089 |  125 | `			PH7_MemObjInitFromBool(pVm,&sResult,1);` |
|     4546 |  126 | `		}else{` |
...
|    90541 |  135 | `		VmLocalExec(pVm,&aByteCode,&sResult,FALSE);` |
|    90541 |  136 | `		if( pCtx ){` |
|        - |  137 | `			/* Set the execution result */` |
|     9187 |  138 | `			ph7_result_value(pCtx,&sResult);` |
|     4591 |  139 | `		}` |
|    90541 |  140 | `		PH7_MemObjRelease(&sResult);` |
|        - |  141 | `	}` |
|    45271 |  142 | `Cleanup:` |
|        - |  143 | `	/* Cleanup the mess left behind */` |
|    90547 |  144 | `	pVm->pByteContainer = pByteCode;` |
|    90547 |  145 | `	SySetRelease(&aByteCode);` |
|        - |  146 | `	/* Restore caller's namespace state */` |
|    90547 |  147 | `	SyBlobReset(&pVm->sNamespace);` |
|    90547 |  148 | `	SyBlobDup(&sSavedNs,&pVm->sNamespace);` |
|    90547 |  149 | `	SyBlobRelease(&sSavedNs);` |
|        - |  150 | `	/* Restore the outer compile's generator state if this was a nested unit. */` |
|    90547 |  151 | `	if( bNested ){` |
...
|        - |  209 | ` */` |
|     9098 |  210 | `static int VmIsIncludedFile(ph7_vm *pVm,SyString *pFile)` |
|        3 |  211 | `{` |
|        - |  212 | `	SyString *aEntries;` |
|        - |  213 | `	sxu32 n;` |
|     9101 |  214 | `	aEntries = (SyString *)SySetBasePtr(&pVm->aIncluded);` |
|        - |  215 | `	/* Perform a linear search */` |
| 20584419 |  216 | `	for( n = 0 ; n < SySetUsed(&pVm->aIncluded) ; ++n ){` |
| 20575334 |  217 | `		if( SyStringCmp(pFile,&aEntries[n],SyMemcmp) == 0 ){` |
|        - |  218 | `			/* Already included */` |
|       16 |  219 | `			return TRUE;` |
|        - |  220 | `		}` |
| 10287661 |  221 | `	}` |
|     9087 |  222 | `	return FALSE;` |
|     4552 |  223 | `}` |
|        - |  224 | `/*` |
...
|    12977 |  285 | `	if( !bMain ){` |
|     9101 |  286 | `		if( VmIsIncludedFile(&(*pVm),&sPath) ){` |
|        - |  287 | `			/* Already included */` |
|       16 |  288 | `			*pNew = 0;` |
|        9 |  289 | `		}else{` |
|        - |  290 | `			/* Insert in the corresponding container */` |
|     9087 |  291 | `			rc = SySetPut(&pVm->aIncluded,(const void *)&sPath);` |
|     9087 |  292 | `			if( rc != SXRET_OK ){` |
...
|        - |  313 | ` */` |
|     9104 |  314 | `static sxi32 VmExecIncludedFile(` |
...
|     9107 |  328 | `	pVm = pCtx->pVm;` |
|     9107 |  329 | `	SyBlobInit(&sContents,&pVm->sAllocator);` |
|     9107 |  330 | `	isNew = 0;` |
|        - |  331 | `	/* Extract the associated stream */` |
|     9107 |  332 | `	pStream = PH7_VmGetStreamDevice(pVm,&pPath->zString,pPath->nByte);` |
|        - |  333 | `	/*` |
|        - |  334 | `	 * Open the file or the URL [i.e: http://ph7.symisc.net/example/hello.php"]` |
|        - |  335 | `	 * in a read-only mode.` |
|        - |  336 | `	 */` |
|     9107 |  337 | `	pHandle = PH7_StreamOpenHandle(pVm,pStream,pPath->zString,PH7_IO_OPEN_RDONLY,TRUE,0,TRUE,&isNew);` |
|     9107 |  338 | `	if( pHandle == 0 ){` |
|        8 |  339 | `		return SXERR_IO;` |
|        - |  340 | `	}` |
|     9101 |  341 | `	rc = SXRET_OK; /* Stupid cc warning */` |
|     9101 |  342 | `	if( IncludeOnce && !isNew ){` |
|        - |  343 | `		/* Already included */` |
|       14 |  344 | `		rc = SXERR_EXISTS;` |
|        8 |  345 | `	}else{` |
|        - |  346 | `		/* Read the whole file contents */` |
|     9089 |  347 | `		rc = PH7_StreamReadWholeFile(pHandle,pStream,&sContents);` |
|     9089 |  348 | `		if( rc == SXRET_OK ){` |
|        - |  349 | `			SyString sScript;` |
|        - |  350 | `			/* Compile and execute the script */` |
|     9089 |  351 | `			SyStringInitFromBuf(&sScript,SyBlobData(&sContents),SyBlobLength(&sContents));` |
|     9089 |  352 | `			VmEvalChunk(pCtx->pVm,&(*pCtx),&sScript,0,TRUE);` |
|     4543 |  353 | `		}` |
|        - |  354 | `	}` |
|        - |  355 | `	/* Pop from the set of included file */` |
|     9101 |  356 | `	(void)SySetPop(&pVm->aFiles);` |
|        - |  357 | `	/* Close the handle */` |
|     9101 |  358 | `	PH7_StreamCloseHandle(pStream,pHandle);` |
|        - |  359 | `	/* Release the working buffer */` |
|     9101 |  360 | `	SyBlobRelease(&sContents);` |
|        - |  361 | `#else` |
...
|        - |  537 | ` */` |
|     9064 |  538 | `PH7_PRIVATE int vm_builtin_include(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|        3 |  539 | `{` |
|        - |  540 | `	SyString sFile;` |
|        - |  541 | `	sxi32 rc;` |
|     9067 |  542 | `	if( nArg < 1 ){` |
...
|        - |  546 | `	}` |
|        - |  547 | `	/* File to include */` |
|     9067 |  548 | `	sFile.zString = ph7_value_to_string(apArg[0],(int *)&sFile.nByte);` |
|     9067 |  549 | `	if( sFile.nByte < 1 ){` |
...
|     9067 |  555 | `	rc = VmExecIncludedFile(&(*pCtx),&sFile,FALSE);` |
|     9067 |  556 | `	if( rc != SXRET_OK ){` |
