...
|      - |   27 | ` */` |
|  52352 |   28 | `PH7_PRIVATE VmOpRc VmExecOpConsume(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   29 | `{` |
|  52357 |   30 | `	ph7_value *pTos = pState->pTos;` |
|  52357 |   31 | `	ph7_value *pStack = pState->pStack;` |
|  52357 |   32 | `	VmInstr *aInstr = pState->aInstr;` |
|  52357 |   33 | `	sxi32 pc = pState->pc;` |
|      - |   34 | `	sxi32 rc;` |
|  26176 |   35 | `	SXUNUSED(pInstr); SXUNUSED(pStack); SXUNUSED(aInstr); SXUNUSED(rc);` |
|  52357 |   36 | `	ph7_output_consumer *pCons = &pVm->sVmConsumer;` |
|  52357 |   37 | `	ph7_value *pCur,*pOut = pTos;` |
|      - |   38 |  |
|  52357 |   39 | `	pOut = &pTos[-pInstr->iP1 + 1];` |
|  52357 |   40 | `	pCur = pOut;` |
|      - |   41 | `	/* Start the consume process  */` |
| 104709 |   42 | `	while( pOut <= pTos ){` |
|      - |   43 | `		/* Force a string cast */` |
|  52357 |   44 | `		if( (pOut->iFlags & MEMOBJ_STRING) == 0 ){` |
|   3293 |   45 | `			PH7_MemObjToString(pOut);` |
|   1644 |   46 | `		}` |
|  52357 |   47 | `		if( SyBlobLength(&pOut->sBlob) > 0 ){` |
|      - |   48 | `			/*SyBlobNullAppend(&pOut->sBlob);*/` |
|      - |   49 | `			/* Invoke the output consumer callback */` |
|  39987 |   50 | `			rc = pCons->xConsumer(SyBlobData(&pOut->sBlob),SyBlobLength(&pOut->sBlob),pCons->pUserData);` |
|  39987 |   51 | `			VmTrackOutput(pVm, SyBlobLength(&pOut->sBlob));` |
|  39987 |   52 | `			SyBlobRelease(&pOut->sBlob);` |
|  39987 |   53 | `			if( rc == SXERR_ABORT ){` |
...
|  52357 |   58 | `		pOut++;` |
|      5 |   59 | `	}` |
|  52357 |   60 | `	pTos = &pCur[-1];` |
|  52357 |   61 | `	VM_EXIT_BREAK;` |
|    ! 0 |   62 | `	VM_EXIT_BREAK;` |
|  26181 |   63 | `}` |
|      - |   64 |  |
|      - |   65 | `/*` |
...
|      - |  407 | ` */` |
|   2788 |  408 | `PH7_PRIVATE VmOpRc VmExecOpLoadException(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  409 | `{` |
|   2793 |  410 | `	ph7_value *pTos = pState->pTos;` |
|   2793 |  411 | `	ph7_value *pStack = pState->pStack;` |
|   2793 |  412 | `	VmInstr *aInstr = pState->aInstr;` |
|   2793 |  413 | `	sxi32 pc = pState->pc;` |
|      - |  414 | `	sxi32 rc;` |
|   1394 |  415 | `	SXUNUSED(pInstr); SXUNUSED(pStack); SXUNUSED(aInstr); SXUNUSED(rc);` |
...
|      - |  418 | `	 * compiled object. */` |
|   2793 |  419 | `	ph7_exception *pException = VmExcActivate(&(*pVm),(ph7_exception *)pInstr->p3);` |
|      - |  420 | `	VmFrame *pFrameLocal;` |
|   2793 |  421 | `	if( pException == 0 ){` |
...
|   2793 |  427 | `	rc = VmEnterFrame(&(*pVm),0,0,&pFrameLocal);` |
|   2793 |  428 | `	if( rc != SXRET_OK ){` |
|    ! 0 |  429 | `		VmExcRelease(&(*pVm),pException);` |
|    ! 0 |  430 | `		VmErrorFormat(&(*pVm),PH7_CTX_ERR,"Fatal PH7 engine is runnig out of memory");` |
|    ! 0 |  431 | `		VM_EXIT_ABORT;` |
|      - |  432 | `	}` |
|   2793 |  433 | `	if( SXRET_OK != SySetPut(&pVm->aException,(const void *)&pException) ){` |
...
|   2793 |  440 | `	pFrameLocal->iFlags \|= VM_FRAME_EXCEPTION;` |
|   2793 |  441 | `	pFrameLocal->iExceptionJump = pInstr->iP2;` |
...
|   2793 |  448 | `	pException->iLandingPc = pFrameLocal->iExceptionJump;` |
|   2793 |  449 | `	pException->pOwnerInstr = (void *)aInstr;` |
...
|   2793 |  454 | `	pException->iStackDepth = (sxi32)(pTos - pStack);` |
|      - |  455 | `	/* '@' depth at try entry — see ph7_exception.iErrSuppress */` |
|   2793 |  456 | `	pException->iErrSuppress = pVm->nErrSuppress;` |
|      - |  457 | `	/* Point to the frame that trigger the exception */` |
|   2793 |  458 | `	pFrameLocal = pFrameLocal->pParent;` |
|   2793 |  459 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
|   2793 |  460 | `	pException->pFrame = pFrameLocal;` |
|   2793 |  461 | `	VM_EXIT_BREAK;` |
|    ! 0 |  462 | `	VM_EXIT_BREAK;` |
|   1399 |  463 | `}` |
|      - |  464 |  |
