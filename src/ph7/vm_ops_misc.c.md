...
|      - |   27 | ` */` |
|  51136 |   28 | `PH7_PRIVATE VmOpRc VmExecOpConsume(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   29 | `{` |
|  51141 |   30 | `	ph7_value *pTos = pState->pTos;` |
|  51141 |   31 | `	ph7_value *pStack = pState->pStack;` |
|  51141 |   32 | `	VmInstr *aInstr = pState->aInstr;` |
|  51141 |   33 | `	sxi32 pc = pState->pc;` |
|      - |   34 | `	sxi32 rc;` |
|  25568 |   35 | `	SXUNUSED(pInstr); SXUNUSED(pStack); SXUNUSED(aInstr); SXUNUSED(rc);` |
|  51141 |   36 | `	ph7_output_consumer *pCons = &pVm->sVmConsumer;` |
|  51141 |   37 | `	ph7_value *pCur,*pOut = pTos;` |
|      - |   38 |  |
|  51141 |   39 | `	pOut = &pTos[-pInstr->iP1 + 1];` |
|  51141 |   40 | `	pCur = pOut;` |
|      - |   41 | `	/* Start the consume process  */` |
| 102277 |   42 | `	while( pOut <= pTos ){` |
|      - |   43 | `		/* Force a string cast */` |
|  51141 |   44 | `		if( (pOut->iFlags & MEMOBJ_STRING) == 0 ){` |
|   3029 |   45 | `			PH7_MemObjToString(pOut);` |
|   1512 |   46 | `		}` |
|  51141 |   47 | `		if( SyBlobLength(&pOut->sBlob) > 0 ){` |
|      - |   48 | `			/*SyBlobNullAppend(&pOut->sBlob);*/` |
|      - |   49 | `			/* Invoke the output consumer callback */` |
|  38577 |   50 | `			rc = pCons->xConsumer(SyBlobData(&pOut->sBlob),SyBlobLength(&pOut->sBlob),pCons->pUserData);` |
|  38577 |   51 | `			VmTrackOutput(pVm, SyBlobLength(&pOut->sBlob));` |
|  38577 |   52 | `			SyBlobRelease(&pOut->sBlob);` |
|  38577 |   53 | `			if( rc == SXERR_ABORT ){` |
...
|  51141 |   58 | `		pOut++;` |
|      5 |   59 | `	}` |
|  51141 |   60 | `	pTos = &pCur[-1];` |
|  51141 |   61 | `	VM_EXIT_BREAK;` |
|    ! 0 |   62 | `	VM_EXIT_BREAK;` |
|  25573 |   63 | `}` |
|      - |   64 |  |
|      - |   65 | `/*` |
...
|      - |  407 | ` */` |
|   2702 |  408 | `PH7_PRIVATE VmOpRc VmExecOpLoadException(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  409 | `{` |
|   2707 |  410 | `	ph7_value *pTos = pState->pTos;` |
|   2707 |  411 | `	ph7_value *pStack = pState->pStack;` |
|   2707 |  412 | `	VmInstr *aInstr = pState->aInstr;` |
|   2707 |  413 | `	sxi32 pc = pState->pc;` |
|      - |  414 | `	sxi32 rc;` |
|   1351 |  415 | `	SXUNUSED(pInstr); SXUNUSED(pStack); SXUNUSED(aInstr); SXUNUSED(rc);` |
...
|      - |  418 | `	 * compiled object. */` |
|   2707 |  419 | `	ph7_exception *pException = VmExcActivate(&(*pVm),(ph7_exception *)pInstr->p3);` |
|      - |  420 | `	VmFrame *pFrameLocal;` |
|   2707 |  421 | `	if( pException == 0 ){` |
...
|   2707 |  427 | `	rc = VmEnterFrame(&(*pVm),0,0,&pFrameLocal);` |
|   2707 |  428 | `	if( rc != SXRET_OK ){` |
|    ! 0 |  429 | `		VmExcRelease(&(*pVm),pException);` |
|    ! 0 |  430 | `		VmErrorFormat(&(*pVm),PH7_CTX_ERR,"Fatal PH7 engine is runnig out of memory");` |
|    ! 0 |  431 | `		VM_EXIT_ABORT;` |
|      - |  432 | `	}` |
|   2707 |  433 | `	if( SXRET_OK != SySetPut(&pVm->aException,(const void *)&pException) ){` |
...
|   2707 |  440 | `	pFrameLocal->iFlags \|= VM_FRAME_EXCEPTION;` |
|   2707 |  441 | `	pFrameLocal->iExceptionJump = pInstr->iP2;` |
...
|   2707 |  448 | `	pException->iLandingPc = pFrameLocal->iExceptionJump;` |
|   2707 |  449 | `	pException->pOwnerInstr = (void *)aInstr;` |
...
|   2707 |  454 | `	pException->iStackDepth = (sxi32)(pTos - pStack);` |
|      - |  455 | `	/* '@' depth at try entry — see ph7_exception.iErrSuppress */` |
|   2707 |  456 | `	pException->iErrSuppress = pVm->nErrSuppress;` |
|      - |  457 | `	/* Point to the frame that trigger the exception */` |
|   2707 |  458 | `	pFrameLocal = pFrameLocal->pParent;` |
|   2707 |  459 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
|   2707 |  460 | `	pException->pFrame = pFrameLocal;` |
|   2707 |  461 | `	VM_EXIT_BREAK;` |
|    ! 0 |  462 | `	VM_EXIT_BREAK;` |
|   1356 |  463 | `}` |
|      - |  464 |  |
