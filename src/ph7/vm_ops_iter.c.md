...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 242916 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 242921 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 242921 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 242921 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 242921 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 242921 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 242921 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 242921 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 242921 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 242921 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 242921 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 242921 |   59 | `	pStep = apStep[nStep - 1];` |
| 242947 |   60 | `	while( nStep > 0 ){` |
| 242947 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 242921 |   62 | `			pStep = apStep[nStep - 1];` |
| 242921 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 242921 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241535 |   71 | `		pNode = pStep->pCursor;` |
| 241535 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23655 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23655 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23655 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11830 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 217885 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 217885 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6945 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6945 |   86 | `				if( pKey ){` |
|   6945 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3470 |   88 | `				}` |
|   3470 |   89 | `			}` |
| 217885 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217815 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217815 |  103 | `				if( pValue ){` |
| 217815 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108905 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122156 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24082 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24087 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24087 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24087 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24087 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24087 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24087 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24087 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24087 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24083 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24083 |  364 | `		if( pStep == 0 ){` |
...
|  24083 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24083 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24083 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24083 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23831 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23831 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23831 |  400 | `				pIterMap = pMap;` |
|  23831 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23831 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23831 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23831 |  417 | `				if( pIterMap == pMap ){` |
|  23827 |  418 | `					pMap->iRef++;` |
|  11911 |  419 | `				}` |
...
|  23831 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11918 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24077 |  518 | `		if( pStep ){` |
|  24077 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12036 |  529 | `		}` |
|      - |  530 | `	}` |
|  24081 |  531 | `	VmPopOperand(&pTos,1);` |
|  24081 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12046 |  534 | `}` |
|      - |  535 |  |
