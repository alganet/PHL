...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 260410 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 260415 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 260415 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 260415 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 260415 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 260415 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 260415 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 260415 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 260415 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 260415 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 260415 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 260415 |   59 | `	pStep = apStep[nStep - 1];` |
| 260441 |   60 | `	while( nStep > 0 ){` |
| 260441 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 260415 |   62 | `			pStep = apStep[nStep - 1];` |
| 260415 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 260415 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 259029 |   71 | `		pNode = pStep->pCursor;` |
| 259029 |   72 | `		if( pNode == 0 ){` |
...
| 235473 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 235473 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6863 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6863 |   86 | `				if( pKey ){` |
|   6863 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3429 |   88 | `				}` |
|   3429 |   89 | `			}` |
| 235473 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 235403 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 235403 |  103 | `				if( pValue ){` |
| 235403 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 117699 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 130903 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  23988 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  23993 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  23993 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  23993 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  23993 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  23993 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  23993 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  23993 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  23993 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  23989 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  23989 |  364 | `		if( pStep == 0 ){` |
...
|  23989 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  23989 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  23989 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  23989 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|      - |  517 | `		}` |
|  23983 |  518 | `		if( pStep ){` |
|  23983 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  11989 |  529 | `		}` |
|      - |  530 | `	}` |
|  23987 |  531 | `	VmPopOperand(&pTos,1);` |
|  23987 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  11999 |  534 | `}` |
|      - |  535 |  |
