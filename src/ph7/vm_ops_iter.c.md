...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 243786 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 243791 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 243791 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 243791 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 243791 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 243791 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 243791 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 243791 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 243791 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 243791 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 243791 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 243791 |   59 | `	pStep = apStep[nStep - 1];` |
| 243817 |   60 | `	while( nStep > 0 ){` |
| 243817 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 243791 |   62 | `			pStep = apStep[nStep - 1];` |
| 243791 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 243791 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 242405 |   71 | `		pNode = pStep->pCursor;` |
| 242405 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23699 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23699 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23699 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11852 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 218711 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 218711 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6955 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6955 |   86 | `				if( pKey ){` |
|   6955 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3475 |   88 | `				}` |
|   3475 |   89 | `			}` |
| 218711 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 218641 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 218641 |  103 | `				if( pValue ){` |
| 218641 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 109318 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122591 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24126 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24131 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24131 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24131 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24131 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24131 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24131 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24131 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24131 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24127 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24127 |  364 | `		if( pStep == 0 ){` |
...
|  24127 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24127 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24127 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24127 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23875 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23875 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23875 |  400 | `				pIterMap = pMap;` |
|  23875 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23875 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23875 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23875 |  417 | `				if( pIterMap == pMap ){` |
|  23871 |  418 | `					pMap->iRef++;` |
|  11933 |  419 | `				}` |
...
|  23875 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11940 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24121 |  518 | `		if( pStep ){` |
|  24121 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12058 |  529 | `		}` |
|      - |  530 | `	}` |
|  24125 |  531 | `	VmPopOperand(&pTos,1);` |
|  24125 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12068 |  534 | `}` |
|      - |  535 |  |
