...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 243246 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 243251 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 243251 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 243251 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 243251 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 243251 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 243251 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 243251 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 243251 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 243251 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 243251 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 243251 |   59 | `	pStep = apStep[nStep - 1];` |
| 243277 |   60 | `	while( nStep > 0 ){` |
| 243277 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 243251 |   62 | `			pStep = apStep[nStep - 1];` |
| 243251 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 243251 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241865 |   71 | `		pNode = pStep->pCursor;` |
| 241865 |   72 | `		if( pNode == 0 ){` |
...
| 218197 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 218197 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6949 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6949 |   86 | `				if( pKey ){` |
|   6949 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3472 |   88 | `				}` |
|   3472 |   89 | `			}` |
| 218197 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 218127 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 218127 |  103 | `				if( pValue ){` |
| 218127 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 109061 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122321 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
