...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 242168 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 242173 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 242173 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 242173 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 242173 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 242173 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 242173 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 242173 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 242173 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 242173 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 242173 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 242173 |   59 | `	pStep = apStep[nStep - 1];` |
| 242199 |   60 | `	while( nStep > 0 ){` |
| 242199 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 242173 |   62 | `			pStep = apStep[nStep - 1];` |
| 242173 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 242173 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 240787 |   71 | `		pNode = pStep->pCursor;` |
| 240787 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23689 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23689 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23689 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11847 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 217103 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 217103 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6985 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6985 |   86 | `				if( pKey ){` |
|   6985 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3490 |   88 | `				}` |
|   3490 |   89 | `			}` |
| 217103 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217033 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217033 |  103 | `				if( pValue ){` |
| 217033 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108514 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 121782 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24116 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24121 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24121 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24121 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24121 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24121 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24121 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24121 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24121 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24117 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24117 |  364 | `		if( pStep == 0 ){` |
...
|  24117 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24117 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24117 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24117 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23865 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23865 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23865 |  400 | `				pIterMap = pMap;` |
|  23865 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23865 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23865 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23865 |  417 | `				if( pIterMap == pMap ){` |
|  23861 |  418 | `					pMap->iRef++;` |
|  11928 |  419 | `				}` |
...
|  23865 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11935 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24111 |  518 | `		if( pStep ){` |
|  24111 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12053 |  529 | `		}` |
|      - |  530 | `	}` |
|  24115 |  531 | `	VmPopOperand(&pTos,1);` |
|  24115 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12063 |  534 | `}` |
|      - |  535 |  |
