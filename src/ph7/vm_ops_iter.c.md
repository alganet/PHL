...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 244116 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 244121 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 244121 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 244121 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 244121 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 244121 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 244121 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 244121 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 244121 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 244121 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 244121 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 244121 |   59 | `	pStep = apStep[nStep - 1];` |
| 244147 |   60 | `	while( nStep > 0 ){` |
| 244147 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 244121 |   62 | `			pStep = apStep[nStep - 1];` |
| 244121 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 244121 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 242735 |   71 | `		pNode = pStep->pCursor;` |
| 242735 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23715 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23715 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23715 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11860 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 219025 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 219025 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6959 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6959 |   86 | `				if( pKey ){` |
|   6959 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3477 |   88 | `				}` |
|   3477 |   89 | `			}` |
| 219025 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 218955 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 218955 |  103 | `				if( pValue ){` |
| 218955 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 109475 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122756 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24142 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24147 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24147 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24147 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24147 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24147 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24147 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24147 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24147 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24143 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24143 |  364 | `		if( pStep == 0 ){` |
...
|  24143 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24143 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24143 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24143 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23891 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23891 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23891 |  400 | `				pIterMap = pMap;` |
|  23891 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23891 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23891 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23891 |  417 | `				if( pIterMap == pMap ){` |
|  23887 |  418 | `					pMap->iRef++;` |
|  11941 |  419 | `				}` |
...
|  23891 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11948 |  424 | `			}else{` |
...
|     53 |  451 | `					pIterAggClass = PH7_VmExtractClass(&(*pVm),"IteratorAggregate",` |
|      - |  452 | `						sizeof("IteratorAggregate")-1,FALSE,0);` |
|     64 |  453 | `					if( pIterAggClass && PH7_VmInstanceOf(pThis->pClass,pIterAggClass) ){` |
...
|      - |  517 | `		}` |
|  24137 |  518 | `		if( pStep ){` |
|  24137 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12066 |  529 | `		}` |
|      - |  530 | `	}` |
|  24141 |  531 | `	VmPopOperand(&pTos,1);` |
|  24141 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12076 |  534 | `}` |
|      - |  535 |  |
