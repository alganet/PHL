...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 243102 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 243107 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 243107 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 243107 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 243107 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 243107 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 243107 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 243107 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 243107 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 243107 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 243107 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 243107 |   59 | `	pStep = apStep[nStep - 1];` |
| 243133 |   60 | `	while( nStep > 0 ){` |
| 243133 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 243107 |   62 | `			pStep = apStep[nStep - 1];` |
| 243107 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 243107 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241721 |   71 | `		pNode = pStep->pCursor;` |
| 241721 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23663 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23663 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23663 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11834 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 218063 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 218063 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6947 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6947 |   86 | `				if( pKey ){` |
|   6947 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3471 |   88 | `				}` |
|   3471 |   89 | `			}` |
| 218063 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217993 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217993 |  103 | `				if( pValue ){` |
| 217993 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108994 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122249 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24090 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24095 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24095 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24095 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24095 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24095 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24095 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24095 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24095 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24091 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24091 |  364 | `		if( pStep == 0 ){` |
...
|  24091 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24091 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24091 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24091 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23839 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23839 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23839 |  400 | `				pIterMap = pMap;` |
|  23839 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23839 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23839 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23839 |  417 | `				if( pIterMap == pMap ){` |
|  23835 |  418 | `					pMap->iRef++;` |
|  11915 |  419 | `				}` |
...
|  23839 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11922 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24085 |  518 | `		if( pStep ){` |
|  24085 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12040 |  529 | `		}` |
|      - |  530 | `	}` |
|  24089 |  531 | `	VmPopOperand(&pTos,1);` |
|  24089 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12050 |  534 | `}` |
|      - |  535 |  |
