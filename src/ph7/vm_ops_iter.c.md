...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 242840 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 242845 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 242845 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 242845 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 242845 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 242845 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 242845 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 242845 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 242845 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 242845 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 242845 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 242845 |   59 | `	pStep = apStep[nStep - 1];` |
| 242871 |   60 | `	while( nStep > 0 ){` |
| 242871 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 242845 |   62 | `			pStep = apStep[nStep - 1];` |
| 242845 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 242845 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241459 |   71 | `		pNode = pStep->pCursor;` |
| 241459 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23679 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23679 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23679 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11842 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 217785 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 217785 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6973 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6973 |   86 | `				if( pKey ){` |
|   6973 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3484 |   88 | `				}` |
|   3484 |   89 | `			}` |
| 217785 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217715 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217715 |  103 | `				if( pValue ){` |
| 217715 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108855 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122118 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24106 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24111 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24111 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24111 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24111 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24111 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24111 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24111 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24111 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24107 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24107 |  364 | `		if( pStep == 0 ){` |
...
|  24107 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24107 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24107 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24107 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23855 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23855 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23855 |  400 | `				pIterMap = pMap;` |
|  23855 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23855 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23855 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23855 |  417 | `				if( pIterMap == pMap ){` |
|  23851 |  418 | `					pMap->iRef++;` |
|  11923 |  419 | `				}` |
...
|  23855 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11930 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24101 |  518 | `		if( pStep ){` |
|  24101 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12048 |  529 | `		}` |
|      - |  530 | `	}` |
|  24105 |  531 | `	VmPopOperand(&pTos,1);` |
|  24105 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12058 |  534 | `}` |
|      - |  535 |  |
