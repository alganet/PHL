...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 242922 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 242927 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 242927 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 242927 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 242927 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 242927 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 242927 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 242927 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 242927 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 242927 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 242927 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 242927 |   59 | `	pStep = apStep[nStep - 1];` |
| 242953 |   60 | `	while( nStep > 0 ){` |
| 242953 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 242927 |   62 | `			pStep = apStep[nStep - 1];` |
| 242927 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 242927 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241541 |   71 | `		pNode = pStep->pCursor;` |
| 241541 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23693 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23693 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23693 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11849 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 217853 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 217853 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6967 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6967 |   86 | `				if( pKey ){` |
|   6967 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3481 |   88 | `				}` |
|   3481 |   89 | `			}` |
| 217853 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217783 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217783 |  103 | `				if( pValue ){` |
| 217783 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108889 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122159 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24120 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24125 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24125 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24125 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24125 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24125 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24125 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24125 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24125 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24121 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24121 |  364 | `		if( pStep == 0 ){` |
...
|  24121 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24121 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24121 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24121 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23869 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23869 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23869 |  400 | `				pIterMap = pMap;` |
|  23869 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23869 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23869 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23869 |  417 | `				if( pIterMap == pMap ){` |
|  23865 |  418 | `					pMap->iRef++;` |
|  11930 |  419 | `				}` |
...
|  23869 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11937 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24115 |  518 | `		if( pStep ){` |
|  24115 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12055 |  529 | `		}` |
|      - |  530 | `	}` |
|  24119 |  531 | `	VmPopOperand(&pTos,1);` |
|  24119 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12065 |  534 | `}` |
|      - |  535 |  |
