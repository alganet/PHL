...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 243410 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 243415 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 243415 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 243415 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 243415 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 243415 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 243415 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 243415 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 243415 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 243415 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 243415 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 243415 |   59 | `	pStep = apStep[nStep - 1];` |
| 243441 |   60 | `	while( nStep > 0 ){` |
| 243441 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 243415 |   62 | `			pStep = apStep[nStep - 1];` |
| 243415 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 243415 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 242029 |   71 | `		pNode = pStep->pCursor;` |
| 242029 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23683 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23683 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23683 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11844 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 218351 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 218351 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6951 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6951 |   86 | `				if( pKey ){` |
|   6951 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3473 |   88 | `				}` |
|   3473 |   89 | `			}` |
| 218351 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 218281 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 218281 |  103 | `				if( pValue ){` |
| 218281 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 109138 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122403 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24110 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24115 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24115 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24115 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24115 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24115 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24115 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24115 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24115 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24111 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24111 |  364 | `		if( pStep == 0 ){` |
...
|  24111 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24111 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24111 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24111 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23859 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23859 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23859 |  400 | `				pIterMap = pMap;` |
|  23859 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23859 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23859 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23859 |  417 | `				if( pIterMap == pMap ){` |
|  23855 |  418 | `					pMap->iRef++;` |
|  11925 |  419 | `				}` |
...
|  23859 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11932 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24105 |  518 | `		if( pStep ){` |
|  24105 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12050 |  529 | `		}` |
|      - |  530 | `	}` |
|  24109 |  531 | `	VmPopOperand(&pTos,1);` |
|  24109 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12060 |  534 | `}` |
|      - |  535 |  |
