...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 260998 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 261003 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 261003 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 261003 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 261003 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 261003 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 261003 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 261003 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 261003 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 261003 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 261003 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 261003 |   59 | `	pStep = apStep[nStep - 1];` |
| 261029 |   60 | `	while( nStep > 0 ){` |
| 261029 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 261003 |   62 | `			pStep = apStep[nStep - 1];` |
| 261003 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 261003 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 259617 |   71 | `		pNode = pStep->pCursor;` |
| 259617 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23617 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23617 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23617 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11811 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 236005 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 236005 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6935 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6935 |   86 | `				if( pKey ){` |
|   6935 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3465 |   88 | `				}` |
|   3465 |   89 | `			}` |
| 236005 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 235935 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 235935 |  103 | `				if( pValue ){` |
| 235935 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 117965 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 131197 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24044 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24049 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24049 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24049 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24049 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24049 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24049 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24049 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24049 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24045 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24045 |  364 | `		if( pStep == 0 ){` |
...
|  24045 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24045 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24045 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24045 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23793 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23793 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23793 |  400 | `				pIterMap = pMap;` |
|  23793 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23793 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23793 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23793 |  417 | `				if( pIterMap == pMap ){` |
|  23789 |  418 | `					pMap->iRef++;` |
|  11892 |  419 | `				}` |
...
|  23793 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11899 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24039 |  518 | `		if( pStep ){` |
|  24039 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12017 |  529 | `		}` |
|      - |  530 | `	}` |
|  24043 |  531 | `	VmPopOperand(&pTos,1);` |
|  24043 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12027 |  534 | `}` |
|      - |  535 |  |
