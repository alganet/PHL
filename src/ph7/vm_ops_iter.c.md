...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 260456 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 260461 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 260461 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 260461 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 260461 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 260461 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 260461 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 260461 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 260461 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 260461 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 260461 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 260461 |   59 | `	pStep = apStep[nStep - 1];` |
| 260487 |   60 | `	while( nStep > 0 ){` |
| 260487 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 260461 |   62 | `			pStep = apStep[nStep - 1];` |
| 260461 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 260461 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 259075 |   71 | `		pNode = pStep->pCursor;` |
| 259075 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23589 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23589 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23589 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11797 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 235491 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 235491 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6877 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6877 |   86 | `				if( pKey ){` |
|   6877 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3436 |   88 | `				}` |
|   3436 |   89 | `			}` |
| 235491 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 235421 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 235421 |  103 | `				if( pValue ){` |
| 235421 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 117708 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 130926 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24016 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24021 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24021 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24021 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24021 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24021 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24021 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24021 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24021 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24017 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24017 |  364 | `		if( pStep == 0 ){` |
...
|  24017 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24017 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24017 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24017 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23765 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23765 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23765 |  400 | `				pIterMap = pMap;` |
|  23765 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23765 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23765 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23765 |  417 | `				if( pIterMap == pMap ){` |
|  23761 |  418 | `					pMap->iRef++;` |
|  11878 |  419 | `				}` |
...
|  23765 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11885 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24011 |  518 | `		if( pStep ){` |
|  24011 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12003 |  529 | `		}` |
|      - |  530 | `	}` |
|  24015 |  531 | `	VmPopOperand(&pTos,1);` |
|  24015 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12013 |  534 | `}` |
|      - |  535 |  |
