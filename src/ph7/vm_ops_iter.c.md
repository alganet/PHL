...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 259106 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 259111 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 259111 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 259111 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 259111 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 259111 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 259111 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 259111 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 259111 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 259111 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 259111 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 259111 |   59 | `	pStep = apStep[nStep - 1];` |
| 259137 |   60 | `	while( nStep > 0 ){` |
| 259137 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 259111 |   62 | `			pStep = apStep[nStep - 1];` |
| 259111 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 259111 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 257737 |   71 | `		pNode = pStep->pCursor;` |
| 257737 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  22969 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  22969 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  22969 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11487 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 234773 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 234773 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6861 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6861 |   86 | `				if( pKey ){` |
|   6861 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3428 |   88 | `				}` |
|   3428 |   89 | `			}` |
| 234773 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 234703 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 234703 |  103 | `				if( pValue ){` |
| 234703 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 117349 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 130245 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  23392 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  23397 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  23397 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  23397 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  23397 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  23397 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  23397 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  23397 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  23397 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  23393 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  23393 |  364 | `		if( pStep == 0 ){` |
...
|  23393 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  23393 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  23393 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  23393 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23145 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23145 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23145 |  400 | `				pIterMap = pMap;` |
|  23145 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23145 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23145 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23145 |  417 | `				if( pIterMap == pMap ){` |
|  23141 |  418 | `					pMap->iRef++;` |
|  11568 |  419 | `				}` |
...
|  23145 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11575 |  424 | `			}else{` |
|    253 |  425 | `				ph7_class_instance *pThis = (ph7_class_instance *)pTos->x.pOther;` |
|      - |  426 | `				ph7_class *pIteratorClass;` |
|      - |  427 | `				/* Check if the object implements Iterator */` |
|    253 |  428 | `				pIteratorClass = PH7_VmExtractClass(&(*pVm),"Iterator",sizeof("Iterator")-1,FALSE,0);` |
|    352 |  429 | `				if( pIteratorClass && PH7_VmInstanceOf(pThis->pClass,pIteratorClass) ){` |
...
|      - |  517 | `		}` |
|  23387 |  518 | `		if( pStep ){` |
|  23387 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  11691 |  529 | `		}` |
|      - |  530 | `	}` |
|  23391 |  531 | `	VmPopOperand(&pTos,1);` |
|  23391 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  11701 |  534 | `}` |
|      - |  535 |  |
