...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 261076 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 261081 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 261081 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 261081 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 261081 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 261081 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 261081 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 261081 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 261081 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 261081 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 261081 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 261081 |   59 | `	pStep = apStep[nStep - 1];` |
| 261107 |   60 | `	while( nStep > 0 ){` |
| 261107 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 261081 |   62 | `			pStep = apStep[nStep - 1];` |
| 261081 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 261081 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 259695 |   71 | `		pNode = pStep->pCursor;` |
| 259695 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23621 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23621 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23621 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11813 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 236079 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 236079 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6935 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6935 |   86 | `				if( pKey ){` |
|   6935 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3465 |   88 | `				}` |
|   3465 |   89 | `			}` |
| 236079 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 236009 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 236009 |  103 | `				if( pValue ){` |
| 236009 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 118002 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 131236 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24048 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24053 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24053 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24053 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24053 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24053 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24053 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24053 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24053 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24049 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24049 |  364 | `		if( pStep == 0 ){` |
...
|  24049 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24049 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24049 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24049 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23797 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23797 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23797 |  400 | `				pIterMap = pMap;` |
|  23797 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23797 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23797 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23797 |  417 | `				if( pIterMap == pMap ){` |
|  23793 |  418 | `					pMap->iRef++;` |
|  11894 |  419 | `				}` |
...
|  23797 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11901 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24043 |  518 | `		if( pStep ){` |
|  24043 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12019 |  529 | `		}` |
|      - |  530 | `	}` |
|  24047 |  531 | `	VmPopOperand(&pTos,1);` |
|  24047 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12029 |  534 | `}` |
|      - |  535 |  |
