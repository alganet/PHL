...
|      - |   28 | ` * VM_EXIT_BREAK.` |
|      - |   29 | ` */` |
| 242764 |   30 | `PH7_PRIVATE VmOpRc VmExecOpForeachStep(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |   31 | `{` |
| 242769 |   32 | `	ph7_value *pTos = pState->pTos;` |
| 242769 |   33 | `	ph7_value *pStack = pState->pStack;` |
| 242769 |   34 | `	VmInstr *aInstr = pState->aInstr;` |
| 242769 |   35 | `	sxi32 pc = pState->pc;` |
|      - |   36 | `	sxi32 rc;` |
| 242769 |   37 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |   38 | `	ph7_foreach_step **apStep,*pStep;` |
|      - |   39 | `	ph7_value *pValue;` |
|      - |   40 | `	VmFrame *pFrameLocal;` |
|      - |   41 | `	sxu32 nStep;` |
| 242769 |   42 | `	pFrameLocal = pVm->pFrame;` |
| 242769 |   43 | `	pFrameLocal = VmSkipExceptionFrames(pFrameLocal);` |
...
| 242769 |   50 | `	apStep = (ph7_foreach_step **)SySetBasePtr(&pInfo->aStep);` |
| 242769 |   51 | `	nStep = SySetUsed(&pInfo->aStep);` |
| 242769 |   52 | `	if( nStep < 1 ){` |
...
|    ! 0 |   57 | `		VM_EXIT_BREAK;` |
|      - |   58 | `	}` |
| 242769 |   59 | `	pStep = apStep[nStep - 1];` |
| 242795 |   60 | `	while( nStep > 0 ){` |
| 242795 |   61 | `		if( apStep[nStep - 1]->pFrame == pFrameLocal ){` |
| 242769 |   62 | `			pStep = apStep[nStep - 1];` |
| 242769 |   63 | `			break;` |
|      - |   64 | `		}` |
|     27 |   65 | `		nStep--;` |
|      1 |   66 | `	}` |
| 242769 |   67 | `	if( pStep->iFlags & PH7_4EACH_STEP_HASHMAP ){` |
...
| 241383 |   71 | `		pNode = pStep->pCursor;` |
| 241383 |   72 | `		if( pNode == 0 ){` |
|      - |   73 | `			/* No more entry to process */` |
|  23691 |   74 | `			pc = pInstr->iP2 - 1; /* Jump to this destination */` |
|  23691 |   75 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
|  23691 |   80 | `			VmForeachHashmapStepRelease(&(*pVm),pInfo,pStep,TRUE);` |
|  11848 |   81 | `		}else{` |
|      - |   82 | `			/* Advance the private cursor */` |
| 217697 |   83 | `			pStep->pCursor = pNode->pPrev; /* Reverse link */` |
| 217697 |   84 | `			if( (pStep->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) > 0 ){` |
|   6981 |   85 | `				ph7_value *pKey = VmExtractMemObj(&(*pVm),&pInfo->sKey,FALSE,TRUE);` |
|   6981 |   86 | `				if( pKey ){` |
|   6981 |   87 | `					PH7_HashmapExtractNodeKey(pNode,pKey);` |
|   3488 |   88 | `				}` |
|   3488 |   89 | `			}` |
| 217697 |   90 | `			if( pStep->iFlags & PH7_4EACH_STEP_REF ){` |
...
| 217627 |  102 | `				pValue = VmExtractMemObj(&(*pVm),&pInfo->sValue,FALSE,TRUE);` |
| 217627 |  103 | `				if( pValue ){` |
| 217627 |  104 | `					PH7_HashmapExtractNodeValue(pNode,pValue,TRUE);` |
| 108811 |  105 | `				}` |
|      - |  106 | `			}` |
|      5 |  107 | `		}` |
| 122080 |  108 | `	}else if( pStep->iFlags & PH7_4EACH_STEP_ITERATOR ){` |
...
|      - |  311 | ` */` |
|  24118 |  312 | `PH7_PRIVATE VmOpRc VmExecOpForeachInit(ph7_vm *pVm,VmExecState *pState,VmInstr *pInstr)` |
|      5 |  313 | `{` |
|  24123 |  314 | `	ph7_value *pTos = pState->pTos;` |
|  24123 |  315 | `	ph7_value *pStack = pState->pStack;` |
|  24123 |  316 | `	VmInstr *aInstr = pState->aInstr;` |
|  24123 |  317 | `	sxi32 pc = pState->pc;` |
|      - |  318 | `	sxi32 rc;` |
|  24123 |  319 | `	ph7_foreach_info *pInfo = (ph7_foreach_info *)pInstr->p3;` |
|      - |  320 | `	void *pName;` |
|      - |  321 | `#ifdef UNTRUST` |
|      - |  322 | `	if( pTos < pStack ){` |
|      - |  323 | `		VM_EXIT_ABORT;` |
|      - |  324 | `	}` |
|      - |  325 | `#endif` |
|  24123 |  326 | `	if( SyStringLength(&pInfo->sValue) < 1 ){` |
...
|    ! 0 |  338 | `	}` |
|  24123 |  339 | `	if( (pInfo->iFlags & PH7_4EACH_STEP_KEY) && SyStringLength(&pInfo->sKey) < 1 ){` |
...
|  24123 |  352 | `	if( (pTos->iFlags & (MEMOBJ_HASHMAP\|MEMOBJ_OBJ)) == 0 \|\| SyStringLength(&pInfo->sValue) < 1 ){` |
...
|  24119 |  363 | `		pStep = (ph7_foreach_step *)SyMemBackendPoolAlloc(&pVm->sAllocator,sizeof(ph7_foreach_step));` |
|  24119 |  364 | `		if( pStep == 0 ){` |
...
|  24119 |  370 | `			SyZero(pStep,sizeof(ph7_foreach_step));` |
|      - |  371 | `			/* Prepare the step */` |
|  24119 |  372 | `			pStep->iFlags = pInfo->iFlags;` |
...
|  24119 |  377 | `			pStep->pFrame = VmSkipExceptionFrames(pVm->pFrame);` |
|  24119 |  378 | `			if( pTos->iFlags & MEMOBJ_HASHMAP ){` |
...
|  23867 |  382 | `				if( (pStep->iFlags & PH7_4EACH_STEP_REF) && pTos->nIdx != SXU32_HIGH ){` |
...
|     13 |  397 | `					}` |
|     13 |  398 | `				}` |
|  23867 |  399 | `				pMap = (ph7_hashmap *)pTos->x.pOther;` |
|  23867 |  400 | `				pIterMap = pMap;` |
|  23867 |  401 | `				if( pMap == pVm->pGlobal && (pStep->iFlags & PH7_4EACH_STEP_REF) == 0 ){` |
...
|    ! 0 |  413 | `					}` |
|      2 |  414 | `				}` |
|  23867 |  415 | `				pStep->iFlags \|= PH7_4EACH_STEP_HASHMAP;` |
|  23867 |  416 | `				pStep->xIter.pMap = pIterMap;` |
|  23867 |  417 | `				if( pIterMap == pMap ){` |
|  23863 |  418 | `					pMap->iRef++;` |
|  11929 |  419 | `				}` |
...
|  23867 |  423 | `				PH7_HashmapRegisterForeachStep(pIterMap,pStep);` |
|  11936 |  424 | `			}else{` |
...
|      - |  517 | `		}` |
|  24113 |  518 | `		if( pStep ){` |
|  24113 |  519 | `			if( SXRET_OK != SySetPut(&pInfo->aStep,(const void *)&pStep) ){` |
...
|  12054 |  529 | `		}` |
|      - |  530 | `	}` |
|  24117 |  531 | `	VmPopOperand(&pTos,1);` |
|  24117 |  532 | `	VM_EXIT_BREAK;` |
|    ! 0 |  533 | `	VM_EXIT_BREAK;` |
|  12064 |  534 | `}` |
|      - |  535 |  |
