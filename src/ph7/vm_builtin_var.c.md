...
|       - |   22 | ` */` |
|  115254 |   23 | `PH7_PRIVATE int vm_builtin_isset(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|       5 |   24 | `{` |
|       - |   25 | `	ph7_value *pObj;` |
|  115259 |   26 | `	int res = 0;` |
|       - |   27 | `	int i;` |
...
|  148037 |   34 | `	for( i = 0 ; i < nArg ; ++i ){` |
|  115271 |   35 | `		pObj = apArg[i];` |
|  115271 |   36 | `		if( pObj->nIdx == SXU32_HIGH ){` |
...
|   79667 |   40 | `			if( (pObj->iFlags & (MEMOBJ_NULL\|MEMOBJ_BOOL)) == 0 ){` |
|       - |   41 | `				/* Not so fatal,Throw a warning */` |
|     ! 0 |   42 | `				ph7_context_throw_error(pCtx,PH7_CTX_WARNING,"Expecting a variable not a constant");` |
|     ! 0 |   43 | `			}` |
|   39831 |   44 | `		}` |
|  115271 |   45 | `		res = (pObj->iFlags & MEMOBJ_NULL) ? 0 : 1;` |
|  115271 |   46 | `		if( !res ){` |
|       - |   47 | `			/* Variable not set,return FALSE */` |
|   82493 |   48 | `			ph7_result_bool(pCtx,0);` |
|   82493 |   49 | `			return SXRET_OK;` |
|       - |   50 | `		}` |
|   16394 |   51 | `	}` |
|       - |   52 | `	/* All given variable are set,return TRUE */` |
|   32771 |   53 | `	ph7_result_bool(pCtx,1);` |
|   32771 |   54 | `	return SXRET_OK;` |
|   57632 |   55 | `}` |
|       - |   56 | `/*` |
...
|       - |   93 | `	}` |
|    5784 |   94 | `	nIdx = SX_PTR_TO_INT(pEntry->pUserData);` |
|    5784 |   95 | `	if( nIdx == pVm->nGlobalIdx ){` |
...
|       - |  101 | `	}` |
|    5784 |  102 | `	pRef = VmRefObjExtract(&(*pVm),nIdx);` |
|       - |  103 | `	/*` |
...
|       - |  110 | `	 */` |
|    5784 |  111 | `	if( pFrame->pParent == 0 ){` |
|    5782 |  112 | `		ph7_value *pGlobals = (ph7_value *)SySetAt(&pVm->aMemObj,pVm->nGlobalIdx);` |
|    5782 |  113 | `		if( pGlobals && (pGlobals->iFlags & MEMOBJ_HASHMAP) ){` |
|    5782 |  114 | `			ph7_hashmap_node *pNode = 0;` |
|       - |  115 | `			ph7_value sKey;` |
|       - |  116 | `			SyString sName;` |
|    5782 |  117 | `			PH7_MemObjInit(&(*pVm),&sKey);` |
|    5782 |  118 | `			SyStringInitFromBuf(&sName,zName,nByte);` |
|    5782 |  119 | `			PH7_MemObjInitFromString(&(*pVm),&sKey,&sName);` |
|    5778 |  120 | `			if( SXRET_OK == PH7_HashmapLookup((ph7_hashmap *)pGlobals->x.pOther,&sKey,&pNode)` |
|    5782 |  121 | `			 && pNode && pNode->nValIdx == nIdx ){` |
|    5780 |  122 | `				if( pRef ){` |
|    5780 |  123 | `					ph7_hashmap_node **apN = (ph7_hashmap_node **)SySetBasePtr(&pRef->aArrEntries);` |
|       - |  124 | `					sxu32 k;` |
|   11654 |  125 | `					for( k = 0 ; k < SySetUsed(&pRef->aArrEntries) ; ++k ){` |
|    5878 |  126 | `						if( apN[k] == pNode ){` |
|    5780 |  127 | `							apN[k] = 0;` |
|    2888 |  128 | `						}` |
|    2941 |  129 | `					}` |
|    2888 |  130 | `				}` |
|    5780 |  131 | `				PH7_HashmapUnlinkNode(pNode,FALSE);` |
|    2888 |  132 | `			}` |
|    5782 |  133 | `			PH7_MemObjRelease(&sKey);` |
|    2889 |  134 | `		}` |
|    2889 |  135 | `	}` |
|       - |  136 |  |
|    5784 |  137 | `	if( pRef == 0 ){` |
...
|       - |  146 | `	}` |
|       - |  147 | `	{` |
|    5784 |  148 | `		SyHashEntry **apEntry = (SyHashEntry **)SySetBasePtr(&pRef->aReference);` |
|    5784 |  149 | `		ph7_hashmap_node **apNode = (ph7_hashmap_node **)SySetBasePtr(&pRef->aArrEntries);` |
|    5784 |  150 | `		sxu32 n, nLive = 0;` |
|       - |  151 | `		/* Forget THIS name in the slot's reference record (leave the others alone) */` |
|   11660 |  152 | `		for( n = 0 ; n < SySetUsed(&pRef->aReference) ; ++n ){` |
|    5880 |  153 | `			if( apEntry[n] == pEntry ){` |
|    5778 |  154 | `				apEntry[n] = 0;` |
|    2887 |  155 | `			}` |
|    2942 |  156 | `		}` |
|    5784 |  157 | `		SyHashDeleteEntry2(pEntry);` |
|       - |  158 | `		/* Anything else still holding the slot? */` |
|   11660 |  159 | `		for( n = 0 ; n < SySetUsed(&pRef->aReference) ; ++n ){` |
|    5880 |  160 | `			if( apEntry[n] ){` |
|      29 |  161 | `				nLive++;` |
|      14 |  162 | `			}` |
|    2942 |  163 | `		}` |
|   11660 |  164 | `		for( n = 0 ; n < SySetUsed(&pRef->aArrEntries) ; ++n ){` |
...
|    5880 |  170 | `			if( apNode[n] && apNode[n]->nValIdx == nIdx ){` |
|      63 |  171 | `				nLive++;` |
|      31 |  172 | `			}` |
|    2942 |  173 | `		}` |
|    5784 |  174 | `		if( nLive < 1 ){` |
|       - |  175 | `			/* Last holder gone: now the value may go too */` |
|    5728 |  176 | `			PH7_VmUnsetMemObj(&(*pVm),nIdx,FALSE);` |
...
|    5728 |  180 | `			VmDropFrameLocalSlot(&(*pVm),nIdx);` |
|    2862 |  181 | `		}` |
|       - |  182 | `	}` |
|    5784 |  183 | `	return SXRET_OK;` |
|    3356 |  184 | `}` |
|       - |  185 | `/*` |
...
|       - |  191 | ` */` |
|  709838 |  192 | `PH7_PRIVATE int PH7_VmSlotIsReferenced(ph7_vm *pVm,sxu32 nIdx)` |
|       5 |  193 | `{` |
|       - |  194 | `	VmRefObj *pRef;` |
|  709843 |  195 | `	sxu32 n, nLive = 0;` |
|       - |  196 | `	SyHashEntry **apEntry;` |
|  709843 |  197 | `	if( nIdx == SXU32_HIGH ){` |
|     ! 0 |  198 | `		return 0;` |
|       - |  199 | `	}` |
|  709843 |  200 | `	pRef = VmRefObjExtract(&(*pVm),nIdx);` |
|  709843 |  201 | `	if( pRef == 0 ){` |
|     ! 0 |  202 | `		return 0;` |
|       - |  203 | `	}` |
|  709843 |  204 | `	apEntry = (SyHashEntry **)SySetBasePtr(&pRef->aReference);` |
|  709847 |  205 | `	for( n = 0 ; n < SySetUsed(&pRef->aReference) ; ++n ){` |
|       5 |  206 | `		if( apEntry[n] ){` |
|       5 |  207 | `			nLive++;` |
|       2 |  208 | `		}` |
|       3 |  209 | `	}` |
...
|  354924 |  211 | `}` |
| 4233509 |  212 | `PH7_PRIVATE sxi32 PH7_VmUnsetMemObj(ph7_vm *pVm,sxu32 nObjIdx,int bForce)` |
|       5 |  213 | `{` |
|       - |  214 | `	ph7_value *pObj;` |
|       - |  215 | `	VmRefObj *pRef;` |
| 4233514 |  216 | `	pObj = (ph7_value *)SySetAt(&pVm->aMemObj,nObjIdx);` |
| 4233514 |  217 | `	if( pObj ){` |
|       - |  218 | `		/* Release the object */` |
| 4233514 |  219 | `		PH7_MemObjRelease(pObj);` |
| 2117378 |  220 | `	}` |
|       - |  221 | `	/* Remove old reference links */` |
| 4233514 |  222 | `	pRef = VmRefObjExtract(&(*pVm),nObjIdx);` |
| 4233514 |  223 | `	if( pRef ){` |
| 4233514 |  224 | `		sxi32 iFlags = pRef->iFlags;` |
|       - |  225 | `		/* Unlink from the reference table */` |
| 4233514 |  226 | `		VmRefObjUnlink(&(*pVm),pRef);` |
| 4233514 |  227 | `		if( (bForce == TRUE) \|\| (iFlags & VM_REF_IDX_KEEP) == 0 ){` |
|       - |  228 | `			VmSlot sFree;` |
|       - |  229 | `			/* Restore to the free list */` |
| 4233502 |  230 | `			sFree.nIdx = nObjIdx;` |
| 4233502 |  231 | `			sFree.pUserData = 0;` |
| 4233502 |  232 | `			SySetPut(&pVm->aFreeObj,(const void *)&sFree);` |
| 2117372 |  233 | `		}` |
| 2117378 |  234 | `	}` |
| 4233514 |  235 | `	return SXRET_OK;` |
|       5 |  236 | `}` |
|       - |  237 | `/*` |
|       - |  238 | ` * void unset($var,...)` |
...
|       - |  422 | ` *  Nothing.` |
|       - |  423 | ` */` |
|     626 |  424 | `PH7_PRIVATE int vm_builtin_var_dump(ph7_context *pCtx,int nArg,ph7_value **apArg)` |
|       4 |  425 | `{` |
|       - |  426 | `	SyBlob sDump; /* Generated dump is stored here */` |
|       - |  427 | `	int i;` |
|     630 |  428 | `	SyBlobInit(&sDump,&pCtx->pVm->sAllocator);` |
|       - |  429 | `	/* Dump one or more expressions */` |
|    1338 |  430 | `	for( i = 0 ; i < nArg ; i++ ){` |
|     712 |  431 | `		ph7_value *pObj = apArg[i];` |
|       - |  432 | `		/* Reset the working buffer */` |
|     712 |  433 | `		SyBlobReset(&sDump);` |
|       - |  434 | `		/* Dump the given expression */` |
|     712 |  435 | `		PH7_MemObjDump(&sDump,pObj,TRUE,0,0,0);` |
|       - |  436 | `		/* Output */` |
|     712 |  437 | `		if( SyBlobLength(&sDump) > 0 ){` |
|     712 |  438 | `			ph7_context_output(pCtx,(const char *)SyBlobData(&sDump),(int)SyBlobLength(&sDump));` |
|     354 |  439 | `		}` |
|     358 |  440 | `	}` |
|       - |  441 | `	/* Release the working buffer */` |
|     630 |  442 | `	SyBlobRelease(&sDump);` |
|     630 |  443 | `	return SXRET_OK;` |
|       4 |  444 | `}` |
|       - |  445 | `/*` |
